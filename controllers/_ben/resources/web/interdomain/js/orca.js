...
// site models to load
var siteModels = [
		  "http://geni-orca.renci.org/owl/mass.rdf",
 		  "http://geni-orca.renci.org/owl/ben.rdf",
 		  "http://geni-orca.renci.org/owl/dukevmsite.rdf",
 		  "http://geni-orca.renci.org/owl/rencivmsite.rdf",
...
var ndlSchemas = [
		  ...
		  "http://geni-orca.renci.org/owl/layer.owl",
		  "http://geni-orca.renci.org/owl/collections.owl",
		  "http://geni-orca.renci.org/owl/location.owl",
		  "http://geni-orca.renci.org/owl/compute.owl",
		  "http://geni-orca.renci.org/owl/dtn.owl",
		  "http://geni-orca.renci.org/owl/ethernet.owl",
		  "http://geni-orca.renci.org/owl/itu-grid.owl",
		  "http://geni-orca.renci.org/owl/orca.owl",
		  "http://geni-orca.renci.org/owl/request.owl",
		  "http://geni-orca.renci.org/owl/storage.owl",
		  "http://geni-orca.renci.org/owl/collections.owl"
		  ];
...
var knownResourceTypes = { VM: { unitCount: "orca:numORCAUnitServer",
				 ls: "<http://geni-orca.renci.org/owl/compute.owl#availableVMLabelSet>",
				 query: guiQueries.vmUnits },
			   GEPort: { unitCount: "ethernet:numGigabitEthernetPort",
				     ls: "<http://geni-orca.renci.org/owl/compute.owl#availablePortLabelSet>",
				     query: guiQueries.portUnits},
			   TenGEPort: { unitCount: "ethernet:numTenGigabitEthernetPort",
					ls: "<http://geni-orca.renci.org/owl/compute.owl#availablePortLabelSet>",
					query: guiQueries.portUnits},
			   ViseTestbed: { unitCount: "orca:numORCAUnitServer",
					  ls: "<http://geni-orca.renci.org/owl/compute.owl#availableVMLabelSet>",
					  query: guiQueries.vmUnits } };
...
function onStartButtonClick(p_oEvent) {
  startButton.setAttributes({disabled: true});
  checkButton.setAttributes({disabled: false});
  cancelButton.setAttributes({disabled: false} );
  InitializeView();
}
...
function onCancelButtonClick(p_oEvent) {
  ...
  submitButton.setAttributes({disabled: true});
}
...
function domainUnitRequest(rec) {
  ...
  rdfRequest += '<' + knownResourceTypes[rec.getData('typeOfResource')].unitCount + ' rdf:datatype="&xsd;integer">' + rec.getData('selected') + '</' + knownResourceTypes[rec.getData('typeOfResource')].unitCount + '>';
...
function formRDFRequest() {
  ...
  tlog("Connection from " + connections[0]['from'].getData("domain") + 
	" to " + connections[0]['to'].getData("domain") + "\n");
  ...
  rdfRequest += '<request:endingTime>' + 
  convertToXMLTime(endDate, endTime) + '</request:endingTime>';
  ...
  for(i=0; i < connections.length; i++ ) {
    ...
    rdfRequest += '<topology:hasInterface rdf:resource="' + uriStripBrackets(connections[i]['from'].getData('device')) + '"/>';
    rdfRequest += '<topology:hasInterface rdf:resource="' + uriStripBrackets(connections[i]['to'].getData('device')) + '"/>';
...
//       guiPanel.beforeRenderEvent.subscribe(function() {
	  ...
	  Event.onAvailable('layout', function() {
		...
		units: [
			{ position: 'left', width: 350, header: 'Resource Selection', resize: true, body: 'left1', gutter: '5px', collapse: true, collapseSize: 50, scroll: true, animate: true },
			{ position: 'center', width: 500, body: 'center1', header: 'Resource Map', gutter: '5px' },
			{ position: 'bottom',  height: 40, resize: true, body: 'bottom1', gutter: '5px', collapse: false }
			]
		    });
	      layout.render();
	    });
...
//     });
  ...
  checkButton = new YAHOO.widget.Button("checkbutton", { disabled: true} );
  checkButton.on("click", onCheckButtonClick);

  cancelButton = new YAHOO.widget.Button("cancelbutton", { disabled: true });
  cancelButton.on("click", onCancelButtonClick);

  submitButton = new YAHOO.widget.Button("submitbutton", { disabled: true });
...
function RunNDLQuery () {
  ...
  for(i = 0; i< nv; i++ ) {
    v = q.vars[i];
    columnHeads.push({key: v.label, sortable: true});
    schema.push(v.label);
  }
...
function showPopResourcesForm(domain, pop) {
  ...
  function createProviderTable() {
    // create a table
    dataTable = new YAHOO.widget.DataTable("result0", 
					   columnHeads, dataSource, 
					   { });
  ...
  var columnHeads = [ 
                     ...
                     { key: "domain", label: "<b>Domain</b>" },
		     { key: "typeOfResource", label: "<b>Resource Type</b>" } ,
                     { key: "units", label: "<b>Available Units</b>" },
		     { key: "selected", label: "<b>Selected Units</b>", editor: new YAHOO.widget.TextboxCellEditor({disableBtns:true})}
		      ];
  var dataSource = new YAHOO.util.DataSource([]);
  dataSource.responseType = YAHOO.util.DataSource.TYPE_JSARRAY;
  dataSource.responseSchema = { fields: requestEndpoints.providerTableSchema };
  ...
  for(provider in resources) {
    ...
    if ((resources[provider]['pop1'] == pop) &&
	(resources[provider]['domain'] == domain)) {
      // add row to a table
      if (requestEndpoints.providerTable == null) {
	createProviderTable();
      } 
	...
	submitButton.setAttributes({disabled: false});
...
function getPopsForDomain(domain) {
  ...
  function processResults (bindings) {
    ...
    domains[domain].popsByName[popUri]['marker'] = createMarker(point, 
								domains[domain].popsByName[popUri]['friendlyName'], 
								domain, popUri, 
								mapIcons[domains[domain].color]);
...
function getDomains() {
  ...
  function processResults (bindings) {
    ...
    domains[mydom] = { color: iconColorPicker.colors[iconColorPicker.index],  popsByName: [] };
...
function CheckState() {
  ...
  for(provider in resources) {
    tinfo("Provider " + provider + " short name: " + resources[provider]['friendlyName'] + " domain: " + resources[provider]['domain'] + " resource type " +
	  resources[provider]['typeOfResource']);
  }
}
...
function createMarker(point, comment, domain, pop, popIcon) {
  var marker = new GMarker(point, { icon: popIcon }),
    d = domain, p = pop;
...
function loadMap() {
  ...
  for (color in iconColorPicker.colors) {
    ...
    mapIcons[colorName].image = "http://www.google.com/intl/en_us/mapfiles/ms/micons/" + 
      colorName + "-dot.png";
  }
  ...
  for(domain in domains) {
    for(pop in domains[domain].popsByName) {
      ...
      domains[domain].popsByName[pop]['marker'] = createMarker(point, 
							 escapeForXML(domains[domain].popsByName[pop]['popName']), 
							 domain, domains[domain].popsByName[pop]['popUri'],
							 mapIcons[domains[domain].color]);
