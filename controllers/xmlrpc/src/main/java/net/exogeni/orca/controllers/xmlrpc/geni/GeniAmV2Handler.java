...
import net.exogeni.orca.controllers.xmlrpc.SliceStateMachine.SliceState;
import net.exogeni.orca.controllers.xmlrpc.XMLRPCDateTransport;
import net.exogeni.orca.controllers.xmlrpc.XmlRpcController;
import net.exogeni.orca.controllers.xmlrpc.XmlrpcControllerSlice;
...
import net.exogeni.orca.ndl.NdlToRSpecHelper;
...
import net.exogeni.orca.util.VersionUtils;
...
/**
 * GENI AM API v2 implementation WARNING: Any method you declare public (non-static) becomes a remote method!
 * 
 * @author ibaldin
 *
 */
public class GeniAmV2Handler extends XmlrpcHandlerHelper implements IGeniAmV2Interface {
    ...
    private Map<String, Object> getSliceManifest(IOrcaServiceManager sm, String slice_urn, Object[] credentials,
            boolean compressed) {
        ...
        try {
            ...
            if (allRes == null) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    return getStandardApiReturn(ApiReturnCodes.ERROR.code, null,
                            "ERROR: Failed due to exception: " + e);
                }
     ...
     * @param users
     * @param addRoot
     *            - add a root login in addition to other users
     * @param rootUrn
     *            - if addRoot is set, use this urn's ssh key for root. If null, use the first key on the list.
     * @return
     */
    ...
    public Map<String, Object> CreateSliver(String slice_urn, Object[] credentials, String resReq,
            List<Map<String, ?>> users, Map<String, Object> options) {
        IOrcaServiceManager sm = null;
        try {
            logger.info("GENI AM v2 CreateSliver() invoked for " + slice_urn);
            Date saExpDate = validateGeniCredential(slice_urn, credentials,
                    new String[] { "*", "pi", "instantiate", "control" }, options, verifyCredentials, logger);
            ...
            // submit the request
            Map<String, Object> rr = orcaHandler.createSlice(slice_urn, credentials, ndlReq,
                    stripLogins(users, useGENIRootLogin, XmlrpcHandlerHelper.getCredentialDN(logger)));
            String ret;
    ...
    public Map<String, Object> DeleteSliver(String slice_urn, Object[] credentials, Map<String, Object> options) {
        ...
        try {
            ...
            validateGeniCredential(slice_urn, credentials, new String[] { "*", "pi", "instantiate", "control" },
                    options, verifyCredentials, logger);
    ...
    /**
     * List resources can return resources of the AM or the slice depending on the options
     */
    ...
    public Map<String, Object> ListResources(Object[] credentials, Map<?, ?> options) {
        ...
        try {
            ...
            if (slice_urn != null) {
                validateGeniCredential(slice_urn, credentials, new String[] { "*", "pi", "instantiate", "control" }, op,
                ...
                validateGeniCredential(null, credentials,
                        new String[] { "*", "pi", "instantiate", "control", "info", "resolve" }, op, verifyCredentials,
                        logger);
                // all resources
    ...
    public Map<String, Object> ListResources(Object[] credentials) {
        try {
            validateGeniCredential(null, credentials,
                    new String[] { "*", "pi", "instantiate", "control", "info", "resolve" }, null, verifyCredentials,
                    logger);
            ...
            if ((Boolean) res.get("err")) {
                logger.error("GENI ListResources: Error encountered converting NDL ads to RSpec via converter service: "
                        + (String) res.get("msg"));
                return getStandardApiReturn(ApiReturnCodes.ERROR.code, null,
                        "Error encountered converting NDL ads to RSpec via converter service: "
                                + (String) res.get("msg"));
            }

            // compress if needed
            if (XmlrpcOrcaState.getInstance().getCompression()) {
                return getStandardApiReturn(ApiReturnCodes.SUCCESS.code,
                        CompressEncode.compressEncode((String) res.get("ret")), null);
            } else
            ...
            if (sliceEndDate != null) {
                ...
                if (cal.before(termEndDateCal)) { // sliceEndDate is before new term's end date
                    if (!extend_alap) { // return error if alap option is false
                        return getStandardApiReturn(ApiReturnCodes.BADARGS.code, null,
                                "Requested new end date is after slice expiration or exceeds system default");
    ...
    public Map<String, Object> Shutdown(String slice_urn, Object[] credentials, Map<String, Object> options) {
        ...
        try {
            ...
            if (allRes == null) {
                throw new Exception("Could not obtain the reservations in  slice with urn " + slice_urn + " sliceId  "
                        + sliceId + " : " + sm.getLastError());
            }
    ...
    /**
     * public static final int ReservationStateUnknown = 0; public static final int ReservationStateNascent = 1; public
     * static final int ReservationStateTicketed = 2; public static final int ReservationStateActive = 3; public static
    ...
    public Map<String, Object> SliverStatus(String slice_urn, Object[] credentials, Map<String, Object> options) {
        ...
        try {
            ...
            try {
                ...
                logger.error("sliverStatus(): Exception encountered for " + slice_urn + ": " + e);
                return getStandardApiReturn(ApiReturnCodes.ERROR.code, null,
                        "ERROR: unable to get slice status for " + slice_urn);
            }
            ...
            if (allRes == null) {
                ...
                if (allRes.size() <= 0) {
                    return getStandardApiReturn(ApiReturnCodes.ERROR.code, null,
                            "ERROR: There are no reservations in the slice with sliceId = " + slice_urn);
                }
                ...
                for (ReservationMng r : allRes) {
                    ...
                    // String resUrn = NdlToRSpecHelper.cidUrnFromUrl(dom, UrnType.Sliver,
                    // NdlToRSpecHelper.getTrueName(resUrl));
                    String resUrn = NdlToRSpecHelper.sliverUrnFromRack(NdlToRSpecHelper.getTrueName(resUrl),
                    ...
                    if ((r.getState() == OrcaConstants.ReservationStateActive)
                            && (r.getPendingState() == OrcaConstants.ReservationPendingStateExtendingTicket)) {
                        res_end = slice_end;
                    }
