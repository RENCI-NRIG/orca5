...
import net.exogeni.orca.controllers.xmlrpc.geni.GeniAmV1Handler;
import net.exogeni.orca.controllers.xmlrpc.x509util.CredentialValidator;
...
public class XmlRpcController extends OrcaController {
    ...
    protected void init() throws Exception {
        ...
        if (requestRulesFileName != null) {
            Log.info("Copying " + XmlRpcController.PropertyUserRequestRulesFile + " set to " + requestRulesFileName
                    + " to " + NdlRequestParser.USER_REQUEST_RULES_FILE_PROPERTY + " system property");
            System.setProperty(NdlRequestParser.USER_REQUEST_RULES_FILE_PROPERTY, requestRulesFileName);
        }
        ...
        if (modifyRulesFileName != null) {
            Log.info("Copying " + XmlRpcController.PropertyUserModifyRulesFile + " set to " + modifyRulesFileName
                    + " to " + NdlModifyParser.USER_MODIFY_RULES_FILE_PROPERTY + " system property");
            System.setProperty(NdlModifyParser.USER_MODIFY_RULES_FILE_PROPERTY, modifyRulesFileName);
        }
    }
    ...
    public void start() throws Exception {
        ...
        if (getProperty(CtrlPortProperty) != null) {
            try {
                ...
                Log.error("Unable to parse controller port property " + CtrlPortProperty + " = "
                        + getProperty(CtrlPortProperty));
            }
        }
        ...
        if (getProperty(CtrlThreadsProperty) != null) {
            try {
                ...
                Log.error("Unable to parse controller port property " + CtrlThreadsProperty + " = "
                        + getProperty(CtrlThreadsProperty));
            }
        }
