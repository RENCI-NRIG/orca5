package net.exogeni.orca.controllers.xmlrpc.statuswatch;
...
public class ReservationStatusUpdateThread implements Runnable {
    ...
    /**
     * Helper interface to abstract checking for various state or status changes for a given list of reservations
     * 
     * @author ibaldin
     *
     */
        ...
        protected StatusChecker.Status check_(IOrcaServiceManager sm, ReservationIDWithModifyIndex l) {
            ...
            try {
                ...
                if ((rm.getState() != OrcaConstants.ReservationStateActive)
                    ...
                    logger.debug("ModifyStatusChecker.check_() - returning NOTREADY for reservation " + rid.toString()
                            + " in state (" + rm.getState() + ", " + rm.getPendingState() + ")");
                    return StatusChecker.Status.NOTREADY;
                }
                ...
                for (PropertyMng prop : uProps.getProperty()) {
                    if ((!modifyFailed) && (codePropname.equals(prop.getName()))) {
                        ...
                        if (modifyErrorCode == 0) {
                            logger.info("Reservation " + rid.getReservationID()
                                    + " was modified successfully for modify index " + rid.getModifyIndex());
                ...
                if (modifyFailed) {
                    logger.info("Reservation " + rid.getReservationID() + " failed modify for modify index "
                            + rid.getModifyIndex() + " with code " + modifyErrorCode + " and message "
                            + modifyErrorMsg);
                    return StatusChecker.Status.NOTOK;
                }
            } catch (Exception e) {
     ...
     * 
     * @param watch
     *            - reservations to watch (with associated modify operation indices)
     * @param act
     *            - reservations to act on (can be null)
     * @param cb
     *            - callback
     */
    public void addModifyStatusWatch(List<ReservationIDWithModifyIndex> watch, List<ReservationID> act,
            IStatusUpdateCallback<ReservationIDWithModifyIndex> cb) {
     ...
     *            - callback object
     */
    public void addActiveStatusWatch(List<ReservationID> watch, List<ReservationID> act,
            IStatusUpdateCallback<ReservationID> cb) {
        if ((watch == null) || (watch.size() == 0) || (cb == null)) {
    ...
    /**
     * Non-null means the watch entry has triggered and no longer needed (ie all entries on watch list either OK or
     * NOTOK). null otherwise (some are NOTREADY)
     * 
     * @param we
     * @param st
     * @return TriggeredWatchEntry or null if it didn't trigger
     */
    private <ID> TriggeredWatchEntry<ID> checkWatchEntry(IOrcaServiceManager sm, WatchEntry<ID> we,
            StatusChecker<ID> sc) {
    ...
    private <ID> void processWatchList(IOrcaServiceManager sm, List<WatchEntry<ID>> watchList, String watchType,
            StatusChecker<ID> sc) {
        ...
        for (TriggeredWatchEntry<ID> twe : toProcess) {
            try {
                ...
                logger.error("run(): Triggered " + watchType + " watch entry for reservations " + twe.watch
                        + " returned with callback exception " + e);
            }
        }
    }
    ...
    public void run() {
        ...
        try {
            ...
            try {
                ...
                this.<ReservationIDWithModifyIndex>processWatchList(sm, modifyWatch, "modify",
                        new ModifyStatusChecker());
            } catch (Exception e) {
