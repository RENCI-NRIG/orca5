...
import net.exogeni.orca.controllers.xmlrpc.pubsub.PublishManager;
...
public final class XmlrpcOrcaState implements Serializable {
    ...
    private static final Pattern macNamePattern = Pattern
            .compile(UnitProperties.UnitEthPrefix + "[\\d]+" + UnitProperties.UnitEthMacSuffix);
    ...
    public static void startThreads() {
        ...
        lstFuture = scheduler.scheduleAtFixedRate(lst, LabelSyncThread.getPeriod(), LabelSyncThread.getPeriod(),
                TimeUnit.SECONDS);
    ...
    /**
     * probably only useful in Unit tests, to make sure each test is independent.
     */
    ...
    // This happens when there is mismatch in passed in tags and the available tags in the site, by a speciall error
    // message
        ...
        try {
            ...
            synchronized (this) {
                for (Entry<String, XmlrpcControllerSlice> urn_wf : slices.entrySet()) {
                    ...
                    if (sl != null) {
                        try {
                            logger.debug("XmlrpcOrcaState.closeDeadSlices(): slice  " + sl.getSliceUrn() + "/"
                                    + sl.getSliceID() + "; isDeadOrClosing=" + sl.isDeadOrClosing() + "; last delete="
                                    + sl.getDeleteAttempt());
                            if (sl.isDeadOrClosing()) {
                                ...
                                if (sl.allFailed()) {
                                    if (sl.getDeleteAttempt() != null) {
                                        ...
                                        if (cc1.after(cc)) {
                                            logger.info("XmlrpcOrcaState.closeDeadSlices(): deleting all failed slice "
                                                    + sl.getSliceID() + "/" + sl.getSliceUrn() + " after 24 hours");
                                            remove_slices.add(sl);
                                        }
                                    } else
                                        sl.markDeleteAttempt();
                                } else {
                                    logger.info("XmlrpcOrcaState.closeDeadSlices(): deleting slice " + sl.getSliceID()
                                            + "/" + sl.getSliceUrn());
                                    remove_slices.add(sl);
                                }
                            }
                        } catch (SliceStateMachine.SliceTransitionException e) {
                            logger.error("closeDeadSlices(): Slice " + sl.getSliceUrn() + "/" + sl.getSliceID()
                                    + " owned by " + sl.getUserDN()
                                    + " has encountered a state transition problem at garbage collection.");
                        }
                    }
                }
            }
    ...
    public synchronized void recover() throws Exception {
        ...
        try {
            ...
            for (ReservationMng a : actives) {
                logger.debug("Inspecting reservation " + a.getReservationID() + " of type " + a.getResourceType()
                        + " from slice " + a.getSliceID());
                ...
                for (PropertyMng p : confProps.getProperty()) {
                    ...
                    if ((label != null) && (domain != null)) {
                        try {
                            ...
                            logger.debug("Setting XmlrpcOrcaState.controllerAssignedLabel " + label + " for domain "
                                    + domain);
                ...
                for (PropertyMng p : confProps.getProperty()) {
                    ...
                    if ((label != null) && (domain != null)) {
                        try {
                            int iLabel = Integer.parseInt(label);
                            logger.debug("Setting XmlrpcOrcaState.controllerAssignedLabel " + label + " for domain "
                                    + domain + ";slice=" + (s != null ? s.sliceUrn : "slice not recovered"));
                            RequestWorkflow.setBitsetLabel(controllerAssignedLabel, domain, iLabel);
                            if (s != null)
