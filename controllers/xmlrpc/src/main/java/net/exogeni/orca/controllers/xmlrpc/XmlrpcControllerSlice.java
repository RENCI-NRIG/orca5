...
import net.exogeni.orca.controllers.xmlrpc.pubsub.PublishQueue;
import net.exogeni.orca.controllers.xmlrpc.pubsub.SliceState;
...
/**
 * This object stores everything we know about a slice. It also maintains various mappings e.g. slice urn to id and back
 * 
 * @author ibaldin
 *
 */
public class XmlrpcControllerSlice implements RequestWorkflow.WorkflowRecoverySetter {
    ...
    /**
     * Create a new controller slice for a given SM, based on a given SliceMng object, with name, user DN and users list
     * (SSH credentials)
     * 
     * @param sm
     *            - SM
     * @param s
     *            - SliceMng structure from SM
     * @param name
     *            - slice name
     * @param dn
     *            - creator DN/URN
     * @param u
     *            - SSH credentials list for slice users
     * @param recover
     *            - is this slice being recovered or created new
     */
    public XmlrpcControllerSlice(IOrcaServiceManager sm, SliceMng s, String name, String dn, List<Map<String, ?>> u,
            Boolean recover) {
        userDN = dn;
        users = u;
        slice = s;
        ...
        try {
            ...
            if (recover) {
                h = new InterCloudHandler(Globals.TdbPersistentDirectory + Globals.PathSep + "controller"
                        + Globals.PathSep + "idm-" + s.getSliceID(), recover);
            } else {
                h = new InterCloudHandler(null, Globals.TdbPersistentDirectory + Globals.PathSep + "controller"
                        + Globals.PathSep + "idm-" + s.getSliceID());
            }
    ...
    /**
     * Close all reservations in the sice. It is expected that the slice lock is held when calling this function.
     */
    public void close() {
        // global label assignment should be cleared preemptively
        // to maintain consistent view
        if ((!globalAssignmentCleared) && ((stateMachine.getState() == SliceStateMachine.SliceState.DEAD)
    ...
    public void deleteFromPublishQ(Logger logger) {
        ...
        if ((pubManifestEnabled == null) || !pubManifestEnabled.equalsIgnoreCase("true")) {
            logger.info(
                    "ORCA.publish.manifest property needs to be set to true for publishing manifests; Can't delete from publish queue");
            return;
        }
    ...
    public void publishManifest(Logger logger) {
        ...
        if ((pubManifestEnabled == null) || !pubManifestEnabled.equalsIgnoreCase("true")) {
            logger.info(
                    "ORCA.publish.manifest property needs to be set to true for publishing manifests; Can't publish manifest");
            return;
        }
        ...
        PublishQueue.getInstance()
                .addToNewSlicesQ(new SliceState(this, SliceState.PubSubState.SUBMITTED, start, end, 0));
    }
    ...
    public void updatePublishedManifest(Logger logger) {
        ...
        if ((pubManifestEnabled == null) || !pubManifestEnabled.equalsIgnoreCase("true")) {
            logger.info(
                    "ORCA.publish.manifest property needs to be set to true for publishing manifests; Can't re-publish manifest");
            return;
        }
    ...
    public boolean isStable() throws SliceStateMachine.SliceTransitionException {
        ...
        if ((ss == SliceStateMachine.SliceState.STABLE_ERROR) || (ss == SliceStateMachine.SliceState.STABLE_OK))
            return true;
        return false;
    }
        ...
        for (PropertyMng p : confProps.getProperty()) {
            ...
            if ((label != null) && (domain != null)) {
                try {
                    ...
                    logger.error("Encountered non-numeric label " + label + " for domain " + domain
                            + " unable to restore, skipping");
                    label = null;
                    domain = null;
                }
                break;
     ...
     *
     * @return The Map will have keys of ReservationID strings, while the values are the Map returned by
     *         ReservationMng.toMap().
     */
    ...
    protected StringBuilder getComputedReservationSummary() {
        ...
        if ((getComputedReservations() != null) && (getComputedReservations().size() > 0)) {
            ...
            for (TicketReservationMng currRes : getComputedReservations()) {
                // LeaseReservationMng currRes = (LeaseReservationMng) sm.getReservation(new
                // ReservationID(ticketReservationMng.getReservationID()));
