...
/**
 * This runs as a standalone thread started by OrcaXmlrpcHandler and deals with slices that have to wait for other
 * slices to complete.
 * 
 * @author ibaldin
 *
...
public class SliceDeferThread implements Runnable {
    ...
    /**
     * Resource types to wait for - due to possible race conditions because of long provisioning times.
     */
    ...
    SliceDeferThread() {
        ...
        try {
            if (OrcaController.getProperty(XmlRpcController.PropertyControllerMaxCreateTimeMs) != null)
                maxCreateWaitTime = Integer
                        .parseInt(OrcaController.getProperty(XmlRpcController.PropertyControllerMaxCreateTimeMs));
            else
        ...
        try {
            if (OrcaController.getProperty(XmlRpcController.PropertyDelayResourceTypes) != null)
                delayResourceTypes = OrcaController.getProperty(XmlRpcController.PropertyDelayResourceTypes)
                        .split("[\\s]");
            else
    ...
    /**
     * Wait in slice queue for up to THREAD_SLEEP_TIME and return either queue head or null;
     * 
     * @param s
     * @return
     */
    ...
    /**
     * This is where the slice gets into the deferred class. Either it is not delayed because nothing else is going on
     * or it doesn't touch on delay domains, so it is submitted immediately, or it is put on a queue for later FIFO
     * processing. It is called with slice lock held.
     *
     * @param s
     *            - slice
     */
    ...
    public void run() {
        while (true) {
            ...
            logger.info("run(): processing previously deferred slice " + lastSlice.getSliceUrn() + "/"
                    + lastSlice.getSliceID());
            ...
            try {
                ...
                if (delayNotDone(lastSlice)) {
                    if ((System.currentTimeMillis() - lastSliceTime) > maxCreateWaitTime)
                        logger.info("run(): maximum wait time exceeded for slice " + lastSlice.getSliceUrn() + "/"
                                + lastSlice.getSliceID() + ", proceeding anyway.");
                    else
                        continue;
                }
            } catch (InterruptedException e) {
                ...
                logger.error("run(): exception while checking slice " + lastSlice.getSliceUrn() + "/"
                        + lastSlice.getSliceID() + ": " + e);
            } finally {
                lastSlice.unlock();
            }
            ...
            try {
                ...
                logger.error("run(): exception while demanding slice " + lastSlice.getSliceUrn() + "/"
                        + lastSlice.getSliceID() + ": " + ee);
            } finally {
                slice.unlock();
            }
        ...
        try {
            ...
            while (it.hasNext()) {
                try {
                    ...
                    logger.debug(
                            "demandSlice(): Issuing demand for reservation: " + currRes.getReservationID().toString());
    ...
    private boolean checkComputedReservations(XmlrpcControllerSlice slice) {
        ...
        if ((slice == null) || (slice.getComputedReservations() == null)) {
            logger.info(
                    "checkComputedReservations(): checkComputedReservaions empty slice or no computed reservations");
            return false;
        }
        ...
        for (TicketReservationMng cr : slice.getComputedReservations()) {
            for (String drt : delayResourceTypes) {
                if (drt.equals(cr.getResourceType())) {
                    logger.info("checkComputedReservations(): checkComputedReservaions " + slice.getSliceUrn() + "/"
                            + slice.getSliceID() + " has delayed domain");
                    return true;
                }
            }
        }
        logger.info("checkComputedReservations(): checkComputedReservaions " + slice.getSliceUrn() + "/"
                + slice.getSliceID() + " has no delayed domains");
        return false;
    }
    ...
    private boolean delayNotDone(XmlrpcControllerSlice slice) {
        ...
        try {
            ...
            logger.error("delayNotDone(): Exception in delayNotDone for slice " + slice.getSliceUrn() + "/"
                    + slice.getSliceID() + ": " + e);
            return false;
        } finally {
            if (sm != null)
        ...
        if (allRes == null) {
            logger.info("delayNotDone(): Slice " + slice.getSliceUrn() + "/" + slice.getSliceID()
                    + " has null reservations in delayNotDone");
            // slice has not been submitted
            return checkComputedReservations(slice);
        } else {
            if (allRes.size() <= 0) {
                logger.info("delayNotDone(): Slice " + slice.getSliceUrn() + "/" + slice.getSliceID()
                        + " has empty reservations in delayNotDone");
                // slice has not been submitted
                return checkComputedReservations(slice);
            }
                ...
                for (String drt : delayResourceTypes) {
                    if (drt.equals(rType)) {
                        ...
                        if ((r.getState() != OrcaConstants.ReservationStateActive)
                            ...
                            logger.info("delayNotDone(): Slice " + slice.getSliceUrn() + "/" + slice.getSliceID()
                                    + " has domain " + drt + " with reservation " + r.getReservationID()
            ...
            logger.info("delayNotDone(): Slice " + slice.getSliceUrn() + "/" + slice.getSliceID()
                    + " has no non-final reservations (" + allRes.size() + ")");
            return false;
        }
    }
