...
package net.exogeni.orca.controllers.xmlrpc.pubsub;
...
import net.exogeni.orca.controllers.xmlrpc.SliceStateMachine.SliceTransitionException;
...
public class PublishManager {
    ...
    public PublishManager() {
        ...
        synchronized (futures) {
            futures.add(scheduler.scheduleAtFixedRate(new PublisherTask(), PUBLISHER_PERIOD, PUBLISHER_PERIOD,
                    TimeUnit.SECONDS));
        }
    }
    ...
    private void initialize() {
        ...
        // NOTE: This loop blocks the controller thread because the controller calls new PublishManager(), which in turn
        // calls initialize()
        // This is a way to make sure that the controller deosn't return until SM is up, but not necessarily the best
        // way.
    ...
    class PublisherTask implements Runnable {
                ...
                synchronized (pubQ) {
                    ...
                    while (it.hasNext()) { // go through all the slicestates
                        ...
                        if (currSliceState.getState() == SliceState.PubSubState.SUBMITTED) { // State 0, just submitted
                            logger.info("Slice: " + currSliceUrn + "/" + currSliceID
                                    + " in state SUBMITTED, going to state INPROGRESS, publishing manifest");
                            doPublish(mPublisher, currSliceUrn);
                            currSliceState.setState(SliceState.PubSubState.INPROGRESS);
                            // pub sliceList
                            publishSliceList = true;
                        } else if (currSliceState.getState() == SliceState.PubSubState.INPROGRESS) { // State 1, in
                                                                                                     // progress
                            ...
                            if (checkDone == true) {
                                logger.info("Publishing Final manifest for slice: " + currSliceUrn
                                        + ", slice going to DONEACTIVE");
                                doPublish(mPublisher, currSliceUrn);
                                currSliceState.setState(SliceState.PubSubState.DONEACTIVE);
                                currSliceState.setWaitTime(2 * 60 * 24); // wait for one day, since publish thread
                                                                         // executed every 30 seconds
                            } else if (checkClosed == true) {
                                logger.info("Slice: " + currSliceUrn
                                        + " is cloded, final manifest is not published, going to DONECLOSED");
                                ...
                                currSliceState.setWaitTime(2); // wait for one minute, since the Publish thread executes
                                                               // every 30 seconds
                                logger.info("Slice: " + currSliceUrn + " going to state WAITINPROGRESS");
                            }
                        } else if (currSliceState.getState() == SliceState.PubSubState.WAITINPROGRESS) { // state 2,
                                                                                                         // wait while
                                                                                                         // inprogress
                            ...
                            // publish sliceList
                            publishSliceList = true;
                        } else if (currSliceState.getState() == SliceState.PubSubState.EXPUNGE) { // end of lifecycle,
                                                                                                  // delete xmpp pubsub
                                                                                                  // node
                            logger.info("Slice: " + currSliceUrn + " in state EXPUNGE");
                            doExpunge(mPublisher, currSliceUrn, currSliceState.getOrcaSliceID(), "manifest"); // delete
                                                                                                              // the
                                                                                                              // manifest
                    ...
                    if (!toRemoveSliceStateQ.isEmpty()) {
                        for (SliceState s : toRemoveSliceStateQ) {
                            logger.info(
                                    "Removing " + s.getSlice_urn() + " from the PubQ after end of manifest lifecycle");
                            currSliceStateQ.remove(s);
                        }
                    }
                } // end synchronized

            } catch (Exception e) { // this is to catch all exceptions in the run() method for the timer thread;
                                    // prevents timer thread from getting killed by exceptions
                logger.error(
                        "Exception occured during Timer thread execution for handling PubSub: Stack trace follows... ",
                        e);
                e.printStackTrace();
            } finally {
                try {
        ...
        private void doPublish(ManifestPublisher mPublisher, String slice_urn) {
            ...
            logger.info("Publishing manifest for : " + actor_name + "---" + actor_guid + " | " + slice_urn + "---"
                    + orcaSliceID);
        ...
        private String buildSliceListString(Set<SliceState> currSliceStateList) {
            ...
            if (sliceListString != null) {
                logger.debug("buildSliceListString(): sliceListString[first 100 chars] = "
                        + sliceListString.substring(1, 100));
            } else {
        ...
        private void doExpunge(ManifestPublisher mPublisher, String slice_urn, String orcaSliceID, String suffix) {
            ...
            logger.info("Deleting node : " + actor_name + "---" + actor_guid + " / " + slice_urn + "---"
                    + orcaSliceID.toString());

            try {
                mPublisher.expungeNode(actor_name + "---" + actor_guid, slice_urn + "---" + orcaSliceID.toString(),
                        suffix);
            } catch (Exception e) {
