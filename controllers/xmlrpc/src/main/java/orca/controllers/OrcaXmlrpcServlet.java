...
public class OrcaXmlrpcServlet extends XmlRpcServlet {
    ...
    /**
     * This class synchronizes access to an internal property handler map. Best guess as to which methods needed to be
     * overridden
     * 
     * @author ibaldin
     *
    ...
    public void doPost(HttpServletRequest pRequest, HttpServletResponse pResponse)
        ...
        if (sslSessionId.get() == null) {
            ...
            /*
             * Log.debug("Available attributes: --------"); Enumeration<?> names = pRequest.getAttributeNames();
             * while(names.hasMoreElements()){ Log.debug(names.nextElement()); } Log.debug("--------");
             */

            X509Certificate[] certChain = (X509Certificate[]) pRequest
                    .getAttribute("javax.servlet.request.X509Certificate");
            clientCertificateChain.set(certChain);

            // if (certChain != null)
            // Globals.Log.debug("Certificate chain in POST of the client " + pRequest.getRemoteAddr() + " is of type "
            // + certChain.getClass().getCanonicalName());
            // else
