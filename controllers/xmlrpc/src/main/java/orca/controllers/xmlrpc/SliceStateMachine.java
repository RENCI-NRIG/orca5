...
public class SliceStateMachine {
    public static enum SliceState {
        NULL("NULL"), CONFIGURING("CONFIGURING"), STABLE_ERROR("STABLE WITH ERRORS"), STABLE_OK("STABLE"), CLOSING(
                "CLOSING"), DEAD("DEAD");
    ...
    public static enum SliceCommand {
        CREATE("Create", SliceState.NULL), MODIFY("Modify", SliceState.STABLE_ERROR, SliceState.STABLE_OK,
                SliceState.CONFIGURING), DELETE("Delete", SliceState.STABLE_ERROR, SliceState.STABLE_OK,
                        SliceState.CONFIGURING, SliceState.DEAD, SliceState.NULL), REEVALUATE("Re-evaluate",
                                SliceState.CONFIGURING, SliceState.CLOSING, SliceState.NULL, SliceState.STABLE_ERROR,
                                SliceState.STABLE_OK, SliceState.DEAD);
    ...
    public SliceState transitionSlice(SliceCommand cmd) throws SliceTransitionException {
        ...
        case REEVALUATE:
            ...
            case CONFIGURING:
                ...
                if ((!b.hasStatesOtherThan(OrcaConstants.ReservationStateActive, OrcaConstants.ReservationStateFailed,
                        OrcaConstants.ReservationStateClosed)) && (b.hasState(OrcaConstants.ReservationStateFailed)))
                    state = SliceState.STABLE_ERROR;
                // we could be dead (due to timeout)
                if (!b.hasStatesOtherThan(OrcaConstants.ReservationStateClosed, OrcaConstants.ReservationStateCloseWait,
            ...
            case STABLE_ERROR:
                ...
                if (!b.hasStatesOtherThan(OrcaConstants.ReservationStateClosed, OrcaConstants.ReservationStateCloseWait,
                        OrcaConstants.ReservationPendingStateClosing, OrcaConstants.ReservationStateFailed))
                    state = SliceState.CLOSING;
                break;
            case CLOSING:
                if (!b.hasStatesOtherThan(OrcaConstants.ReservationStateClosed, OrcaConstants.ReservationStateCloseWait,
