...
public class ReservationElementCollection {
    ...
    public ReservationElementCollection(Collection<NetworkElement> elements, HashMap<String, DomainElement> fge,
            Logger log) {
        ...
        if (interfaces != null) {
            for (Interface intf : interfaces) {
                ...
                if (intf_ont.getProperty(NdlCommons.ipMacAddressProperty) == null)
                    intf_ont.addProperty(NdlCommons.ipMacAddressProperty,
                            ReservationConverter.generateNewMAC(XmlrpcOrcaState.getInstance()));
            }
        }
    }
    ...
    public List<String> group_getVMsInGroup(String group) {
        ...
        if (ces == null)
            logger.error(
                    "ReservationElement: ces is null, group=" + group + "NodeGroupMap.szie=" + NodeGroupMap.size());
