	...
	public void success(List<ReservationID> ok, List<ReservationID> actOn)
		...
		try {
			...
			if(e_p_str!=null){
				...
				logger.debug("Number of existing parent reservations:"+p+";num_interface="+num_interface_int+";num_storage="+num_storage_int);
				for(int i=0;i<p;i++){
						...
						if(isNetwork!=null && isNetwork.equals("1")){	//Parent is a networking reservation
							...
							List<UnitMng> un = sm.getUnits(new ReservationID(p_r.getReservationID()));
								if (un != null) {
									...
									//	Thread.sleep(1000L);
								//}
							logger.debug("parent r_id="+r_id+";unit tag:"+unit_tag+";unit_parent_url:"+unit_parent_url+";n=" + n);
							if(unit_tag!=null){
						...
						if(isLun!=null && isLun.equals("1")){	//Parent is a storage reservation
							...
							if(unit_tag!=null){
								...
								if(local.getProperty(storagePrefix + UnitProperties.UnitTargetLunGuid)!=null)
									modifyProperties.setProperty(UnitProperties.UnitTargetLunGuid, local.getProperty(storagePrefix + UnitProperties.UnitTargetLunGuid));
								...
								if(local.getProperty(storagePrefix + UnitProperties.UnitTargetChapSecretSuffix)!=null)
									modifyProperties.setProperty(UnitProperties.UnitTargetChapSecret, local.getProperty(storagePrefix + UnitProperties.UnitTargetChapSecretSuffix));
