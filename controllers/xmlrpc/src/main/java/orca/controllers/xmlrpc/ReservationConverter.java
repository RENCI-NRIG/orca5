...
public class ReservationConverter implements LayerConstant {
    ...
    public ReservationConverter(List<Map<String, ?>> u, IOrcaServiceManager asm, XmlrpcControllerSlice sl)
    ...
    public ArrayList<TicketReservationMng> getReservations(IOrcaServiceManager sm,
            Collection<NetworkElement> boundElements, HashMap<String, SiteResourceTypes> typesMap,
            OrcaReservationTerm OTerm, RequestSlice rSlice) throws NdlException {
        setLeaseTerm(OTerm);
    ...
    public HashMap<String, ReservationRequest> formReservations(IOrcaServiceManager sm,
            Collection<NetworkElement> boundElements, HashMap<String, SiteResourceTypes> typesMap, RequestSlice rSlice)
            throws NdlException {
        ...
        for (NetworkElement device : boundElements) {
            ...
            config.setProperty(PropertyConfigDuration,
                    String.valueOf(leaseEnd.getTime() / 1000 - leaseStart.getTime() / 1000));
            if (leaseStart != null) {
            ...
            logger.debug("ReservationConverter::formReservations():" + UNIT_URL_RES + " " + device_name + ";uni.domain=" + device.getURI());
            ...
            if (resrequest.isNetwork) {
                ...
                if ((ndlRequest == null) || (ndlRequest.length() == 0)) {
                ...
                if (rSlice != null) {
                    logger.debug("ReservationConverter::formReservations():OF Slice:" + rSlice.getOfCtrlUrl() + ":" + rSlice.getOfUserEmail() + ":"
                            + rSlice.getOfSlicePass());
                    ...
                    if (bw != null) {
                        if (from == null && to == null) {
                            throw new RuntimeException(
                                    "Bandwidth requested, but no interface specified for domain: " + domain);
                        }
    ...
    public ArrayList<TicketReservationMng> setDependency(Collection<NetworkElement> boundElements,
            HashMap<String, ReservationRequest> map) {
        ...
        for (NetworkElement device : boundElements) {
            ...
            if (preds == null) {
                ...
                r.reservation.setConfigurationProperties(
                        OrcaConverter.merge(config, r.reservation.getConfigurationProperties()));
                continue;
            }
            ...
            for (Entry<DomainElement, OntResource> parent : de.getPrecededBySet()) {
                ...
                // 1. if both reservations are for network resources we must map the vlan tag from pr into a prefixed
                // vlan tag
                // so that it does not collide with the vlan tags of other dependent reservations
                // 2. for two reservations from the same domain, parent could be the one w/ static tag, in which case,
                // the interface value is null
                    ...
                    if (prNetwork) {
                        ...
                        if (pr_interface != null) {
                            ...
                            if (pr != null && pr.reservation != null) {
                                pr.reservation.setConfigurationProperties(
                                        OrcaConverter.merge(pr_config, pr.reservation.getConfigurationProperties()));
                                pr.reservation.setLocalProperties(
                                        OrcaConverter.merge(pr_config, pr.reservation.getLocalProperties()));
                            }
                        }
                        // }
                    }
                        ...
                        if (prNetwork) {
                            ...
                            if (site_host_interface == null) {
                                ...
                                if (parent.getValue().getProperty(NdlCommons.hostInterfaceName) == null)
                                    manifestModel.add(parent.getValue(), NdlCommons.hostInterfaceName,
                                            site_host_interface);
                            }
                            ...
                            if (index > 0) {
                                ...
                                local.setProperty(UnitProperties.UnitEthPrefix + host_interface
                                        + UnitProperties.UnitHostEthSuffix, site_host_interface);
                                if (de.getPrecededBySet().size() >= 1) {
                                    parent_tag_name = parent_tag_name.concat(host_interface)
                                            .concat(UnitProperties.UnitEthVlanSuffix);
                                    parent_quantum_uuid = parent_quantum_uuid.concat(host_interface)
                                            .concat(UnitProperties.UnitEthNetworkUUIDSuffix);
                                }
                            } else {
                                ...
                                if (de.getPrecededBySet().size() >= 1) {
                                    ...
                                    if (pr != null) {
                                        ...
                                        pr.reservation.setConfigurationProperties(OrcaConverter.merge(pr_config,
                                                pr.reservation.getConfigurationProperties()));
                                        pr.reservation.setLocalProperties(
                                                OrcaConverter.merge(pr_config, pr.reservation.getLocalProperties()));
                                    }
                                }
                            ...
                            Properties formedProperties = formInterfaceProperties(parent, site_host_interface,
                                    r.networkDependencies, 0);
                            ...
                            if (parent.getKey().getStaticLabel() != 0) {
                                config.setProperty(parent_tag_name,
                                        String.valueOf((int) parent.getKey().getStaticLabel()));
                                local.setProperty(parent_tag_name,
                                        String.valueOf((int) parent.getKey().getStaticLabel()));
                            } else
    ...
    public Properties formInterfaceProperties(OntModel manifestModel, DomainElement dd,
            Entry<DomainElement, OntResource> parent, String site_host_interface, int num_parent, int num) {
    ...
    public Properties formInterfaceProperties(Entry<DomainElement, OntResource> parent, String site_host_interface,
        ...
        property.setProperty(UnitProperties.UnitEthPrefix + host_interface + UnitProperties.UnitHostEthSuffix,
                site_host_interface);
        ...
        if (mac_addr != null) {
            ...
            logger.debug("ReservationConverter::formInterfaceProperties():ReservationConverter:mac property:"
        ...
        if (site_host_interface == null) {
            ...
            if (p_de.getDownNeighbour(idm) != null) {
                if (p_de.getDownNeighbour(idm).getProperty(NdlCommons.hostInterfaceName) != null) {
                    site_host_interface = p_de.getDownNeighbour(idm).getProperty(NdlCommons.hostInterfaceName)
                            .getString();
                } else {
                    if (p_de.getUpNeighbour(idm) != null) {
                        if (p_de.getUpNeighbour(idm).getProperty(NdlCommons.hostInterfaceName) != null) {
                            site_host_interface = p_de.getUpNeighbour(idm).getProperty(NdlCommons.hostInterfaceName)
                                    .getString();
                        }
                    }
                }
    ...
    public void processIPCollectionProperty(HashMap<DomainElement, LinkedList<ReservationRequest>> collection) {
        ...
        for (Entry<DomainElement, LinkedList<ReservationRequest>> entry : collection.entrySet()) {
            ...
            for (Entry<ReservationRequest, String> r_entry : r_uuid_map.entrySet()) {
                ...
                String parent_tag_name = UnitProperties.UnitStoragePrefix + host_interface
            ...
            lun_r.reservation.setConfigurationProperties(
                    OrcaConverter.merge(config, lun_r.reservation.getConfigurationProperties()));
        }
    ...
    public void setVMISCSIProperty(Properties p, String r_uuid, String lun_uuid, IPAddressRange ip_range,
            String host_interface) {
        ...
        if (ip_range != null) {
            if (ip_range.getBase_IP() != null) {
                ...
                logger.warn("ReservationConverter::setVMISCSIProperty(): VMISCSI: base_ip_addr is null");
        } else
            logger.warn("ReservationConverter::setVMISCSIProperty(): VMISCSI: ip_range is null");
    ...
    public void setVMISCSIParam(Properties p, ComputeElement s_ce, int s_num) {
        ...
        if (s_ce.getMntPoint() != null)
            ...
            p.setProperty(storagePrefix + UnitProperties.UnitFSMountPointSuffix,
                    UnitProperties.DefaultMountPoint + String.valueOf(s_num));
        if (s_ce.isDoFormat())
        ...
        logger.debug(
                "ReservationConverter::setVMISCSIProperty():" + storagePrefix + UnitProperties.UnitFSMountPointSuffix + "=" + s_ce.getMntPoint()
                        + ":" + storagePrefix + UnitProperties.UnitTargetChapUserSuffix + "=" + s_ce.getCHAP_User());
    }

    public void processIPCollection(HashMap<DomainElement, LinkedList<ReservationRequest>> collection,
            DomainElement p_de, ReservationRequest r) {
    ...
    public void updateTerm(OntModel manifestModel) {
        ...
        if (rs.hasNext()) {
            ...
            if ((term_rs != null) && (sd != null)) {
                ...
                cal.setTime(sd);
                st_rs.addProperty(NdlCommons.inXSDDateTime, DatatypeConverter.printDateTime(cal),
    ...
    public String getManifest(OntModel manifestModel, LinkedList<OntResource> domainInConnectionList,
            Collection<NetworkElement> deviceList, List<ReservationMng> allRes) {
    ...
    public OntModel getManifestModel(OntModel manifestModel, LinkedList<OntResource> domainInConnectionList,
            Collection<NetworkElement> deviceList, List<ReservationMng> allRes) {
        ...
        for (i = 0; i < domainInConnectionList.size(); i++) {
            ...
            boolean inConnection = false; // if true in interdomain connection request, or the same reservation in
                                          // cloudhandler
            if (domain_ont.getProperty(NdlCommons.inConnection) != null) { // defined in
                                                                           // InterDomainHandler.createManifest()
                            ...
                            if (un != null) {
                                ...
                                if (un.size() > 0) {
                                    for (UnitMng u : un) {
                                        ...
                                        if ((rType.equalsIgnoreCase("vlan"))
                                            ...
                                            if (p.getProperty(UnitProperties.UnitVlanQoSRate) != null) {
                                                Statement st_bw = domain_ont
                                                        .getProperty(NdlCommons.layerBandwidthProperty);
                                            ...
                                            if (p.getProperty(UnitProperties.UnitQuantumNetUUID) != null) {
                                                domain_ont.addProperty(NdlCommons.quantumNetUUIDProperty,
                                                        p.getProperty(UnitProperties.UnitQuantumNetUUID));
                                            }
                                            updateState(manifestModel, domain_ont, NdlCommons.requestMessage, notice);
                                            updateState(manifestModel, domain_ont,
                                                    NdlCommons.requestHasReservationState, reservationState);
                                        ...
                                        if (rType.equalsIgnoreCase("vm") || rType.endsWith("baremetalce")
                                            ...
                                            if (vm_ont == null) {
                                                ...
                                                if (rType.equalsIgnoreCase("lun"))
                                                    vm_ont = manifestModel.createIndividual(vm_url,
                                                            NdlCommons.networkStorageClass);
                                                else
                                                    vm_ont = manifestModel.createIndividual(vm_url,
                                            ...
                                            if (domain_ont.hasProperty(NdlCommons.inDomainProperty)) {
                                                inDomain_rs = domain_ont.getProperty(NdlCommons.inDomainProperty)
                                                        .getResource();
                                            }
                                            ...
                                            Individual service_ont = manifestModel.createIndividual(vm_url + "/Service",
                                                    NdlCommons.networkServiceClass);
                                            service_ont.addProperty(NdlCommons.domainHasAccessMethod,
                                                    NdlCommons.domainSSHServiceClass);
                                            vm_ont.addProperty(NdlCommons.domainHasServiceProperty, service_ont);
                                            // Individual vm_IP = manifestModel.createIndividual(vm_url + "/IP",
                                            // NdlCommons.IPAddressOntClass);
                                            // logger.info("ReservationConverter::getManifestModel():Service:"+service_ont.getURI()+";"+vm_IP.getURI());
                                            ...
                                            String unitManagementPort = p
                                                    .getProperty(UnitProperties.UnitManagementPort);
                                            if (unitManagementIP != null) {
                                                // vm_IP.addProperty(NdlCommons.layerLabelIdProperty, unitManagementIP);
                                                service_ont.addProperty(NdlCommons.topologyManagementIP,
                                                        unitManagementIP);
                                    ...
                                    if (rType.equalsIgnoreCase("vlan")) {
                                        if ((domain_ont.getURI().equals(unit_url))
                                                || (domain_ont_url.equals(unit_url))) {
                                            updateState(manifestModel, domain_ont, NdlCommons.requestMessage, notice);
                                            updateState(manifestModel, domain_ont,
                                                    NdlCommons.requestHasReservationState, reservationState);
                                    ...
                                    if (rType.equalsIgnoreCase("vm") || rType.endsWith("baremetalce")
                                        ...
                                        if (inConnection == true) {
                                            ...
                                            if (vm_ont == null)
                                                vm_ont = manifestModel.createIndividual(unit_url,
                                                        NdlCommons.computeElementClass);
                                            Resource inDomain_rs = null;
                                            if (domain_ont.hasProperty(NdlCommons.inDomainProperty)) {
                                                inDomain_rs = domain_ont.getProperty(NdlCommons.inDomainProperty)
                                            ...
                                            Resource rc = null;
                                            List<Statement> st_list = domain_ont
                                                    .listProperties(NdlCommons.inRequestNetworkConnection).toList();
                    ...
                    if (d.getName().equalsIgnoreCase(entry.getKey())) {
                        ...
                        if (ce.getType() != null)
                            vm_reservation_ont.addProperty(NdlCommons.domainHasResourceTypeProperty,
                                    ce.getResourceType().getTypeResource());
                        ...
                        if (ce.getSpecificCETypeurl() != null) {
                            Resource ceType_rs = vm_reservation_ont.getModel()
                                    .createResource(ce.getSpecificCETypeurl());
                        ...
                        if (ce.getPostBootScript() != null) {
                            if (vm_reservation_ont.hasProperty(NdlCommons.requestPostBootScriptProperty)) {
                                vm_reservation_ont.setPropertyValue(NdlCommons.requestPostBootScriptProperty,
                                        manifestModel.createLiteral(ce.getPostBootScript()));
                            } else {
                                vm_reservation_ont.addProperty(NdlCommons.requestPostBootScriptProperty,
                        ...
                        if (d.getPrecededBySet() != null) {
                            for (Entry<DomainElement, OntResource> parent : d.getPrecededBySet()) {
                                ...
                                if (parentDevice.isAllocatable() == false) {
                                    // Resource
                                    // parentDevice_rs=manifestModel.getResource(parentDevice.getResource().getURI());
                                    // logger.error("ReservationConverter::getManifestModel():Remove interface:"+parentDevice.getName()
                                    ...
                                    // parentDevice.getResource().addProperty(NdlCommons.topologyHasInterfaceProperty,
                                    // parent.getValue());
                                    // entry.getValue().addProperty(NdlCommons.topologyHasInterfaceProperty,
                                    // parent.getValue());
                                    // adding the dependency information
                                    addDependencyProperty(parentDevice, parent.getValue(), entry.getValue(),
                                            manifestModel);
                                    continue;
                                }
                                ...
                                for (Entry<String, OntResource> vlanentry : vlanReservationList.entrySet()) {
                                    ...
                                    if (vlanDeviceName.equals(parentDevice.getName())) {
                                        ...
                                        if (vlan_ont.hasProperty(NdlCommons.RDFS_Label)) {
                                            parentDevice.setStaticLabel(
                                                    vlan_ont.getProperty(NdlCommons.RDFS_Label).getFloat());
                                        }
                                        vlanentry.getValue().addProperty(NdlCommons.topologyHasInterfaceProperty,
                                                parent.getValue());
                                        entry.getValue().addProperty(NdlCommons.topologyHasInterfaceProperty,
                                                parent.getValue());
                                        // adding the dependency information
        ...
        logger.debug("ReservationConverter::getManifestModel(): end associating manifest with ORCA reservation states...");
        // TDB.sync(manifestModel);
        return manifestModel;
    }

    // adding the dependency information
    public void addDependencyProperty(DomainElement parent_device, OntResource intf_ont, OntResource child_ont,
        ...
        parent_device_ont = manifestModel.createIndividual(parent_device.getURI() + "/" + local_name,
                NdlCommons.networkDomainOntClass);
    ...
    public List<ReservationMng> modifyReservations(OntModel manifestModel, List<ReservationMng> allRes,
        ...
        for (NetworkElement ne : modifiedDevices) {
            ...
            int numStorage = 0;
            logger.debug("ReservationConverter::modifyReservations():storage:d_uri=" + d_uri + ";" + dd.getGUID() + ";reservation=" + rmg);
            if (rmg != null) {
            ...
            if (rmg != null) {
                ...
                for (Entry<DomainElement, OntResource> parent : dd.getPrecededBySet()) {
                    ...
                    if (p_rmg != null && !p_r.contains(p_rmg)) {
                        ...
                        if (parent.getValue().getProperty(NdlCommons.layerLabelIdProperty) != null) {
                            String intf_name = parent.getValue().getProperty(NdlCommons.layerLabelIdProperty)
                                    .getString();
                        ...
                        if (!isExistingInterface && !parent_de.getResourceType().getResourceType().endsWith("lun")) {
                            ...
                            if (site_host_interface != null) {
                                ...
                                p_rmg.setConfigurationProperties(
    ...
    public List<ReservationMng> removeReservations(OntModel manifestModel, List<ReservationMng> allRes,
    ...
    public void modifyTerm(OntModel manifestModel, OrcaReservationTerm term) {
        ...
        if (rs.hasNext()) {
            ...
            if (duration_rs != null) {
                ...
                duration_ont.addProperty(NdlCommons.daysProperty, String.valueOf(term.getDurationDays()),
                        XSDDatatype.XSDdecimal);
                clearProperty(manifestModel, duration_ont, NdlCommons.hoursProperty);
                duration_ont.addProperty(NdlCommons.hoursProperty, String.valueOf(term.getDurationHours()),
                        XSDDatatype.XSDdecimal);
                clearProperty(manifestModel, duration_ont, NdlCommons.minutesProperty);
                duration_ont.addProperty(NdlCommons.minutesProperty, String.valueOf(term.getDurationMins()),
                        XSDDatatype.XSDdecimal);
                clearProperty(manifestModel, duration_ont, NdlCommons.secondsProperty);
                duration_ont.addProperty(NdlCommons.secondsProperty, String.valueOf(term.getDurationSecs()),
                        XSDDatatype.XSDdecimal);
            }
            ...
            if (ed != null) {
                ...
                ed_ont.addProperty(NdlCommons.inXSDDateTime, DatatypeConverter.printDateTime(cal),
                        XSDDatatype.XSDdateTime);
            }
    ...
    public static String getDomainLocalName(String domain_url) {
        ...
        if (index >= 0) {
            ...
            if (index2 >= 0) {
                localName = domain_url.substring(index + 1, index2) + "-"
                        + domain_url.substring(index2 + 1, domain_url.length());
            } else
    ...
    public static Properties generateSSHProperties(List<Map<String, ?>> users) throws ReservationConverterException {
        ...
        for (Map<String, ?> e : users) {
            ...
            sshProperties.setProperty(String.format(ConfigurationProperties.ConfigSSHLoginPattern, loginCount),
                    userName);
            sshProperties.setProperty(String.format(ConfigurationProperties.ConfigSSHKeyPattern, loginCount),
                    sb.toString());
    ...
    public static List<Map<String, ?>> restoreUsers(List<PropertyMng> props) {
        ...
        for (Integer index = 0; index <= maxIndex; index++) {
            if ((logins.get(index) != null) && (sudo.get(index) != null) && (keys.get(index) != null)) {
