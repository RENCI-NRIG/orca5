...
/**
 * Custom class loader to be used for loading Shirako classes and extension
 * plugins from Tomcat. This class loader consists of two components: an
 * interface (IShirakoWebappClassLoader) and an implementation
 * (ShirakoWebappClassLoader). The implementation file should be resolvable by
 * Tomcat's Catalina class loader, thus it should reside in a jar inside
 * Tomcat's server/lib folder. Since code within Shirako needs to be able to
 * resolve the interface, the interface should be resolvable by Tomcat's Common
 * loader. Thus, the interface should be in a jar file in common/lib. <br>
 * <br>
 * Note that shirako.war cannot include neither the implementation or the
 * interface of this class loader. If the war contains either of these two
 * files, they will be loaded by the Webapps's loader and it will be impossible
 * to perform type casts of the loader instance as obtained by calling
 * Thread.currentThread().getContextLoader(). <br>
 * <br>
 * To use this class loader for Shirako's web application, add the following to
 * the context describing the application: &lt;Loader
 * loaderClass="shirako.util.tomcat.ShirakoWebappClassLoader" /&gt;
 */
public class ShirakoWebappClassLoader extends WebappClassLoader implements IShirakoWebappClassLoader
{
     ...
     * <ul>
     * <li>If the loader has loaded this class before using a registered
     * loader, it will try to load the class using the corresponding registered
     * loader.
     * <li>If the class has not been loaded before using a registered loader or
     * attempting to load it with the corresponding registered loader failed,
     * the base class loadClass() will be called.
     * <li>If super.loadClass() cannot load the class, registered loaders will
     * be asked, one by one, to load the class. If a loader returns a class
     * definition, this loaders name will be recorded so that no search will be
     * performed next time the class loader is asked to load this class.
    ...
    public Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException
    {
        ...
        /*
         * We have remembered that this class was resolved from one of out
         * loaders. Try to resolve the class using that loader. If we fail, we
         * will try searching for the class again.
         */
        if (loaderName != null) {
            ...
            if (loader != null) {
                try {
                    ...
                    /*
                     * The loader we thought knew how to load this class says it
                     * cannot do it anymore. This should never happen
                     */
                }
            }
        }
        ...
        if (result == null) {
            ...
            if (result == null) {
                /*
                 * The base does not know how to load this class. Ask each
                 * loader in our list if it can load this class
                 */
                ...
                synchronized (this) {
                    ...
                    while (result == null && i.hasNext()) {
                        Map.Entry<String, ShirakoURLClassLoader> entry = (Map.Entry<String, ShirakoURLClassLoader>) i.next();
                        try {
    ...
    public URL getResource(String name)
    {
        ...
        if (result == null) {
            /*
             * The base does not know how to load this resource. Ask each loader
             * in our list if it can load this resource
             */
