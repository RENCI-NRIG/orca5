...
public class VlanPolicyNDLPropertyExtractor extends
	...
	public Properties getPoolProperties() {
		...
		for(ResIterator j=delegateModel.listResourcesWithProperty(ontProcessor.availableLabelSet);j.hasNext();){
        	...
        	float lb_id=lb.getProperty(ontProcessor.layerLabelIdProperty).getFloat();
        	float ub_id=lb.getProperty(ontProcessor.layerLabelIdProperty).getFloat();	
