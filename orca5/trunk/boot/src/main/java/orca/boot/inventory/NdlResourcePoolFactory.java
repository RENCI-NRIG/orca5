...
public class NdlResourcePoolFactory extends ResourcePoolFactory {
    ...
    protected static final HashMap<String,String> hmControlToNDLExtractor =
            new HashMap<String, String>(){
                {
                    // Controls for which we are extracting properties
                	put("orca.plugins.ben.control.NdlVLANControl", "orca.embed.policyhelpers.VlanPolicyNDLPoolPropertyExtractor");
		    		put("orca.plugins.ben.control.NdlInterfaceVLANControl", "orca.embed.policyhelpers.VlanPolicyNDLPoolPropertyExtractor");
    ...
    protected String getAbstractNdl(String resourceType) throws ConfigurationException {
        ...
        if(Globals.getContainer().getConfiguration().getProperty(OrcaContainer.PropertyRegistryClass)!=null) {
             if(Globals.getContainer().getConfiguration().getProperty(OrcaContainer.PropertyRegistryClass).contains("Distributed")) {
	...
	//Globals.Log.debug("desc.getResourceType() = " + desc.getResourceType().toString());
        ...
        if (attr != null) {
        	Globals.Log.warn("NdlResourcePoolFactory: Attribute resource.domain found in config.xml file, usually it is better to let ORCA figure it out (i.e. omit it)");
        } else {
        ...
        while(e.hasMoreElements()){
            ...
            Globals.Log.info("NdlResourcePoolFactory: Current resource control class name = " + rc.getClass().getName());
            ...
            for(Iterator it = lResTypes.iterator(); it.hasNext();){
                ...
                if(currType.equalsIgnoreCase(desc.getResourceType().getType())){
                    ...
                    Globals.Log.debug("NdlResourcePoolFactory: Calling populateAttributesAndProperties for resource Type = " + desc.getResourceType().getType());
    ...
    private void populateAttributesAndProperties(ResourcePoolDescriptor desc, ResourceControl rc) {
        ...
        try {
            ...
            try {
                ...
                GenericNDLPoolPropertyExtractor ndlPropExtractor = (GenericNDLPoolPropertyExtractor) cons.newInstance(getAbstractNdl(desc.getResourceType().getType()));
                Properties poolProp = ndlPropExtractor.getPoolProperties();
                if(poolProp == null){
                    Globals.Log.debug("NdlResourcePoolFactory: No pool properties extracted from ndl for resource type: " + desc.getResourceType().getType());
                }
                else {
                    Globals.Log.debug("NdlResourcePoolFactory: Merging properties extracted from ndl with existing pool properties");
                    ...
                    Globals.Log.debug("NdlResourcePoolFactory: Merged final pool properties:" + desc.getPoolProperties());
                }

                List<ResourcePoolAttributeDescriptor> lpoolAttr = ndlPropExtractor.getPoolAttributes();
                if(lpoolAttr == null || lpoolAttr.size() == 0){
                    Globals.Log.debug("NdlResourcePoolFactory: No pool attributes extracted from ndl for resource type: " + desc.getResourceType().getType());
                }
                else {
                    for(Iterator it = lpoolAttr.iterator(); it.hasNext();){
                        Globals.Log.debug("NdlResourcePoolFactory: Adding attributed extracted from ndl with existing pool attributes");
                ...
                Globals.Log.warn("Illegal argument to constructor while creating an instance of ndlpropertyextractor class" + ex);
