...
public class VTRequestMapping extends RequestMapping {
	...
	public VTRequestMapping(String requestFile, String substrateFile, String currentSubstrateFileName) throws IOException{
	...
	public Hashtable <String,LinkedList <Device> > handleMapping(int numVM, Hashtable <String,NetworkConnection>requestMap,Hashtable <String,DomainResourceType> domainResourcePools){
		...
		/*
		for(int k =1; k<numCloud;k++){
			if(!needPartition(k,Hashtable <String,DomainResourceType> domainResourcePools){
				break;
			}
			OntModel vt=partition(requestModel,k,ghTree);
			mappingCost = vtMapping(vt,cloudMeshOntModel);
			if(mappingCost<minMappingCost){
				min_k=k;
			}
		}
		// expand the real inter-cloud links of min_k mapping 
	...
	public LinkedList directVTMapping(){
		...
		/*String whereStr="WHERE {"	+
							"?Node1 ndl:hasInterface/ndl:linkTo/ndl:interfaceOf ?Node2."	+
							"?Node3 ndl:hasInterface/ndl:linkTo/ndl:interfaceOf ?Node2"+	
							" }";*/
		
		String whereStr="WHERE {" +
							"?Node1 ndl:hasInterface ?Inf11."+
							"?Inf11 ndl:linkTo ?Inf21."+
							"?Inf21 ndl:interfaceOf ?Node2. "+
							
							"?Node2 ndl:hasInterface ?Inf22."+
							"?Inf22 ndl:linkTo ?Inf31. "+
							"?Inf31 ndl:interfaceOf ?Node3. FILTER(?Node1 != ?Node3) "+
							
							"?Node3 ndl:hasInterface ?Inf32."+
							"?Inf32 ndl:linkTo ?Inf41. "+
							"?Inf41 ndl:interfaceOf ?Node4.FILTER(?Node2 != ?Node4)"+
							
							"?Node4 ndl:hasInterface ?Inf42."+
							"?Inf42 ndl:linkTo ?Inf12. "+
							"?Inf12 ndl:interfaceOf ?Node1."+
				
			" }";
		/*String whereStr="WHERE {" +
				"?Node1 ndl:interfaceOf ?Node2."+
		" }";*/
	...
	public LinkedList vtMapping() throws IOException{
		...
		for(Entry <String,NetworkConnection > entry:requestMap.entrySet()){
			...
			whereStr+=" ?"+source + " ndl:hasInterface ?" + source_intf +". ?" +
						source_intf + " ndl:linkTo ?" + destination_intf +". ?"+
						destination_intf + " ndl:interfaceOf ?" + destination +".";
	...
	public boolean directConnected(Resource node_rs1,Resource node_rs2,int vt){
		...
		String whereStr="WHERE {" +
							"<"+node_rs1+">"+" ndl:hasInterface ?Inf1."+
							"?Inf1 ndl:linkTo ?Inf2."+
