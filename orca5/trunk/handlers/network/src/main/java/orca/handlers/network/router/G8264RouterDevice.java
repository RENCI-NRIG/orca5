...
public class G8264RouterDevice extends IBMRouterDevice {
	...
	/**
	 * Create a vlan. Note that the IBM switches take 
	 * QoS rate as <64-40000000>  kilobits per second, a multiple of 64 kbps. 
	 * Burst size as <32-4096>  kilobits, one of 8 values: 32 64 128 256 512 1024 2048 4096
	 */
	@Override
	public void createVLAN(String vlanTag, String qosRate, String qosBurstSize)
		...
		if ((qosBurstSize != null) && ((qInt = Long.parseLong(qosBurstSize.trim())) > 0)) {
			...
			if (!accepted) 
				throw new CommandException("QoS burst size must be in kbps, one of 32 64 128 256 512 1024 2048 4096, not " + qosBurstSize);
		}
