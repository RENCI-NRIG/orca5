...
/**
 * Class to build sequenced paths of NDL links and elements Elements can be added randomly and in the end a head-to-tail
...
public class NdlPath extends NdlCommons {
    ...
    /**
     * individual path elements are links, crossconnects or other resources that 'hasInterface' on interfaces. The way
     * to sequence them is to look at what interfaces they have. A path is a linked list of path elements
     * 
     * @author ibaldin
     *
     */
    ...
    private boolean tryPathRecursive(PathElement end, List<PathElement> path) {
        ...
        for (Resource u : unattached) {
            ...
            if (unInt.get(0).equals(last.getTail())) {
                ...
                candidates.add(pe);
            } else if (unInt.get(1).equals(last.getTail())) {
                if (debug)
        ...
        while (attached) {
            ...
            for (Resource u : unattachedTmp) {
                ...
                if (unInt.get(0).equals(current.getTail())) {
                    ...
                    if (debug)
                        System.out.println("    Attaching " + u + " via " + unInt.get(0));
                } else if (unInt.get(1).equals(current.getTail())) {
                    if (debug)
    ...
    private List<PathElement> assemblePath(Resource start, Resource end) throws NdlException {
        ...
        for (Resource i : headInterfaces) {
            if (debug)
                System.out.println(
                        "  Trying build a path from " + pathHead + " to " + pathTail + " using interface " + i);
    ...
    /**
     * Get a list of paths in this graph. Simple for point-to-point connections. For multipoint it is a list of paths
        ...
        if (roots.size() == 0) {
            if (ends.size() != 2) {
                ...
                throw new NdlException(
                        "Path has " + ends.size() + " (odd number) of endpoints: [" + sb.toString() + "] - expected 2");
            }
            ...
            if (roots.size() > 1) {
                ...
                for (Resource root : roots) {
                    ...
                    for (Resource otherRoot : tmpRoots) {
                        ...
                        if ((partPath == null) || (partPath.size() == 0))
                            throw new NdlException(
                                    "Unable to find path between multicast roots " + root + " and " + otherRoot);
