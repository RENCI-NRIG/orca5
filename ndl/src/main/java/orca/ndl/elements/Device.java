...
//import orca.network.LayerConstant.Action;
...
public class Device extends NetworkElement {
	...
	public void setParameters(OntModel ontModel, OntResource rs_ont){
		...
		if(st!=null) setManagementIP(st.getString()); 
		st=rs_ont.getProperty(NdlCommons.hostName_p);
		if(st!=null) setHostName(st.getString());
		st=rs_ont.getProperty(NdlCommons.isLabelProducer);
		if(st!=null) setLabelProducer(st.getBoolean());
		st=rs_ont.getProperty(NdlCommons.layerLabelIsPrimary);
		if(st!=null) setLayerLabelPrimary(st.getBoolean());
		...
		while(stit!=null && stit.hasNext()){
			...
			if(st!=null) sm.setDirection(st.getResource().getLocalName());
			...
			if(st!=null) this.setAllocatable(st.getBoolean());
			
			setSwitchMatrix(sm);
			setDirection(sm.getDirection());
			if(atLayer==null) atLayer=cap;
...
//					}			
	...
	public LinkedList <SwitchingAction> findUNIInterface(SwitchingAction action){
		...
		if(i>0) action.setDefaultAction(Action.Temporary.toString());
	...
	public void print(Logger logger){
		...
		if(switchingMatrix!=null){
			...
			for(i=0;i<size;i++){
				...
				if(matrix==null) logger.error("No Switching Matrix");
				else matrix.print(logger);
			}
		}
	...
	public void addSwitchingAction(SwitchingAction action){
		if(actionList==null) actionList=new LinkedList<SwitchingAction>();
	...
	public void setSwitchMatrix(SwitchMatrix matrix){
		if(switchingMatrix==null) switchingMatrix = new LinkedList<SwitchMatrix>(); 
