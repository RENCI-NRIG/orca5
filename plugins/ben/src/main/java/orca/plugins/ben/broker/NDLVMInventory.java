...
import orca.policy.core.util.SimplerUnitsInventory;
...
public class NDLVMInventory extends SimplerUnitsInventory {
    ...
    protected String [] rp = {ResourceProperties.ResourceNumCPUCores,ResourceProperties.ResourceMemoryCapacity,ResourceProperties.ResourceStorageCapacity};
    ...
    public void donate(IClientReservation source) {
        ...
        //To deal with the case when the units are partially donated to one of a few brokers, need to change the abstract RDF
        	...
        	try {
        		...
        		if(c!=null){
        			if(c.getBandwidth()>=value){
        				...
        				throw new RuntimeException("Insufficient <"+key+","+c.getBandwidth()+"> to meet request:"+value);
        			}
        		}
        		i++;	
