...
import net.exogeni.orca.policy.core.ResourceControl;
...
public class BenNdlControl extends ResourceControl {
    ...
    protected void closeConnectionCleanup(Unit u) {
        ...
        try {
            ...
            Properties teardownProperties = (Properties) propertiesConverterConvertMethod.invoke(null, teardown,
                    handler, logger);
    ...
    public void configurationComplete(String action, ConfigToken token, Properties outProperties) {
        ...
        if (Config.TargetJoin.equals(action)) {
            setInprogress(false);
            logger.debug("BenNdlControl.configurationComplete(): this request " + token.getReservationID()
                    + " is finished: inProgress = " + inprogress);
            ...
            if (result != 0) {
                logger.warn("BenNdlControl.configurationComplete(): reservation " + token.getReservationID()
                        + " failed, cleaning up");
                setcloseInProgress();
    ...
    public void close(IReservation reservation) {
        ...
        logger.debug(
                "Using properties converter " + propertiesConverter.getCanonicalName() + " in BenNdlControl.close()");
        ...
        if ((u != null) && (propertiesConverterConvertMethod != null)) {
            ...
            try {
                Properties setupProperties = (Properties) propertiesConverterConvertMethod.invoke(null, con, handler,
                        logger);
            ...
            unsetcloseInProgress();
            logger.debug("BenNdlControl.close(): Missing unit for reservation or unable to get convert method for "
    ...
    protected void releaseResources(Unit u) throws Exception {
        ...
        try {
             ...
             * if(releaseConnection==null){ setcloseInProgress(); NetworkConnection teardown =
             * handler.getConnectionTeardownActions(requestID); Properties teardownProperties =
             * BenNdlPropertiesConverter.convert(teardown); u.mergeProperties(teardownProperties); } releaseConnection =
