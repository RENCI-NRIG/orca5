package net.exogeni.orca.plugins.ben.control;
...
public class NlrNdlPropertiesConverter extends BenNdlPropertiesConverter {
    ...
    public static Properties convert(NetworkConnection con, NetworkHandler h, Logger logger) {
        ...
        String site = null; // get rid of "/vlan" etc. added to the network domain name to be consistent to the
                            // handlers.
        ...
        for (NetworkElement nd : config_device_list) {
            ...
            if (d.getResource().getProperty(NdlCommons.hostName_p) != null)
                PropList.setProperty(p, "unit.device." + numDevice + ".hostname",
                        d.getResource().getProperty(NdlCommons.hostName_p).getString());
            if (d.getResource().getProperty(NdlCommons.topologyManagementIP) != null)
                PropList.setProperty(p, "unit.device." + numDevice + ".managementip",
                        d.getResource().getProperty(NdlCommons.topologyManagementIP).getString());
            ...
            for (SwitchingAction a : actions) {
                ...
                if (isMulticast) {
                    ...
                    if (a.getDefaultAction().equals(LayerConstant.Action.VLANtag.toString())
                        ...
                        PropList.setProperty(p, UnitProperties.UnitVlanTag + "." + numAction + "." + site,
                                (int) a.getLabel_ID());
                        PropList.setProperty(p, UnitProperties.UnitBandwidth + "." + numAction + "." + site, a.getBw());
                        PropList.setProperty(p, UnitProperties.UnitBandwidth + ".burst." + numAction + "." + site,
                                (long) a.getBw() / 8);
                    }

                    if (ifs.size() != 2) {
                        logger.debug("Not 2 interfaces:" + ifs.size() + " ;d.name=" + d.getName() + " ;d.url="
                                + d.getURI() + " ;d.type=" + type);
                    ...
                    if (ifs.get(0).getName() == null && ifs.get(1).getName() == null) {
                        logger.debug("No interface name for configuration:" + ifs.get(0).getName() + ";"
                                + ifs.get(1).getName());
                        continue;
                    }
            ...
            if (isMulticast) {
                ...
                for (Entry<Interface, String> entry : exchange_intf_list.entrySet()) {
                    ...
                    logger.debug(site + ":" + intf.getName() + ";label=" + label + "e_tag=" + exchange_tag + "bw="
                            + exchange_bw);
                    PropList.setProperty(p, UnitProperties.UnitVlanTag + "." + numAction + "." + site, label);
                    // if(label!=exchange_tag)
                    PropList.setProperty(p, UnitProperties.UnitVlanTag + ".swap." + numAction + "." + site,
                            exchange_tag);
                    ...
                    logger.debug(site + ".action." + numAction + ".ports=" + intf.getName() + ":tag=" + label + ":swap="
                            + exchange_tag + ":bandwidth=" + exchange_bw);
                }
            }
        }
