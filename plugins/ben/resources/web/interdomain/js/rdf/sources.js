 ...
 * 
 * SVN ID: $Id: sources.js 2530 2007-01-09 16:17:00Z timbl $
 *
 ************************************************************/
...
function SourceFetcher(store, timeout, async) {
    ...
    SourceFetcher.RDFXMLHandler.term = this.store.sym(this.thisURI + ".RDFXMLHandler")
    SourceFetcher.RDFXMLHandler.toString = function() {
    ...
    SourceFetcher.XHTMLHandler = function(args) {
        ...
        this.recv = function(xhr) {
            xhr.handle = function(cb) {
                ...
                if (title.length > 0) {
                    kb.add(xhr.uri, kb.sym('dc', 'title'), kb.literal(title[0].textContent), xhr.uri)
                ...
                for (var x = links.length - 1; x >= 0; x--) {
                    if ((links[x].getAttribute('rel') == 'alternate' || links[x].getAttribute('rel') == 'seeAlso' || links[x]
                            .getAttribute('rel') == 'meta')
                            && links[x].getAttribute('href')) {
                        var join = Util.uri.join
                        var uri = kb.sym(join(links[x].getAttribute('href'), xhr.uri.uri))
                        kb.add(xhr.uri, kb.sym('rdfs', 'seeAlso'), uri, xhr.uri)
                ...
                if (head) {
                    ...
                    if (profile && Util.uri.protocol(profile) == 'http') {
                        log.info("GRDDL: Using generic " + "2003/11/rdf-in-xhtml-processor.");
                        sf.request(kb.sym('http://www.w3.org/2005/08/' + 'online_xslt/xslt?'
                                + 'xslfile=http://www.w3.org' + '/2003/11/' + 'rdf-in-xhtml-processor' + '&xmlfile='
    ...
    SourceFetcher.XHTMLHandler.term = this.store.sym(this.thisURI + ".XHTMLHandler")
    SourceFetcher.XHTMLHandler.toString = function() {
        return "XHTMLHandler"
    }
    ...
    SourceFetcher.XMLHandler = function() {
        this.recv = function(xhr) {
            xhr.handle = function(cb) {
                var kb = sf.store
                var dom = (new DOMParser()).parseFromString(xhr.responseText, 'application/xml')
                ...
                for (var c = 0; c < dom.childNodes.length; c++) {
                    ...
                    if (dom.childNodes[c].nodeType == 1) {
                        ...
                        if (dom.childNodes[c].namespaceURI == kb.namespaces['rdf']) {
                            log.info(xhr.uri + " seems to have a root element" + " in the RDF namespace. We'll assume "
                                    + "it's RDF/XML.")
                ...
                if (dom.doctype) {
                    ...
                    if (dom.doctype.name == 'html' && dom.doctype.publicId.match(/^-\/\/W3C\/\/DTD XHTML/)
                            && dom.doctype.systemId.match(/http:\/\/www.w3.org\/TR\/xhtml/)) {
                        log.info(xhr.uri + " has XHTML DOCTYPE. Switching to " + "XHTML Handler.")
    ...
    SourceFetcher.XMLHandler.term = this.store.sym(this.thisURI + ".XMLHandler")
    SourceFetcher.XMLHandler.toString = function() {
        return "XMLHandler"
    }
    ...
    SourceFetcher.HTMLHandler.term = this.store.sym(this.thisURI + ".HTMLHandler")
    SourceFetcher.HTMLHandler.toString = function() {
        return "HTMLHandler"
    }
    ...
    SourceFetcher.TextHandler.term = this.store.sym(this.thisURI + ".TextHandler")
    SourceFetcher.TextHandler.toString = function() {
        return "TextHandler"
    }
    ...
    Util.callbackify(this, [ 'request', 'recv', 'load', 'fail', 'refresh', 'retract', 'done' ])
    ...
    this.store.register('httph', "http://dig.csail.mit.edu/2005/ajar/ajaw/httph#")
    this.store.register('ical', "http://www.w3.org/2002/12/cal/icaltzd#")

    this.addProtocol = function(proto) {
        sf.store.add(sf.appNode, sf.store.sym("tab", "protocol"), sf.store.literal(proto), this.appNode)
    }
    ...
    this.store.add(this.store.sym('http://dig.csail.mit.edu/2005/ajar/ajaw/data#Tabulator'), this.store.sym('tab',
            "session"), this.appNode, this.appNode)
    this.store.add(this.appNode, this.store.sym('rdfs', 'label'), this.store.literal('This Session'), this.appNode);

    [ 'http', 'https', 'file' ].map(this.addProtocol); // ftp?

    [ SourceFetcher.RDFXMLHandler, SourceFetcher.XHTMLHandler, SourceFetcher.XMLHandler, SourceFetcher.HTMLHandler,
    ...
    this.addCallback('done', function(uri, r) {
        ...
        var refs = sf.store.statementsMatching(undefined, type, undefined, udoc)
        refs.map(function(x) {
    ...
    this.request = function(term, rterm, force) { //sources_request_new
        ...
        if (rterm) {
            if (rterm.uri) {
                kb.add(uri, kb.sym('tab', "requestedBy"), kb.sym(Util.uri.docpart(rterm.uri)), this.appNode)
            } else {
                kb.add(uri, kb.sym('tab', "requestedBy"), rterm, this.appNode)
            }
        }
        ...
        kb.add(req, kb.sym('rdfs', "label"), kb.literal('Request for ' + uri), this.appNode)
        ...
        if (typeof kb.anyStatementMatching(this.appNode, kb.sym('tab', "protocol"), Util.uri.protocol(uri.uri)) == "undefined") {
        ...
        xhr.onreadystatechange = function() {
            ...
            case 3:
                if (!xhr.recv) {
                    ...
                    kb.add(req, kb.sym('http', 'status'), kb.literal(xhr.status), sf.appNode)
                    kb.add(req, kb.sym('http', 'statusText'), kb.literal(xhr.statusText), sf.appNode)
                    ...
                    xhr.headers = {}
                    if (Util.uri.protocol(xhr.uri.uri) == 'http' || Util.uri.protocol(xhr.uri.uri) == 'https') {
                        xhr.headers = Util.getHTTPHeaders(xhr)
                        for ( var h in xhr.headers) {
                            kb.add(req, kb.sym('httph', h), xhr.headers[h], sf.appNode)
                        }
                    }
                    ...
                    if (loc) {
                        ...
                        if (!force && udoc != xhr.uri.uri && sf.requested[udoc]) {
                            // should we smush too?
                            log.info("HTTP headers indicate we have already" + " retrieved " + xhr.uri + " as " + udoc
                                    + ". Aborting.")
                            ...
                            return {
                                onChannelRedirect : function(oldC, newC, flags) {
                                    ...
                                    if (xhr.status == 302 || xhr.status == 303) {
                                        ...
                                        sf.redirectFetch(xhr, args, xhr.status + " to <" + newC.URI.spec + ">",
                                                newC.URI.spec)
                                    }
                                }
                            }
    ...
    this.isPending = function(uri) { // sources_pending
        var req = this.store.anyStatementMatching(this.store.sym(Util.uri.docpart(uri.uri)), this.store.sym('tab',
                'request'))
        if (!req) {
            return false
        }
        var status = this.store.anyStatementMatching(req.object, this.store.sym('tab', 'status'))
        if (!status) {
            return true
        }
        return (this.requested[Util.uri.docpart(uri.uri)] && !status.object.elements.filter(function(x) {
            return (x.toString() == 'done')
        }).length)
    }
}
...
function isExternal(uri) {
    for (x in kb.spEndpointIndex)
        if (uri.match(new RegExp('^' + x)))
            return kb.spEndpointIndex[x]; //redirect to external
}
