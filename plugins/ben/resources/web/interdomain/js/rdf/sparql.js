...
function SPARQLToQuery(SPARQL, testMode) {
    ...
    function parseLiterals(str) {
        //var sin = (str.indexOf(/[ \n]\'/)==-1)?null:str.indexOf(/[ \n]\'/), doub = (str.indexOf(/[ \n]\"/)==-1)?null:str.indexOf(/[ \n]\"/);
        var sin = (str.indexOf("'") == -1) ? null : str.indexOf("'"), doub = (str.indexOf('"') == -1) ? null : str
                .indexOf('"');
        //alert("S: "+sin+" D: "+doub);
        if (!sin && !doub) {
            var a = new Array(1);
        ...
        if (str.slice(end + ind + 2).match(/^\^\^/)) {
            ...
            //alert(end2)
            res[1] = kb.literal(str.slice(ind + 1, ind + 1 + end), "", kb.sym(removeBrackets(str.slice(ind + 4 + end,
                    ind + 2 + end + end2))))
            ...
            //alert(end2)
            res[1] = kb
                    .literal(str.slice(ind + 1, ind + 1 + end), str.slice(ind + 3 + end, ind + 2 + end + end2), null)
            //alert(res[1].datatype.uri)
        ...
        for (x = 0; x < input.length; x++) {
            ...
            if (isVar(input[x]))
                ...
                if (prefixes[getPrefix(input[x])])
                    ...
                    terror("SPARQL error: " + input[x] + " with prefix " + getPrefix(input[x])
                            + " does not have a correct prefix entry.")
                    res[x] = input[x]
                }
            } else
                res[x] = input[x];
    ...
    function setWhere(input, pat) {
        ...
        while (arrayIndexOf("OPTIONAL", terms)) {
            ...
            if (terms[opt + 1] != "{")
                twarn("Bad optional opening bracket in word " + opt)
            var end = getMatchingBracket(terms.slice(opt + 2), "{", "}")
            if (end == -1)
                terror("No matching bracket in word " + opt)
            else {
        ...
        while (arrayIndexOf("FILTER", terms)) {
            ...
            if (terms[filt + 1] != "(")
                twarn("Bad filter opening bracket in word " + filt);
            var end = getMatchingBracket(terms.slice(filt + 2), "(", ")")
            if (end == -1)
                terror("No matching bracket in word " + filt)
            else {
    ...
    for (x in q.pat.statements) {
        ...
        /*&& sf.isPending(st.subject.uri)*/) { //This doesn't work.
            sf.request(st.subject, "sparql:" + st.subject)
        }
        if (st.object.termType == 'symbol'
        /*&& sf.isPending(st.object.uri)*/) {
            sf.request(st.object, "sparql:" + st.object)
        }
    }
    //alert(q.pat);
    return q;
...
function SPARQLResultsInterpreter(xml, callback, doneCallback) {
    ...
    function xmlMakeTerm(node) {
        ...
        if (handleP(node.nodeName) == spns + "uri")
            ...
            twarn("Don't know how to handle xml binding term " + node);
        return false
    }
    function handleResult(result) {
        ...
        for (var x = 0; x < result.childNodes.length; x++) {
            ...
            if (handleP(result.childNodes[x].nodeName) != spns + "binding") {
                twarn("Bad binding node inside result");
                continue;
            }
    ...
    for (var x = 0; x < sparql.attributes.length; x++)
        //deals with all the prefixes beforehand
