...
function sources_fetch_remote(subject, externalURI) {
    //alert('fetch-remote')
    var uri = subject.uri
    var query = "SELECT ?pred ?obj ?isubj ?ipred WHERE { { <" + subject.uri
            + "> ?pred ?obj . } UNION { ?isubj ?ipred <" + subject.uri + "> . } } "
    ...
    kb.accessRemoteStore(query, externalURI, onreadystateloaded(subject, function(bindings) {
        ...
        if (pred && obj)
            kb.add(subject, pred, obj, kb.sym(externalSource(subject, externalURI)));
        if (isubj && ipred)
            kb.add(isubj, ipred, subject, kb.sym(externalSource(subject, externalURI)))
    }), uri)
}

function externalSource(subject, external) {
    if (subject.uri)
        var subject = subject.uri;
    if (external.indexOf('?') > 0)
        var external = external.slice(0, external.indexOf("?"))
