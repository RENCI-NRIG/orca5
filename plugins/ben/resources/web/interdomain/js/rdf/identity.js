...
function RDFIndexedFormula() {
    ...
    this.reverseNamespaces = {}; // reverse lookup of namespace by prefix uri
    ...
    function handleRDFType(formula, subj, pred, obj, why) {
        ...
        if (x)
            return x(formula, subj, pred, obj);
        return false; // statement given is needed
    } //handleRDFType

    //If the predicate is #type, use handleRDFType to create a typeCallback on the object
    this.propertyAction['<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>'] = handleRDFType;

    // Assumption: these terms are not redirected @@fixme
    this.propertyAction['<http://www.w3.org/2002/07/owl#sameAs>'] = function(formula, subj, pred, obj, why) {
        formula.equate(subj, obj);
        return false; // statement given is needed e.g. when dereferencing
    }; //sameAs -> equate & don't add to index

    this.propertyAction['<http://www.w3.org/2006/link#obsoletes>'] = function(formula, subj, pred, obj, why) {
    ...
    this.classAction["<" + owl_ns + "InverseFunctionalProperty>"] = function(formula, subj, pred, obj, addFn) {
        return newPropertyAction(formula, subj, handle_IFP); // yes subj not pred!
    }; //IFP -> handle_IFP, do add to index

    this.classAction["<" + owl_ns + "FunctionalProperty>"] = function(formula, subj, proj, obj, addFn) {
...
RDFIndexedFormula.prototype.register = function(prefix, nsuri) {
    ...
    this.reverseNamespaces[nsuri] = prefix;
}
...
RDFIndexedFormula.prototype.replaceWith = function(big, small) {
    ...
    if ((typeof action != 'undefined') && (typeof this.classAction[small.hashString()] == 'undefined')) {
        this.classAction[small.hashString()] = action;
    }

    action = this.propertyAction[big.hashString()];
    if ((typeof action != 'undefined') && (typeof this.propertyAction[small.hashString()] == 'undefined')) {
...
RDFIndexedFormula.prototype.statementsMatching = function(subj, pred, obj, why, justOne) {
        ...
        if (typeof (obj) == 'undefined') {
            if (typeof (pred) == 'undefined') {
                ...
                if (typeof candidates == 'undefined')
                    return [];
                fyi("Trying predicate " + pred + " index of: " + candidates.length + " for {" + subj + " " + pred + " "
                        + obj + " @ " + why + "}");
            }
    ...
    for (i = 0; i < candidates.length; i++) {
        ...
        RDFTermMatch(subj, st.subject) && RDFTermMatch(obj, st.object) && RDFTermMatch(why, st.why)) {
            // fyi("   Found: "+st)
            if (justOne)
                return [ st ];
            results.push(st);
...
RDFIndexedFormula.prototype.remove = function(st) {
    ...
    if (typeof this.subjectIndex[subj.hashString()] == 'undefined')
        twarn("statement not in sbj index: " + st);
    if (typeof this.predicateIndex[pred.hashString()] == 'undefined')
        twarn("statement not in pred index: " + st);
    if (typeof this.objectIndex[obj.hashString()] == 'undefined')
        twarn("statement not in obj index: " + st);
...
RDFIndexedFormula.prototype.removeMany = function(subj, pred, obj, why, limit) {
    fyi("entering removeMany w/ subj,pred,obj,why,limit = " + subj + ", " + pred + ", " + obj + ", " + why + ", "
            + limit);
