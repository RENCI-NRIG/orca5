...
//
// SVN ID: $Id: query.js 2298 2006-10-28 20:25:46Z timbl $
...
function predParentOf(node) {
    ...
    while (true) {
        if (n.getAttribute('predTR'))
            ...
            terror("Could not find predParent");
            return node
        }
    }
}
...
function makeQueryRow(q, tr, constraint) {
    ...
    var nodes = tr.childNodes, n = tr.childNodes.length, inverse = tr.AJAR_inverse, i, hasVar = 0, pattern, v, c, parentVar = null, level, pat;
...
function RDFUnifyTerm(self, other, bindings, formula) {
    ...
    if (!actual.complexType) {
        if (formula.redirection[actual])
            actual = formula.redirection[actual];
        if (formula.redirection[other])
            other = formula.redirection[other];
...
RDFIndexedFormula.prototype.query = function(foodog, callback, fetcher) {
    ...
    function optionalCallback(bindings, pat) {
        if (pat.optional.length == 0)
            callback(bindings);
        //alert("OPTIONAL: "+pat.optional)
        var tcount = function() {
            this.val = pat.optional.length;
            return this
        };
        var tc = new tcount();
        for (x in pat.optional) {
...
function prepare(f, item, bindings) {
    ...
    item.index = null;
    if (!f.statements)
        twarn("@@@ prepare: f is " + f);
    //    tdebug("Prepare: f has "+ f.statements.length);
    tdebug("Prepare: Kb size " + f.statements.length + " Preparing " + item);

    var t, c, terms = [ item.subject, item.predicate, item.object ], ind = [ f.subjectIndex, f.predicateIndex,
            f.objectIndex ];
    for (i = 0; i < 3; i++) {
        ...
        if (terms[i].isVar && !(terms[i] in bindings)) {
            ...
            if (f.redirection[t.hashString()])
                t = f.redirection[t.hashString()]; //redirect
            termIndex = ind[i]
...
function match(f, g, bindingsSoFar, level, fetcher, callback, branchCount) {
    ...
    if (fetcher) { //Fetcher is used to fetch URIs, function first term is a URI term, second is the requester
        ...
        var fetchResource = function(requestedURI, id) {
            sf.addCallback('done', function(uri) {
                if (!uri.sameTerm(requestedURI)) {
                    return true
                }
            ...
            fetcher(requestedURI, id)
        }
        for (i = 0; i < n; i++) {
            item = pattern[i]; //for each of the triples in the query
            if (item.subject in bindingsSoFar && bindingsSoFar[item.subject].uri
                ...
                return; //@@tbl
            } else if (item.object in bindingsSoFar && bindingsSoFar[item.object].uri
...
function match2(f, g, bindingsSoFar, level, fetcher, callback, branchCount) //post-fetch
{
    ...
    fyi(level + "Match2 searching " + item.index.length + " for " + item + "; bindings so far="
            + bindingDebug(bindingsSoFar));
    //var results = [];
    ...
    for (c = 0; c < nc; c++) { // For each candidate statement
        ...
        nbs1 = RDFArrayUnifyContents([ item.subject, item.predicate, item.object ], [ st.subject, st.predicate,
                st.object ], bindingsSoFar, f);
    ...
    if (branchCount.count == 0 && !branchCount.success) {
        ...
        tdebug("Branch finished. Tasks remaining: " + branchCount.numTasks.val + " Optional array length: "
                + g.optional.length);
        if (branchCount.numTasks.val == 0)
            branchCount.onFail();
