...
package orca.nodeagent.tests.security.unknown;
...
public class PWCallbackHandlerClient implements CallbackHandler {
    public void handle(Callback[] callbacks) throws UnsupportedCallbackException, IOException {
        String clientPassword = "trudykeypass";

        for (int i = 0; i < callbacks.length; i++) {
            if (callbacks[i] instanceof WSPasswordCallback) {
                WSPasswordCallback callback = (WSPasswordCallback) callbacks[i];

                System.out.println("Callback is of type " + callback.getClass().getName());
                System.out.println("Callback usage is " + callback.getUsage());
