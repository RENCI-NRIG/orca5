...
import net.exogeni.orca.embed.policyhelpers.VMPolicyNDLPoolPropertyExtractor;
import net.exogeni.orca.embed.policyhelpers.VlanPolicyNDLPoolPropertyExtractor;
...
public class DomainTest extends TestCase {
    ...
    public void testGetDomain() throws NdlException {
        ...
        String queryPhrase4 = "SELECT ?d ?r " + "WHERE {" + "?d" + " a ndl:NetworkDomain" + "      }";
         ...
         * 
         * // Execute the query and obtain results QueryExecution qe = QueryExecutionFactory.create(query,aM); results =
         * qe.execSelect();
         */
    ...
    public void testDelegateDomain() throws IOException, NdlException {
        ...
        for (int i = 0; i < numDomain; i++) {
            ...
            // java.lang.RuntimeException: http://geni-orca.renci.org/owl/mass.rdf#mass/Domain/vm:VM;Too many delegated
            // units from the config.xml, total=20, than specified in the site RDF units=8, please modify and restart
            // the actor!
            // java.lang.RuntimeException: http://geni-orca.renci.org/owl/starlight.rdf#starlight/Domain/vlan:VLAN;Too
            // many delegated units from the config.xml, total=20, than specified in the site RDF units=11, please
            // modify and restart the actor!
            DomainResources res;
            if (i == 3 || i == 6) {
