...
import net.exogeni.orca.embed.cloudembed.MultiPointNetworkHandler;
import net.exogeni.orca.embed.cloudembed.NetworkHandler;
import net.exogeni.orca.embed.cloudembed.PortHandler;
...
import net.exogeni.orca.embed.workflow.RequestParserListener;
...
public class InterDomainHandlerTest extends MappingHandlerTest {
    ...
    String requestFile3, requestFile4, requestFile5, requestFile33, requestFile20Duke, requestFileVMSDuke,
            requestFileUNC;
    InterDomainHandler handler;

    String substrateFileName, nlrsubstrateFileName, rencivmsubstrateFileName, dukevmsubstrateFileName,
            uncvmsubstrateFileName;
    ...
    public void print(LinkedList<NetworkElement> connection) throws IOException, RequestMappingException, NdlException {
        ...
        for (NetworkElement domain : connection) {
            ...
            // if(d.getURI().equals("http://geni-orca.renci.org/owl/rencivmsite.rdf#rencivmsite/Domain/GEPort"))
            // d.setDownNeighbourPortsList(portNDLControl(out.toString(),1));
            // if(d.getURI().equals("http://geni-orca.renci.org/owl/dukevmsite.rdf#dukevmsite/Domain/GEPort"))
            // d.setDownNeighbourPortsList(portNDLControl(out.toString(),2));

            System.out.println(
                    "===> Device name=" + d.getName() + "(" + d.getURI() + ")" + " action count=" + d.getActionCount());
            System.out.println("isAllocatable=" + d.isAllocatable() + ":" + d.getSwappingCapability() + ":"
                    + d.isLabelProducer() + ":" + d.getDegree() + ":" + d.isDepend() + "\n");
            // System.out.println("Reservation Term:"+d.getStartingTime()+":"+d.getEndingTime()+"\n");
            if (d.getResourceType() != null)
                System.out.println("Reservation unit:" + d.getResourceType().getResourceType() + ":"
                        + d.getResourceType().getCount());
            ...
            if (d.getFollowedBy() != null) {
                for (Entry<DomainElement, OntResource> follower : d.getFollowedBySet()) {
                    ...
                    /*
                     * if(d.getDownNeighbourPortsList()!=null){ intf_name=d.getDownNeighbourPortsList();
                     * follower.getValue().addProperty(handler.getMapper().RDFS_Label,intf_name); }
                     */
                    System.out.println("Followed By:" + follower.getKey().getURI() + ":"
                            + follower.getValue().getProperty(NdlCommons.RDFS_Label));
                    System.out.println(intf_name + ":" + follower.getValue() + ":"
                            + follower.getValue().getProperty(NdlCommons.layerLabelIdProperty) + "\n");
            ...
            for (SwitchingAction a : actions) {
                ...
                System.out.print(
                        "Action=" + a.getDefaultAction() + " Label=" + label_id + " BandWidth = " + a.getBw() + "\n");
    ...
    public void MPControl(String mpRequest) throws IOException, InetNetworkException, NdlException {
        ...
        /*
         * mphandler.getConnectionTeardownActions(request1); try { mphandler.releaseReservation(request1); } catch
     ...
     * 
     * ByteArrayInputStream is = new ByteArrayInputStream(portRequest.getBytes()); String portList = null; if(end==1){
     * 
     ...
     * 
     * portHandler1.releaseReservation(request); } else if(end==2){
     * 
     ...
     * 
     * portHandler2.releaseReservation(request); } System.out.println("Port List:"+portList);
     * 
     * 
     * return portList; }
     */
    ...
    public void print(NetworkConnection con) {
        ...
        /*
         * for (Object o : list) { if (o instanceof Device) { Device d = (Device)o; String uri = d.getUri(); if
         * (uri.indexOf("#Renci") != -1) { // this is a renci device renci.add(d); } else if (uri.indexOf("#UNC") != -1)
         * { // this a UNC device unc.add(d); } else { throw new RuntimeException("Device is from an unknown site: " +
         * uri); } } }
         */
        Properties p = new Properties();
        /*
         * print2("unc", unc, p); print2("renci", renci, p);
         */
        print2("BEN", list, p);
    ...
    public void convertToOrca(LinkedList<NetworkElement> connection) {
        ...
        for (NetworkElement device : connection) {
            ...
            for (Entry<DomainElement, OntResource> parent : d.getPrecededBySet()) {
                ...
                DatatypeProperty hostInterfaceName = handler.getIdm()
                        .createDatatypeProperty("http://geni-orca.renci.org/owl/" + "topology.owl#hostInterfaceName");
                ...
                if (site_host_interface == null) {
                    ...
                    System.out.println(
                            parent.getKey() + ":" + parent.getKey().getDownNeighbour(parent.getKey().getModel()) + ":"
                                    + parent.getKey().getUpNeighbour(parent.getKey().getModel()) + "\n");
                    if (parent.getKey().getDownNeighbour(parent.getKey().getModel()) != null) {
                        if (parent.getKey().getDownNeighbour(parent.getKey().getModel())
                                .getProperty(hostInterfaceName) != null) {
                            site_host_interface = parent.getKey().getDownNeighbour(parent.getKey().getModel())
                                    .getProperty(hostInterfaceName).getString();
                        } else {
                            if (parent.getKey().getUpNeighbour(parent.getKey().getModel()) != null) {
                                if (parent.getKey().getUpNeighbour(parent.getKey().getModel())
                                        .getProperty(hostInterfaceName) != null)
                                    site_host_interface = parent.getKey().getUpNeighbour(parent.getKey().getModel())
                                            .getProperty(hostInterfaceName).getString();
                            }
                        }
                    } else {
            ...
            if (actions != null) {
                for (SwitchingAction a : actions) {
                    System.out.print("Domain: " + domain + " Action=" + a.getDefaultAction() + " Label="
                            + a.getLabel_ID() + " Bandwidth=" + a.getBw() + "\n");
