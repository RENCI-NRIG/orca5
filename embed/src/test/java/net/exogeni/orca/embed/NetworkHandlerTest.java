...
public class NetworkHandlerTest extends TestCase {
    ...
    public void _testHandleMapping() throws IOException, InetNetworkException, NdlException {
        ...
        /*
         * handler.handleRequestFile(requestFileUNCRenci); request2=handler.getRequest().getReservation();
         * 
         * NetworkConnection con2 = handler.getDeviceConnection(); print(con2);
         * 
         * handler.getConnectionTeardownActions(request2); handler.releaseReservation(request2);
         * 
         * handler.handleRequestFile(requestFileDukeUNC);
         * 
         * request3=handler.getRequest().getReservation();
         * 
         * NetworkConnection con3 = handler.getDeviceConnection(); print(con3);
         * 
         * handler.getConnectionTeardownActions(request3); handler.releaseReservation(request3);
         */
                ...
                for (int i = 0; i < actionCount; i++) {
                    ...
                    if (ifs.size() != 2) {
                        ...
                        PropList.setProperty(p, site + "." + name + ".action." + (anum + 1) + ".dport",
                                ifs.get(1).getName());
                        System.out.println(
                                site + "." + name + ".action." + (anum + 1) + ".sport=" + ifs.get(0).getName());
                        System.out.println(
                                site + "." + name + ".action." + (anum + 1) + ".dport=" + ifs.get(1).getName());
                        anum++;
                    }
                }
