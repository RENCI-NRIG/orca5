...
public class Domain implements IDomainAbstractor {
    ...
    public String updateAbstactModel(String str, int total, BitSet bSet) throws NdlException {
        ...
        for (StmtIterator j = networkService.listProperties(NdlCommons.availableLabelSet); j.hasNext();) {
            ...
            if (!res.hasType(type)) {
                ...
                Statement typeRank = set.getProperty(NdlCommons.domainHasResourceTypeProperty).getResource()
                        .getProperty(NdlCommons.resourceTypeRank);
                if (typeRank != null)
        ...
        if (total_units >= total) {
            ...
            throw new RuntimeException(domain + ":" + type + ";Too many delegated units from the config.xml, total="
                    + total + ", than specified in the site RDF units=" + total_units
                    + ", please modify and restart the actor!\n");
        }
    ...
    // to be called by the broker policy for vlan resource, to get the allocatable resource <type, unit>, and
    // <interface,bw>, etc constraints
    ...
    public DomainResources getDomainResources(OntModel delegateModel, int total) throws IOException, RuntimeException {
        ...
        for (StmtIterator i = domain.listProperties(NdlCommons.topologyHasInterfaceProperty); i.hasNext();) {
            ...
            while (true) {
                if (intf.hasProperty(NdlCommons.RDF_TYPE,
                        delegateModel.createResource(NdlCommons.ORCA_NS + "ethernet.owl#EthernetNetworkElement"))) {
                    rs = intf;
                    break;
                }
                ...
                if (rs.hasProperty(NdlCommons.RDF_TYPE,
                        delegateModel.createResource(NdlCommons.ORCA_NS + "ethernet.owl#EthernetNetworkElement"))) {
    ...
    public OntModel delegateDomainModel(String resourceType) {
        ...
        if (resourceType != null) {
            ...
            logger.error("Domain::delegateDomainModel():Domain Delegation Error:Required type is NULL!");
            return null;
        }

        HashMap<String, String> delegate = new HashMap<String, String>();

        // 1. Get the abstract NDL model, NetworService(delegated resource,etc.) and edge ports resource information
        ...
        Property[] serviceProperty = { NdlCommons.domainHasAccessMethod, NdlCommons.domainHasAggregateManager,
                NdlCommons.domainHasTopology, NdlCommons.atLayer, NdlCommons.hasCastType };
    ...
    public OntModel abstractDomain(OntModel model, String rType) {
        ...
        if (rType == null)
            domainName = domain.getURI();
        else
            domainName = domain.getURI() + "/" + rType;
            ...
            if (intf_type) {
                ...
                while (true) {
                    for (StmtIterator j = currentIntf_rs.listProperties(); j.hasNext();) {
                        ...
                        /*
                         * if(statement.getPredicate().equals(NdlCommons.RDF_TYPE)){ for (StmtIterator
                         * k=statement.getResource().listProperties();k.hasNext();){ abstractModel.add(k.next()); } }
                         */
                    }
    ...
    public void addSwitchingMatrix(OntModel abstractModel, OntResource device, OntResource intf) {
        ...
        while (results.hasNext()) {
            ...
            sw = sm.getProperty(NdlCommons.swappingCapability) == null ? null
                    : sm.getProperty(NdlCommons.swappingCapability).getResource();
            st = sm.getProperty(NdlCommons.tunnelingCapability) == null ? null
                    : sm.getProperty(NdlCommons.tunnelingCapability).getResource();
            // System.out.print("TunnelCapability:"+sm+"::"+st+"\n");
            if (!scList.isEmpty()) {
                if (scList.get(sc) != null)
                    if (scList.get(sc).equals(sw))
                        continue;
            } else {
                sm_ont = abstractModel.createIndividual(domainElement.getURI() + "/SwitchingMatrix",
                        NdlCommons.switchingMatrixOntClass);
