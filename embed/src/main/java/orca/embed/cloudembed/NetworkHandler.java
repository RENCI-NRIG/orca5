...
public class NetworkHandler extends MappingHandler 
{
	...
	public SystemNativeError runEmbedding(String domainName,RequestReservation rr) {
		...
		for(Entry <String, BitSet>entry:nhState.localAssignedLabel.entrySet()){
			logger.debug("NetworkHandler.runEmbedding(): set used layer label="+entry.getKey()+"---"+entry.getValue());
		...
		//if the request is from the edge interfaces, adding the two devices/interfaces connecting to these 2 edge interfaces to the substrate model
	...
	public void setUsedLabel(LinkedList<NetworkElement> deviceList,boolean swapping){
		...
		for(NetworkElement nd:deviceList){
	        ...
	        if(swapping && (d.getURI().contains("up") || d.getURI().contains("down")) ){ //the first and last devices are the servers that may carry labels 
					...
					if(intf!=null){
						...
						if(intf.getResource().hasProperty(NdlCommons.linkTo)){
							...
							if(p_intf_rs!=null){
								...
								p_tag=this.mapper.get_pdomain_tag(p_intf_rs.getURI(),static_label,this.pdomain_properties);
								if( (p_tag!=0) && (layerBitSet!=null) ){
						        	logger.debug("set used tag from the parent p_intf="+p_intf_rs.getURI() +"; p_tag = "+p_tag);
	...
	public void edgeRequestModel(OntModel requestModel){
		/*try {
			FileOutputStream ben_os = new FileOutputStream("ben-request.rdf");
		...
		while (results.hasNext()){
			...
			ont_rs1=substrateModel.getOntResource(rs1);
			ont_rs2=substrateModel.getOntResource(rs2);

			ont_rs1.removeProperty(NdlCommons.connectedTo, rs2);
			ont_rs2.removeProperty(NdlCommons.connectedTo, rs1);
		}*/
	}
