...
public class RequestMapping extends OntProcessor implements IConnectionManager {
    ...
    // public static String outFile_int=PathGuesser.getOrcaControllerHome() + "logs" +
    // System.getProperty("file.separator") + "perf_int.dat";
    // public static File file_int = new File(outFile_int);
    // public static String outFile=PathGuesser.getOrcaControllerHome() + "logs" + System.getProperty("file.separator")
    // + "perf.dat";
    ...
    public RequestMapping(OntModel rModel, OntModel substrateModel, boolean inter) throws IOException {
        ...
        /*
         * try { if(inter) output = new BufferedWriter(new FileWriter(file_int,true)); else output = new
         * BufferedWriter(new FileWriter(file,true));
         * output.write(ontModel.size()+":"+ontModel.listOntologies().toSet().size()+":"+ontModel.listIndividuals().
         * toSet().size()); } catch (IOException e) { // TODO Auto-generated catch block e.printStackTrace(); }
         */
    }
    ...
    public RequestMapping(OntModel substrateModel) {
        ontModel = substrateModel;
        /*
         * try { output = new BufferedWriter(new FileWriter(file)); output.write(ontModel.size()+":"); } catch
    ...
    public SystemNativeError createConnection(NetworkConnection requestConnection, boolean interDomainRequest,
        ...
        logger.debug("RequestMapping::createConnection() IN");
        ...
        /*
         * try { output.write((solution.size()/2+1)+":"+time+"\n"); output.close(); } catch (IOException e) { // TODO
        ...
        if (error != null)
            logger.error(error);
        logger.debug("RequestMapping::createConnection() OUT");
        return error;
    }
    ...
    private synchronized NetworkConnection toConnection(ArrayList<ArrayList<OntResource>> path,
            NetworkConnection requestConnection, boolean interDomainRequest) {
        logger.debug("RequestMapping::toConnection() IN");
        ...
        if (path != null) {
            if (!path.isEmpty()) {
                ...
                for (int i = 0; i < path.size() - 1; i = i + 2) {
                    ...
                    logger.info("RequestMapping::toConnection():Link " + i + ": Creat Link Connection:" + 
                                device_ont + ":" + intf_ont + "\n");
                    ...
                    if (!deviceOntList.contains(next_device_ont)) {
                        ...
                        next_device = getDevice(next_device_ont, deviceList);
                    ...
                    // add requested resource to the edge domains and intermedium domain according to the label set of
                    ...
                    // Directly add a connectedTO between the two interfaces, in case they're connected via pre-switched
                    // path.
                    boolean cc = false;
                    Resource j_res = null;
                    ...
                    if (!cc) {
                        ...
                        logger.debug("RequestMapping::toConnection():Wired via existed crossconnect" + intf_ont + 
                                     ":" + next_intf_ont);
                    }
                }
        ...
        logger.debug("RequestMapping::toConnection() OUT");
    ...
    public boolean processAction(Interface intf, Device device, long bw) {
        ...
        logger.debug("RequestMapping::processAction(): IN");
        ...
        if (device.getActionList() != null) { // another request??
            ...
            logger.debug("RequestMapping::instantiate action");
            action = new SwitchingAction(ontModel);
            device.addSwitchingAction(action);
        }

        action.addClientInterface(intf);
        logger.debug("RequestMapping::processAction():add a new interface to the action:" + intf.getURI() + 
                     ":" + action.getClientInterface().size() + "\n");
        ...
        logger.debug("RequestMapping::processAction(): OUT");
    ...
    public Resource toDomain(Resource rs, String resourceType) {
        ...
        if (NdlCommons.isStitchingNodeInManifest(rs)) {
            ...
            boolean found = false;
            for (ResIterator j = ontModel.listResourcesWithProperty(NdlCommons.RDF_TYPE,
                ...
                // break;
                // }
                if ((rDomain.getLocalName().equalsIgnoreCase(rType))
                        && (rDomain.getURI().startsWith(domain.getURI()))) {
                    found = true;
                    break;
                }
            }
        ...
        if (deviceConnection != null) {
            ...
            while (it.hasNext()) {
                ...
                if (static_label != null) {
                    logger.info("RequestMapping::processDeviceConnection():Num of actions:" + 
                                d.getActionCount() + ";" + d.getActionList().size()
            ...
            Collections.sort(deviceList, new NetworkElement.LayerComparator()); // sort the devices according to their
                                                                                // layers, lower layer first
        }
        ...
        if (matrixList != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (matrix == null)
                    ...
                    logger.debug("RequestMapping::processInterface():" + i + ":SwitchingMatrix layer:" + 
    ...
    public void processAdaptation(Device device, String currentLayer) {
        ...
        for (int j = 0; j < actionCount; j++) {
            ...
            logger.info("RequestMapping::processAdaptation():---------Adaptation Client --------" + 
                        device.getURI() + ":" + actionCount + ":" + currentLayer
                    + ":" + action.getAtLayer());
            if (action.getAtLayer().equals(currentLayer)) {
                ...
                for (i = 0; i < size; i++) {
                    ...
                    if (intf_client != null) {
                        ...
                        logger.debug("RequestMapping::processAdaptation():Current action:" + intf_client.getURI() + 
                                     ":" + action.getLabel_ID());
                    } else {
                        logger.error("RequestMapping::processAdaptation():Client interface not defined," + 
                                     " probably due to lack of available label");
                    }
                }
                ...
                while (it.hasNext()) {
                    ...
                    for (int k = i; k < size; k++) {
                        ...
                        if (intf.getLabel() != null) {
                            label_rs = intf.getLabel().getResource(intf.getModel()) != null
                                    ? intf.getLabel().getResource(intf.getModel())
                                    : intf.getLabel().getResource(ontModel);
                            if (label_rs != null)
                                rs.addProperty(label_p, intf.getLabel().getResource(intf.getModel()));
                            else
                                logger.error("RequestMapping::processAdaptation():label_rs is null: label=" + 
                                             intf.getLabel().getURI());
                        }
                        Label k_label = interfaceList.get(k).getLabel();
                        if (k_label != null) {
                            label_rs = k_label.getResource(interfaceList.get(k).getModel()) != null
                                    ? k_label.getResource(interfaceList.get(k).getModel())
                                    : intf.getLabel().getResource(ontModel);
                            if (label_rs != null)
    ...
    public Interface getAdaptationClientInterface(Interface parent, String currentLayer, Label currentLabel,
        ...
        Resource label_rs = null;
        logger.info("RequestMapping::getAdaptationClientInterface():Client:" + parent.getURI() + 
                    ":" + parent.getAtLayer() + ";" + currentLayer + "\n");
        if (clientList == null) {
            if (currentLayer.equals(Layer.LambdaNetworkElement.toString())) {
                if (parent.getAtLayer().equals(Layer.OCGNetworkElement.toString())) {
                    ...
                    if (label_rs != null)
                        ...
                        logger.error("RequestMapping::getAdaptationClientInterface():No more available label " + 
                                     "form this OCG group!");
                        return null;
                    }
                    ...
                    logger.info("RequestMapping::getAdaptationClientInterface():Interface setLabel:" + url + 
                                "--" + label_rs.getURI());
                    ...
                    intf.getResource().addProperty(NdlCommons.topologyInterfaceOfProperty,
                            parent.getResource().getProperty(NdlCommons.topologyInterfaceOfProperty).getResource());
                }
            }
        } else {
    ...
    public Interface processClientInterface(Interface intf, Label currentLabel, Label labelFromStack) {
        ...
        if (intf.getAtLayer().equals(Layer.EthernetNetworkElement.toString())) {
            ...
            if (label_rs != null && label_rs.getProperty(NdlCommons.layerLabelIdProperty) != null) {
                ...
                logger.error("RequestMapping::processClientInterface():label_rs is null");
            }
        ...
        logger.info("RequestMapping::processClientInterface():Obtained Label ID:" + label_rs + ":" + intf.getURI());
    ...
    public Label findCommonLabel(LinkedList<Device> deviceList) throws Exception {
        ...
        while (it.hasNext()) {
                ...
                if (action != null) {
                    ...
                    if (intf != null) {
                        ...
                        if (intf.getResource().hasProperty(NdlCommons.linkTo)) {
                            ...
                            logger.info("RequestMapping::findCommonLabel():intf=" + intf.getResource().getURI() + 
                                        ";p_intf=" + p_intf_rs.getURI());
                        }

                        if (intf.getResource().hasProperty(NdlCommons.layerLabel)) {
                            static_label_rs = null;
                            for (StmtIterator j = intf.getResource().listProperties(NdlCommons.layerLabel); j
                                    .hasNext();) {
                                static_label_rs = j.next().getResource();
                                if (static_label_rs.getProperty(NdlCommons.visited) != null) { // defined in
                            ...
                            for (Resource static_rs : static_label_rs_list) {
                                if (static_rs.hasProperty(NdlCommons.layerLabelIdProperty)) {
                                    ...
                                    logger.info("RequestMapping::findCommonLabel():static_rs=" + static_rs.getURI() + 
                                    ...
                                    if (p_tag == static_label) {
                                        ...
                                        logger.info("RequestMapping::findCommonLabel():Passed label:intf=" + 
                                                    p_intf_rs.getURI() + ";static Label="
                                        ...
                                        // static_label = p_tag;
                                        static_label = -1;
                                        static_label_rs = this.ontModel.createIndividual(
                                                p_intf_rs.getNameSpace() + String.valueOf(p_tag),
                                                NdlCommons.labelOntClass);
                                        static_label_rs = intf.getModel().createIndividual(
                                                p_intf_rs.getNameSpace() + String.valueOf(p_tag),
                                                NdlCommons.labelOntClass);
                                        static_label_rs.addProperty(NdlCommons.layerLabelIdProperty,
                                                String.valueOf(p_tag), XSDDatatype.XSDint);
                                        logger.error("RequestMapping::findCommonLabel():Passed in static label is not in " + 
                                                     "the available labelset:static="
                                                + static_label + ";set=");
                                        throw new Exception(
                            ...
                            if (p_tag != 0) {
                                ...
                                static_label_rs = intf.getModel().createIndividual(
                                        p_intf_rs.getNameSpace() + String.valueOf(p_tag), NdlCommons.labelOntClass);
                                static_label_rs.addProperty(NdlCommons.layerLabelIdProperty, String.valueOf(p_tag),
                                        XSDDatatype.XSDint);
                                /*
                                 * OntResource static_label_ont =
                                 * this.ontModel.createOntResource(static_label_rs.getURI()); label = new
                                 * Label(static_label_ont,p_tag,static_layer); action.setLabel(label);
                                 * action.setLabel_ID(p_tag); p_intf_rs.addProperty(NdlCommons.layerLabel,
                                 * static_label_rs);
                                 */
                                logger.info("RequestMapping::findCommonLabel():Passed label from domain reservation:intf=" + 
                                            p_intf_rs.getURI() + ";tag="
            ...
            if (matrixList != null) {
                ...
                for (int i = 0; i < size; i++) {
                    matrix = (SwitchMatrix) matrixList.get(i);
                    logger.debug("RequestMapping::findCommonLabel():Device:" + device.getResource().getLocalName() + 
                                 ":" + i + ":SwitchingMatrix layer:"
                            + matrix.getAtLayer() + ":size=" + size);
        ...
        logger.debug("RequestMapping::findCommonLabel():static_label=" + static_label + 
                     ";d_swapping=" + d_swapping + "currentLayer="
                     + currentLayer);
        ...
        if (static_label > 0 && d_swapping == null) {
            if (staticLabelSet == null) {
                ...
                logger.debug("RequestMapping::findCommonLabel():check match:" + current_static_label + 
                             ";static_label=" + static_label);

                if (!staticLabelSet.get((int) static_label)) {
                    current_static_label = staticLabelSet.nextSetBit(0);
                    logger.error("RequestMapping::findCommonLabel():Passed in static label is not in the available " + 
                                 "labelset:static=" + static_label
                                 + ";set=" + staticLabelSet);
                    throw new Exception("Passed in static label is not in the available labelset:static=" + static_label
                            + ";set=" + staticLabelSet);

                    /*
                     * Note: if the site becomes d_swapping, it should pick a local available tag, and swap the passed
                     * in tag.
                     * 
                     * logger.error("RequestMapping::findCommonLabel():AM choose one available instead"+current_static_label); String
                     * label_namespace=null; if(static_label_rs!=null) label_namespace=static_label_rs.getNameSpace();
                     * else label_namespace=device.getResource().getNameSpace();
                     * 
                     * static_label_rs=this.ontModel.createResource(label_namespace+String.valueOf(current_static_label)
                     * );
                     * static_label_rs.addProperty(NdlCommons.layerLabelIdProperty,String.valueOf(current_static_label),
                     * XSDDatatype.XSDint); static_label=current_static_label;
                     */
                }
            }
        }
        ...
        if (static_label_rs != null) {
            OntResource static_label_ont = this.ontModel.createIndividual(static_label_rs.getURI(),
                    NdlCommons.labelOntClass);
    ...
    public Resource processLabel(Interface intf, Label currentLabel, Label labelFromStack) {
        ...
        Resource label_rs = processLabel(intf.getModel(), rsURI, availableLableSet, usedLabelSet, usedLabelSet_str,
                usedSet, currentLabel, labelFromStack, currentLayer);

        return label_rs;
    }

    public Resource processLabel(OntModel m, String rsURI, String availableLableSet_str, String usedLabelSet,
            String usedLabelSet_str, String usedSet_str, Label currentLabel, Label labelFromStack,
            String currentLayer) {

        if (currentLabel != null) {
            if ((currentLabel.getResource(m) != null) && (currentLabel.label > 0)
                    && (currentLayer.equals(Layer.EthernetNetworkElement.toString()))) {
        ...
        Resource picked_label_rs = ontLabelUpdate(availableSet_rs, lowestLabelRange_rs, lowest, lowestUpper,
                lowestLabel, lowestUpperLabel, stackLabel.intValue());
    ...
    public void setReleaseNetworkConnection(NetworkConnection releaseNetworkConnection) {
    ...
    public void setUsedLabelSetPerLayer(String layer, BitSet usedLabelSet) {
        if (this.usedLabelSetPerLayer == null) {
