...
public class ConnectionManager extends RequestMapping {
    ...
    ConnectionManager(OntModel rModel, OntModel substrateModel, boolean inter) throws IOException {
                                    ...
                                    if (i < size - 1) {
                                        ...
                                        if (type_peer_next != null) {
                                            if ((!type_peer_next.equals("vm")) && peerLayer(device, device_peer_next)) {
                                                if (type_peer_next.equals("vlan")) { // if it is vlan layer, no
                                                                                     // multi-hop connection
                                                    connection_ont = createPeerConnection(device, device_peer,
                                                            deviceList);
                                                } else
                                                    continue;
                                            } else {
                                            ...
                                            if (connection_ont != null) {
                                                ...
                                                StmtIterator stit = connection_ont
                                                        .listProperties(NdlCommons.collectionItemProperty);
    ...
    public OntResource createPeerConnection(Device device, Device device_peer, LinkedList<Device> deviceList) {
        ...
        OntResource device_ont = ontModel
                .getOntResource(intf.getProperty(NdlCommons.topologyInterfaceOfProperty).getResource());
    ...
    public OntResource toNetworkConnection(Resource intf0_rs, Resource intf1_rs, Device device0, Device device1) {
        ...
        while (results.hasNext()) {
            ...
            if (!in2.getURI().equals(device0.getDownNeighbour(device0.getModel()))
                    && (device1.getDownNeighbour(device1.getModel()) == null)) // last hop
                continue;
            if (!in0.getURI().equals(device1.getUpNeighbour(device1.getModel()))
                    && (device0.getUpNeighbour(device0.getModel()) == null)) // first hop
                continue;
        ...
        for (Entry<String, Resource[]> link : linkList.entrySet()) {
            if ((link.getValue()[1].getProperty(NdlCommons.topologyInterfaceOfProperty).getResource() != intf0_rs
                    .getProperty(NdlCommons.topologyInterfaceOfProperty).getResource())
                    && (link.getValue()[2].getProperty(NdlCommons.topologyInterfaceOfProperty).getResource() != intf0_rs
                            .getProperty(NdlCommons.topologyInterfaceOfProperty).getResource())) {
                if ((link.getValue()[1].getProperty(NdlCommons.topologyInterfaceOfProperty).getResource() != intf1_rs
                        .getProperty(NdlCommons.topologyInterfaceOfProperty).getResource())
                        && (link.getValue()[2].getProperty(NdlCommons.topologyInterfaceOfProperty)
    ...
    public boolean switchingActionInModel(SwitchingAction action, String current_layer, boolean releaseFlag,
            String requestURI) {
