package net.exogeni.orca.embed.cloudembed.controller;
...
import net.exogeni.orca.embed.cloudembed.IConnectionManager;
...
public class UnboundRequestHandler extends MultiPointHandler {
    ...
    public SystemNativeError runEmbedding(RequestReservation rr, DomainResourcePools domainResourcePools,
        ...
        for (Entry<String, Integer> requestEntry : rr.getTypeTotalUnits().entrySet()) {
            ...
            if ((rTypeDomainList == null) || (rTypeDomainList.size() == 0)) {
                ...
                error.setErrno(10);
                error.setMessage(
                        "No available substrate for the required resource:" + requestEntryType + ":" + requestUnits);
                return error;
            }
            int totalCount = 0;
            ...
            if (requestUnits > totalCount) {
                ...
                error.setErrno(1);
                error.setMessage(
                        "Insufficient available resources for the request:" + requestEntryType + ":" + requestUnits);
                return error;
            }
        }
        ...
        for (Entry<String, Integer> requestEntry : rr.getTypeTotalUnits().entrySet()) {
            ...
            if (requestUnits <= delta) { // required units smaller than the threshold, embedding in one domain.
                ...
                if (second > 0) {
                    ...
                    RequestReservation twoWayRequest = generateVCConnection(rr, requestEntryType, x1, x2, max_domain,
                            second_domain);
                    try {
                    ...
                    error = new SystemNativeError();
                    error.setErrno(100);
                    error.setMessage("No splitting, only one site available: Requested type:" + requestEntryType + ":"
                            + requestUnits);
                }
            } else {
                /*
                 * VTRequestMapping vtMapper; try { vtMapper = new VTRequestMapping(this.getIdm()); RequestReservation
                 * request = vtMapper.handleMapping(rr,domainResourcePools,requestEntryType);
                 * error=runEmbedding(request, domainResourcePools); rr.setElements(request.getElements()); } catch
                 * (IOException e) { // TODO Auto-generated catch block e.printStackTrace(); }
                 */
                /*
                 * error = new SystemNativeError(); error.setErrno(100);
                 * error.setMessage("More than two-way splitting is not supported: Requested type:"+requestEntryType+":"
                 * +requestUnits);
                 */
            }
            break;
    ...
    private boolean checkResourceAvailability(String max_domain, RequestReservation rr,
    ...
    protected RequestReservation generateVCConnection(RequestReservation rr, String rType, int x1, int x2,
        ...
        request.setRequest(requestModel, connection, rr.getTerm(), rr.getReservationDomain(), rr.getReservation(),
                rr.getReservation_rs());

        logger.info("InterCloud VC connection:dType=" + rType + "--" + x1 + ":" + x2 + ";" + connectionName + ":"
                + master_str + ":" + slave_str + ":" + max_domain + ":" + second_domain);
