...
public class ManifestParserListener implements INdlManifestModelListener, INdlRequestModelListener {
    ...
    @Override
    public void ndlNode(Resource ce, OntModel m, Resource ceClass, List<Resource> interfaces) {
            ...
            if (ce_rs.hasProperty(NdlCommons.manifestHasParent)) {
                ...
                for (StmtIterator sti = ce_rs.listProperties(NdlCommons.manifestHasParent); sti.hasNext();) {
                    ...
                    if (parent_rs.getProperty(NdlCommons.collectionElementProperty) == null) {
                        logger.error(
                                "Parent element missed: ce_rs=" + ce_rs.getURI() + ";parent=" + parent_rs.getURI());
                        continue;
                    }
                    ...
                    if (parent != null && interface_rs != null) {
                        ...
                        if (ce.getCe() != null) {
                            if (parent_domain_rs.hasProperty(NdlCommons.inRequestNetworkConnection))
                                nc_rs = parent_domain_rs.getProperty(NdlCommons.inRequestNetworkConnection)
                                        .getResource();
                            else
                                nc_rs = parent_domain_rs;
                            NetworkConnection nc = new NetworkConnection(parent.getModel(), nc_rs.getURI(),
                                    nc_rs.getURI());
                            ...
                            if (storageSharedLink(parent_domain_rs)) {
                                if (parent_domain_rs.hasProperty(NdlCommons.inDomainProperty)) {
                                    String inDomainName = parent_domain_rs.getProperty(NdlCommons.inDomainProperty)
                                            .getResource().getURI();
                                    ...
                                    if (interface_rs.getProperty(NdlCommons.ip4LocalIPAddressProperty) != null) {
                                        ip_rs = interface_rs.getProperty(NdlCommons.ip4LocalIPAddressProperty)
                                                .getResource();
                        ...
                        logger.debug("recover:dependency:parent=" + parent.getURI() + ";child=" + ce.getURI() + ";intf="
                                + interface_rs.getURI());
                        if (nc_rs != null)
                            logger.debug(";ncByInterface=" + nc_rs.getURI());
                    } else
                        logger.error("recover:dependency:something missed:parent=" + parent_domain_rs + ";intf="
                                + interface_rs);
                }
            } else {
                ...
                if (domain_rs.hasProperty(NdlCommons.collectionItemProperty)) {
                    ...
                    for (Statement st : st_list) {
                        ...
                        if (rc.hasProperty(NdlCommons.domainHasResourceTypeProperty)) {
                            if (rc.getProperty(NdlCommons.domainHasResourceTypeProperty).getResource().getURI()
                                    .endsWith("LUN")) {
                                isStorage = true;
                                break;
                            }
    ...
    private void setCEProperty(ComputeElement c_e, Resource ce_rs, Resource ceClass) {
        ...
        if (ce_rs.hasProperty(NdlCommons.domainHasResourceTypeProperty))
            rType = ce_rs.getProperty(NdlCommons.domainHasResourceTypeProperty).getResource().getLocalName()
                    .toLowerCase();
    ...
    @Override
    public void ndlCrossConnect(Resource c_rs, OntModel m, long bw, String label, List<Resource> interfaces,
            Resource parent) {
        // ignore request items
        if (requestPhase)
            return;
        ...
        if (c_rs.hasProperty(NdlCommons.domainHasResourceTypeProperty))
            rType = c_rs.getProperty(NdlCommons.domainHasResourceTypeProperty).getResource().getLocalName()
                    .toLowerCase();
        DomainResourceType dType = new DomainResourceType(rType, 1);
        String domainName = null;
    ...
    public void ndlSlice(Resource sl, OntModel m) {
        ...
        if (sl.hasProperty(NdlCommons.RDF_TYPE, NdlCommons.ofSliceClass)) {
            ...
            if ((currentSlice.getOfUserEmail() == null) || (currentSlice.getOfSlicePass() == null)
