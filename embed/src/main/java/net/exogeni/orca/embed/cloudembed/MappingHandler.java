...
import net.exogeni.orca.embed.workflow.IRequestEmbedder;
...
public class MappingHandler implements IRequestEmbedder {
    ...
    @NotPersistent
    protected HashMap<String, NetworkConnection> requestMap = new HashMap<String, NetworkConnection>(); // bookkeeping
                                                                                                        // of the
                                                                                                        // provisioned
                                                                                                        // connections
                                                                                                        // for releasing
    ...
    public int findIPRangeHole(HashMap<String, IPAddressRange> group_base_ip, String ip_str)
        ...
        logger.debug("findIPRangeHole:group base ip=" + group_base_ip.get(network_str) + ";network_str=" + network_str);
    ...
    public HashMap<String, IPAddressRange> getIPRange(DomainElement firstElement, Collection<DomainElement> cde)
            throws Exception {
        ...
        for (Interface intf : firstElement.getClientInterface()) {
            ...
            if (ip == null)
                ...
                IPAddressRange ip_range = new IPAddressRange(base_IPAddr.address, base_IPAddr.netmask,
                        ip.getResource(intf.getModel()));
            ...
            if (bSet != null) {
                if (p_entry.getKey().getFollowedBy() != null) {
                    for (Entry<DomainElement, OntResource> c_entry : p_entry.getKey().getFollowedBySet()) {
                        ...
                        logger.debug("findIPRange:follower intf=" + intf_ont.getURI() + ";ip="
                                + intf_ont.getProperty(NdlCommons.layerLabelIdProperty));
                        if (intf_ont.getProperty(NdlCommons.layerLabelIdProperty) != null) {
                            ip_str = intf_ont.getProperty(NdlCommons.layerLabelIdProperty).getString();
