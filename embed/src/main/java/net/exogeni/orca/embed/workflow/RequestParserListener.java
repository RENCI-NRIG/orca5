...
/**
 * Listener for NDL request parser. It gets objects in this order: - Reservation and its details - Nodes - Node
 * dependencies - Network Connections - Interfaces - All reservation resources (including nodes and network connections
 * again)
 * 
...
public class RequestParserListener implements INdlRequestModelListener {
    ...
    public void ndlReservationEnd(Literal e, OntModel m, Date end) {
        ...
        if (end.before(term.getStart())) {
            try {
                throw new NdlException("Term End time " + end + " should be after beginning time: " + term.getStart());
            } catch (NdlException e1) {
                err = new SystemNativeError();
                err.setErrno(-1);
                err.setMessage("Term End time " + end + " should be after beginning time: " + term.toString());
    ...
    public void ndlNetworkConnection(Resource l, OntModel om, long bandwidth, long latency, List<Resource> interfaces) {
        ...
        if ((interfaces.size() <= 2)
                && !(l.hasProperty(NdlCommons.RDF_TYPE, NdlCommons.topologyBroadcastConnectionClass))) {
            // point-to-point link
            // the ends
            ...
            if (interfaces.size() == 2) {
                ...
                if ((if2Node.getInDomain() != null)
                        && (if2Node.getInDomain().contains(NdlCommons.stitching_domain_str))) {
                    start_if = if2;
                    if2 = if1;
                    if1 = start_if;
                }
    ...
    public NetworkElement createNode(Resource ce, OntModel om, Resource ceClass, List<Resource> interfaces) {
        if (ce == null)
            return null;
        NetworkElement newNode;
        if ((ceClass.equals(NdlCommons.serverCloudClass)) || (ceClass.equals(NdlCommons.computeElementClass))
                || (ceClass.equals(NdlCommons.deviceOntClass)) || (ceClass.equals(NdlCommons.networkStorageClass))) {
