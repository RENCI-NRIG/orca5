...
/**
 * This object implements the generic workflow of turning a request into Orca reservations.
 * 
 * @author ibaldin
 *
 */
public class RequestWorkflow {
    ...
    public synchronized SystemNativeError run(DomainResourcePools domainResourcePools, List<String> abstractModels,
        ...
        NdlRequestParser nrp = new NdlRequestParser(resReq, parserListener, NdlModel.ModelType.TdbPersistent,
                Globals.TdbPersistentDirectory + Globals.PathSep + "controller" + Globals.PathSep + "request-"
                        + sliceId);
        nrp.processRequest();
        ...
        if (!bound && RequestReservation.MultiPoint_Domain
        ...
        manifestModel = ((CloudHandler) embedderAlgorithm).createManifest(boundElements, request, userDN,
                controller_url, sliceId);
    ...
    public void staticLabelDependency() {
        ...
        // make network reservations without static tag (local control picks) depending on reservations w/ static tag to
        // avoid tag mismatch
        for (Entry<String, LinkedList<NetworkElement>> entry : domainCount.entrySet()) {
            ...
            if (elements.size() > 1 && (de.getCe() == null
                    || (de.getCastType() != null && de.getCastType().equalsIgnoreCase(NdlCommons.multicast)))) { // network
                                                                                                                 // domain
                                                                                                                 // with
                                                                                                                 // >1
                                                                                                                 // reservations
                ...
                for (DomainElement fde : free_elements) {
                    for (DomainElement sde : static_elements) {
                        ...
                        logger.debug("RequestWorkflow::run():adding static tag dependency for network reservations: domain="
    ...
    public synchronized void modify(DomainResourcePools domainResourcePools, String modReq, String sliceId,
        ...
        err = ((MappingHandler) embedderAlgorithm).modifySlice(domainResourcePools, modifyElements, manifestModel,
                sliceId, nodeGroupMap, firstGroupElement, requestModel, tmpModifyModel);
    ...
    public void clearGlobalControllerAssignedLabel() {
        ...
        for (Entry<String, BitSet> entry : controllerAssignedLabel.entrySet()) {
            domain = entry.getKey();
            bitSet = entry.getValue();
            logger.debug("RequestWorkflow::modifyGlobalControllerAssignedLabel():ClearGlobalLabel:" + domain + ";controller bitset=" + bitSet);
            if (globalControllerAssignedLabel.containsKey(domain)) {
                globalBitSet = globalControllerAssignedLabel.get(domain);
                globalBitSet.andNot(bitSet);
                logger.debug("RequestWorkflow::modifyGlobalControllerAssignedLabel():ClearGlobalLabel:assignedLabel=" + globalControllerAssignedLabel.get(domain)
    ...
    public void recover(Logger logger, WorkflowRecoverySetter sliceSetter, OntModel reqModel, OntModel manModel) {
        ...
        try {
            ...
            parserListener.parse(manifestModel); // no need to close here - we need to keep manifest model open until
                                                 // the slice goes away
    ...
    public LinkedList<OntResource> recoverDomainInConnectionList() {
        ...
        logger.info("RequestWorkflow::recoverDomain():Start to recover domainInConnectionList...");
        for (NetworkElement ne : boundElements) {
            ne_ont = ne.getResource();
            if (ne_ont == null) {
                logger.error("RequestWorkflow::recoverDomain():No ontResource, ne=" + ne.getName());
                continue;
            }
            domainList.add(ne_ont);
            logger.debug("RequestWorkflow::recoverDomain():domain:" + ne_ont.getURI() + ";");
            /*
             ...
             * domainList.add(domain_ont); logger.debug("RequestWorkflow::domain:"+domain_ont.getURI()+";"); }else{
             * logger.error("RequestWorkflow::No inDomain property:ne="+ne_ont.getURI()); }
             */
        }
        return domainList;
    }
