...
public class ReservationConverter implements LayerConstant {
    ...
    public List<IServiceManagerReservation> getReservations(InterDomainHandler handler,
            HashMap<String, ResourceType> typesMap, String sshKey, Term term, IServiceManager sm, ISlice slice) {
        ...
        Map<String, ArrayList<String>> scriptMap = new HashMap<String, ArrayList<String>>(); // maps group name to a
                                                                                             // list of ips
        ...
        try {
            ...
            for (Device d : list) {
                ...
                logger.debug("getReservations(): d.getResourceType() : " + d.getResourceType().getResourceType()
                        + " count " + d.getResourceType().getCount());
                ...
                IServiceManagerReservation r = (IServiceManagerReservation) ServiceManagerReservationFactory
                        .getInstance().create(rset, term, slice, brokerProxy);
                ...
                if (request.isNetwork) {
                    ...
                    if (actions != null) {
                        ...
                        /*
                         * int count = 0; for (Interface iff : ifs) { if (iff.getName() != null) {
                         * r.setConfigurationProperty("config.interface." + (count+1), iff.getName());
                         * System.out.println("domain=" + domain + " setting property config.interface." + (count+1) +
                         * "=" + iff.getName()); count++; } }
                         */
                        ...
                        if (r.getResources() != null)
                            if (r.getResources().getConfigurationProperties() != null)
                                logger.debug("config.interface.ports = " + r.getResources().getConfigurationProperties()
                                        .getProperty("config.interface.ports") + "\n");
        ...
        for (Device d : list) {
            ...
            logger.debug(
                    "from " + r.domain + " units=" + r.reservation.getUnits() + " type=" + r.reservation.getType());
            ...
            for (Entry<Device, Resource> parent : d.getPrecededBySet()) {
                ...
                if (r.isNetwork) {
                    ...
                    // throw new RuntimeException("The current version of the controller only supports one dependency
                    // between non network and network resource");
                    // }
                }
                ...
                if (r.isNetwork) {
                    ...
                    if (intf_name != null) {
                        ...
                        if (pr.isNetwork) {
                            ...
                            DatatypeProperty hostInterfaceName = handler.getIdm().createDatatypeProperty(
                            ...
                            if (site_host_interface == null) {
                                logger.debug(
                                        "Host Interface Definition not here: IP address is used as the parent value or its neighbors are network domains!!");
                                if (parent.getKey().getDownNeighbour() != null) {
                                    if (parent.getKey().getDownNeighbour().getProperty(hostInterfaceName) != null) {
                                        site_host_interface = parent.getKey().getDownNeighbour()
                                                .getProperty(hostInterfaceName).getString();
                                    } else {
                                        if (parent.getKey().getUpNeighbour() != null) {
                                            if (parent.getKey().getUpNeighbour()
                                                    .getProperty(hostInterfaceName) != null) {
                                                site_host_interface = parent.getKey().getUpNeighbour()
                                                        .getProperty(hostInterfaceName).getString();
                                            }
                                        }
                                    }
                            ...
                            if (index > 0) {
                                ...
                                host_interface = String
                                        .valueOf(Integer.valueOf(intf_name.substring(index + 1)).intValue() + 1);
                                parent_ip_addr = parent_ip_addr.concat(host_interface).concat(".ip");
                                r.reservation.setConfigurationProperty("unit.eth" + host_interface + ".hosteth",
                                        site_host_interface);
                                ...
                                ip_addr = intf_name;
                                parent_ip_addr = parent_ip_addr.concat(String.valueOf(r.networkDependencies))
                                        .concat(".ip");
                                if (d.getPrecededBySet().size() >= 1) {
                                    r.reservation.setConfigurationProperty(
                                            "unit.eth" + String.valueOf(r.networkDependencies) + ".hosteth",
                                            site_host_interface);
                                    parent_tag_name = parent_tag_name.concat(String.valueOf(r.networkDependencies))
                                            .concat(".vlan.tag");
                                }
                            ...
                            try {
                                ...
                                if (d.getIPAddress() != null) {
                                    ...
                                    InetAddress addr1 = InetAddress.getByName(ip_addr.split("/")[0]); // this is only
                                                                                                      // for throwing an
                                                                                                      // exception for a
                                                                                                      // mal-formated IP
                                                                                                      // address.
    ...
    public String getManifest(InterCloudHandler handler, IReservation[] allRes, ISubstrateDatabase db) {
        ...
        Individual manifest = manifestModel.createIndividual(reservation.getNameSpace() + "manifest",
                handler.mapper.manifestOntClass);

        Resource intf_start, intf_next, intf_next_next;
        ExtendedIterator<Individual> connectionIterator = manifestModel
                .listIndividuals(handler.mapper.networkConnectionOntClass);
        ...
        if (handler.cloudRequest != true) {
            while (domainConnectionListIt.hasNext()) {
                ...
                logger.debug("Connection name from handler domain list:" + connectionName + " ;num hops="
                        + domainList.size());
                if (networkConnectionList.size() == 0) { // likely slitted request case
                    networkConnection = manifestModel.createIndividual(connectionName,
                            handler.mapper.networkConnectionOntClass);
                    networkConnectionList.add(networkConnection);
                }
                for (int j = 0; j < networkConnectionList.size(); j++) {
                    networkConnection = networkConnectionList.get(j);
                    logger.debug("Connection name from request rdf:" + networkConnection.getURI() + "; resource="
                            + networkConnection);
                    if (connectionName.equals(networkConnection.getURI())) {
                        ...
                        if ((split == true) && (connectionType == null))
                            connectionType = "InterDomainRequest";
                        if (connectionType.equals("InterDomainRequest")) {
                            for (int i = 1; i < domainList.size(); i++) {
                                next_Hop = domainList.get(i);
                                logger.debug(
                                        "link connections..." + i + ":" + domainList.size() + ":" + next_Hop.getURI());
                                // source domain
                                if (i == 1) {
                                    link_url = start.getURI();
                                    link_ont = manifestModel.createIndividual(link_url,
                                            handler.mapper.computeElementOntClass);
                                ...
                                link_ont = manifestModel.createIndividual(link_url,
                                        handler.mapper.linkConnectionOntClass);
                                ...
                                if (i < domainList.size() - 1) {
                                    ...
                                    link_ont = manifestModel.createIndividual(link_url,
                                            handler.mapper.crossConnectOntClass);
                                ...
                                if (i == domainList.size() - 1) {
                                    ...
                                    start.getDownNeighbour().addProperty(handler.mapper.RDF_TYPE,
                                            handler.mapper.interfaceOntClass);
                        ...
                        if (connectionType.equals("CloudRequest")) {
                            ...
                            for (int i = 0; i < domainList.size(); i++) {
                                ...
                                logger.debug("create individual url:" + next_Hop.getURI() + " :Name="
                                        + next_Hop.getName() + " :Individual=" + link_ont);
                            }
                        }
            ...
            while (domainConnectionListIt.hasNext()) {
                ...
                for (int i = 0; i < domainList.size(); i++) {
                    ...
                    // reservation.addProperty(handler.mapper.item, link_ont);
                    // manifest.addProperty(handler.mapper.element, link_ont);
                    logger.debug("create individual url:" + next_Hop.getURI() + " :Name=" + next_Hop.getName()
                            + " :Individual=" + link_ont);
                }
            }
        }
        ...
        for (int i = 0; i < networkConnectionList.size(); i++) {
            ...
            for (int j = 0; j < allRes.length; j++) {
                ...
                logger.debug("getManifest: domain=" + domain + " ;aDomian = " + aDomain + " :rDomain=" + rDomain
                        + " rType=" + rType);
                ...
                if ((aDomain.equalsIgnoreCase(rDomain)) && (domain.endsWith(rType))) {
                    try {
                        ...
                        Resource reservationState = manifestModel.createIndividual(
                                handler.mapper.ORCA_NS + "request.owl#" + state,
                                handler.mapper.reservationStateOntClass);
                                ...
                                for (Properties p : v) {
                                    if (rType.equalsIgnoreCase("vlan")) {
                                        ...
                                        if ((handler.interDomainRequest == false) && (handler.cloudRequest == true)) {
                                            vlan_ont = manifestModel.createIndividual(vlan_url,
                                                    handler.mapper.linkConnectionOntClass);
                                        ...
                                        if (vlan_ont.hasProperty(handler.mapper.hasURL)) {
                                            if (vlan_ont.getProperty(handler.mapper.hasURL).getString()
                                                    .equals(vlan_url)) {
                                                if (p.getProperty("unit.vlan.tag") != null) {
                                                    vlan_ont.addProperty(handler.mapper.RDFS_Label,
                                                            p.getProperty("unit.vlan.tag"));
                                                } else {
                                                    logger.error("unit.vlan.tag is null");
                                                }
                                                if (p.getProperty("unit.vlan.qos.rate") != null) {
                                                    vlan_ont.addProperty(handler.mapper.bandwidth,
                                                            p.getProperty("unit.vlan.qos.rate"));
                                                } else {
                                                    logger.error("unit.vlan.qos.rate is null");
                                                }
                                                vlan_ont.addProperty(handler.mapper.message, notice);
                                                vlan_ont.addProperty(handler.mapper.hasReservationState,
                                                        reservationState);
                                                vlanReservationList.put(vlan_url, vlan_ont);
                                            }
                                        }
                                    ...
                                    if (rType.equalsIgnoreCase("vm")) {
                                        ...
                                        Individual vm_ont = manifestModel.createIndividual(vm_url,
                                                handler.mapper.vmOntClass);
                                        /*
                                         * if( (handler.interDomainRequest==false) && (handler.cloudRequest == true) ){
                                        ...
                                        Individual service_ont = manifestModel.createIndividual(vm_url + "/Service",
                                                handler.mapper.serviceOntClass);
                                        ...
                                        Individual vm_IP = manifestModel.createIndividual(vm_url + "/IP",
                                                handler.mapper.IPAddressOntClass);
                                        ...
                                        if (unitManagementIP != null) {
                                            ...
                                            if ((unitManagementPort == null) || (unitManagementPort.equals("22"))) { // No
                                                                                                                     // DNAT
                                        ...
                                        if (p.getProperty("unit.instance.config") != null) {
                                            vm_ont.addProperty(handler.mapper.postBootScript,
                                                    p.getProperty("unit.instance.config"));
                                        } else {
                                    ...
                                    if (logger.isDebugEnabled()) {
                                        for (Enumeration<?> e = p.propertyNames(); e.hasMoreElements();) {
                                            ...
                                            logger.debug("{Property key: " + key + " | Property value: "
                                                    + p.getProperty(key) + "}");
                                        }
                                    }
                                }
        ...
        try {
            for (Device d : domainList) {
                logger.debug("d in deviceList:" + d.getName() + " url=" + d.getURI() + " resourceType="
                        + d.getResourceType().getResourceType().toString());
                        ...
                        if (d.getName().equalsIgnoreCase(entry.getKey())) {
                            ...
                            if (d.getPostBootScript() != null) {
                                if (entry.getValue().hasProperty(handler.mapper.postBootScript)) {
                                    entry.getValue().setPropertyValue(handler.mapper.postBootScript,
                                            manifestModel.createLiteral(d.getPostBootScript()));
                                } else {
                                    /*
                                     * try {
                                     * NdlGenerator.addTypedProperty(entry.getValue(),"request-schema","postBootScript",
                                     * d.getPostBootScript(),XSDDatatype.XSDstring); } catch (NdlException e) { // TODO
                            ...
                            /*
                             * if(d.getIPAddress()!=null){ Individual vm_IP = manifestModel.createIndividual(d.getName()
                             * + "/IP", handler.mapper.IPAddressOntClass); vm_IP.addProperty(handler.mapper.label_ID,
                             * d.getIPAddress()); if(d.getIPNetmask()!=null){
                             ...
                             * entry.getValue().setPropertyValue(NdlCommons.ip4LocalIPAddressProperty,vm_IP); }else{
                            ...
                            if (d.getPrecededBySet() != null) {
                                ...
                                for (Entry<Device, Resource> parent : d.getPrecededBySet()) {
                                    ...
                                    for (Entry<String, Individual> vlanentry : vlanReservationList.entrySet()) {
                                        ...
                                        logger.debug("Parent:" + parentDevice.getName() + " ;value=" + parent.getValue()
                                                + "----vlanentry: key=" + vlanDeviceName + " ;value="
                                                + vlanentry.getValue());
                                        if (vlanDeviceName.equals(parentDevice.getName())) {
                                            Individual vm_IP = manifestModel.createIndividual(
                                                    d.getName() + "/IP/" + String.valueOf(i),
                                                    handler.mapper.IPAddressOntClass);
                                            vm_IP.addProperty(handler.mapper.label_ID,
                                                    parent.getValue().getURI().split("/")[0]);
                                            if ((handler.interDomainRequest == false)
                                                    && (handler.cloudRequest == true)) {
    ...
    public static String getHadoopScript(String ip, String hostsScript, String topologyScript, String type) {
        ...
        String script = line.concat(line0).concat(line1).concat(line2).concat(line3).concat(line4).concat(line5)
                .concat(line6);
        return script;
    }
    ...
    public static String getHadoopTopologyScript(String topology, int topologyFlag) {
        ...
        String line9 = "\t echo \"        print join([RACK_MAP.get(i, DEFAULT_RACK) for i in sys.argv[1:]]," + '"' + " "
                + '"' + ") \" >> /home/hadoop/topology.py \n";

        script = script.concat(line1).concat(line2).concat(line3).concat(line4).concat(line5).concat(line6)
                .concat(line7).concat(line8).concat(line9);
