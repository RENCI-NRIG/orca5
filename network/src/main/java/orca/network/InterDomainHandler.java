...
public class InterDomainHandler extends MappingHandler implements LayerConstant {
     ...
     * 
     * Domain domain = new Domain(inputFile,processor,importFlag); OntModel model=domain.abstractDomain();
    ...
    public Hashtable<String, LinkedList<Device>> handleRequest(InputStream requestStream)
            throws IOException, InetNetworkException {
        mapper.setOntModel(idm);
        return handleRequest(mapper.ontCreate(requestStream));
    }

    public Hashtable<String, LinkedList<Device>> handleRequest(String requestFile)
            throws IOException, InetNetworkException {
        mapper.setOntModel(idm);
        return handleRequest(mapper.ontCreate(requestFile));
    }

    public Hashtable<String, LinkedList<Device>> handleRequest(OntModel model)
    ...
    public Hashtable<String, LinkedList<Device>> handleRequest(Hashtable<String, NetworkConnection> requestMap)
            throws InetNetworkException, IOException {
        ...
        for (Entry<String, NetworkConnection> entry : requestMap.entrySet()) {
            ...
            if ((sourceDomain == null) || (destDomain == null)) {
                ...
                if (sourceDomain == destDomain) {
                    ...
                    ArrayList<ArrayList<OntResource>> path = mapper.findShortestPath(idm, sourceDomain, destDomain, bw,
                            resourceType);
            ...
            // LinkedList <Device> domainList=domainConnection(deviceList,bw); //Return list of domain device with
            // connections interfaces in the action.
            ...
            logger.info("Source =" + source.getResource() + " imageURL=" + source.getVMImageURL() + " imageGUID="
                    + source.getVMImageGUID() + " postBootScript=" + source.getPostBootScript());
            ...
            logger.info("Destination =" + source.getResource() + " imageURL=" + source.getVMImageURL() + " imageGUID="
                    + source.getVMImageGUID() + " postBootScript=" + source.getPostBootScript());
    ...
    public NetworkConnection VCHandler(String domain, Hashtable<String, NetworkConnection> requestMap,
            boolean isConnection) throws IOException {
    ...
    public Resource toDomain(Resource rs, String resourceType) {
        ...
        boolean found = false;
        for (ResIterator j = idm.listResourcesWithProperty(mapper.RDF_TYPE, mapper.networkDomainOntClass); j
                .hasNext();) {
            ...
            if ((rDomain.getLocalName().equalsIgnoreCase("Testbed"))
    ...
    public OntModel domainRequest(Device domain) {
        ...
        if (reservation != null) {
            ...
            connection_rs = domainRequestModel.createIndividual(reservation.getURI() + "/conn",
                    mapper.networkConnectionOntClass);
            if (up_device_rs != null)
                connection_rs.addProperty(mapper.hasInterface, up_device_rs);
            if (down_device_rs != null)
                connection_rs.addProperty(mapper.hasInterface, down_device_rs);
            if (rType != null) {
                connection_rs.addProperty(mapper.hasResourceType,
                        domainRequestModel.createResource(rType.getResourceType()));
        ...
        /*
         * if(reservation!=null){ if(reservation.getProperty(mapper.startingTime)!=null) startingTime =
         * DatatypeConverter.parseDateTime(reservation.getProperty(mapper.startingTime).getString()).getTime();
         * if(reservation.getProperty(mapper.endingTime)!=null) endingTime =
         * DatatypeConverter.parseDateTime(reservation.getProperty(mapper.endingTime).getString()).getTime(); }
         */
    ...
    public void domainNoDepend(Device domain) {
        ...
        if (action != null) {
            for (Interface intf : action.getSwitchingInterface()) {
                ...
                if (intf_rs.getProperty(mapper.vlan) != null)
                    vlan_rs = intf_rs.getProperty(mapper.vlan).getResource();
    ...
    public Device domainDepend(LinkedList<Device> domainList, LinkedList<Device> dependList, Resource rs1_ip,
            Resource rs2_ip) {
    ...
    public Device domainDepend(Device start, Device next, LinkedList<Device> dependList, int hop, int path_len,
            Resource rs1_ip, Resource rs2_ip) {
        LabelSet sSet, nSet;
        ...
        // System.out.println(start.getName() + ":"+ start.isLabelProducer() +":" + sRType + ":"+ sRank + ":" +
        // start.getSwappingCapability());
        // System.out.println(next.getName() + ":"+ next.isLabelProducer() +":" +nRType + ":"+ nRank + ":" +
        // next.getSwappingCapability()+"\n");
                ...
                if (sRank == nRank) {
                    if ((start.getTunnelingCapability() != null) || (next.getTunnelingCapability() != null)) {
                        ...
                        if (start.getSwappingCapability() != null) {
                            ...
                            logger.info("LabelSet Size:" + start.getName() + ":" + sSetSize + ":"
                                    + sSet.getMinLabel_ID() + "-" + sSet.getMaxLabe_ID() + "****" + next.getName() + ":"
                                    + nSetSize + ":" + nSet.getMinLabel_ID() + "-" + nSet.getMaxLabe_ID());
                            if (sSetSize == 0) {
                                if (nSetSize > 0) {
        ...
        if ((device != null)
                && (device.getResourceType().getResourceType() == "http://geni-orca.renci.org/owl/compute.owl#VM")) {
            start = device;
        } else {
            if (hop == 1)
                dependList.add(start);
        ...
        if (flag) {// start depends on next
            if ((hop == 1) && (rs1_ip != null)) { // First device should be the VM and check if IP is given from the
                                                  // request
            ...
            if ((hop == path_len - 1) && (rs2_ip != null)) { // Last device should be the VM and check if IP is given
                                                             // from the request
