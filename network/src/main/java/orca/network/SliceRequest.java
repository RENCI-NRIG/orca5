...
public class SliceRequest extends OntProcessor {
    ...
    public Hashtable<String, NetworkConnection> parseRequestSite(OntModel model)
        ...
        String rs0_str = null, rs1_str = null, rs3_str = null, rs4_str = null, rs5_str = null, rs6_str = null,
                rs7_str = null, rs8_str = null, rs10_str = null, rs11_str = null;
        ...
        while (results.hasNext()) {
            ...
            /*
             * if(units==0){ rType = requestResourceType(rs1); units=rType.getCount(); } else{
             * rType=getResourceType(rs2.getURI()); rType.setCount(units); }
             */
    ...
    public Hashtable<String, NetworkConnection> parseRequest(OntModel model)
            throws UnknownHostException, InetNetworkException {
        requestModel = model;
        ...
        String rs0_str = null, rs1_str = null, rs3_str = null, rs4_str = null, rs5_str = null, rs6_str = null,
                rs7_str = null, rs8_str = null, rs9_str = null;
        String connectionName;
        ...
        while (results.hasNext()) {
            ...
            if (existEdge(visitedConnection, rs0_str, rs1_str))
                continue;
            ...
            if (connection.getEndPoint1_type() != connection.getEndPoint2_type()) {
                logger.error("Requested resource type are not the same at both ends:" + connection.getEndPoint1_type()
                        + ":" + connection.getEndPoint2_type());
                throw new RuntimeException("Requested resource type are not the same at both ends:"
                        + connection.getEndPoint1_type() + ":" + connection.getEndPoint2_type());
            }

            if (rs3_str != null)
                connection.setBw(Long.valueOf(rs3_str).longValue());

            // default connection type is VLAN
            connection.setType("http://geni-orca.renci.org/owl/domain.owl#VLAN");

            logger.info("Request connection:" + connectionName + ":" + rs0 + "(" + rs4_str + ")-" + rs1 + "(" + rs6_str
                    + "):" + rs3_str + ":" + connection.getEndPoint1_type() + ":" + connection.getResourceCount1() + "-"
                    + connection.getEndPoint2_type() + ":" + connection.getResourceCount2());
    ...
    public void init() {
        ...
        if (reservation != null) {
            ...
            logger.info(reservation + ":" + domain + ":" + layer + ":Term:" + startTime + ":" + endTime + ":"
                    + termDuration + ":default bandwidth:" + defaultBandwidth);
        }
    }
    ...
    public String setRequest(String p, boolean dataProperty) {
        ...
        if (results.hasNext()) {
            if (dataProperty)
                ...
                ob = results.nextSolution().getResource(varName).toString();
        }
    ...
    public Resource getRequest() {
        ...
        while (results.hasNext()) {
            ...
            termDuration = (long) ((((years * 365 + months * 30 + weeks * 7 + days) * 24 + hours) * 60 + minutes) * 60
                    + seconds);
        }
        if (reservation == null) {
            reservation = requestModel.createResource("http://geni-orca.renci.org/owl/" + UUID.randomUUID().toString(),
                    (Resource) requestModel.createOntResource(NdlCommons.ORCA_NS + "request.owl#Reservation"));
        }
    ...
    public String getVMImageURL(Resource rs) {
        ...
        if (rs != null) {
            // ObjectProperty hasVMImage =
            // requestModel.createObjectProperty(NdlCommons.ORCA_NS+"compute.owl#diskImage");
            if (rs.hasProperty(diskImage))
                vmImage = rs.getRequiredProperty(diskImage).getResource();
            if (vmImage != null) {
                // DatatypeProperty
                // hasURL=requestModel.createDatatypeProperty(NdlCommons.ORCA_NS+"topology.owl#hasURL");
    ...
    public String getVMImageGUID(Resource rs) {
        ...
        if (rs != null) {
            ...
            if (vmImage != null) {
                // DatatypeProperty
                // hasGUID=requestModel.createDatatypeProperty(NdlCommons.ORCA_NS+"topology.owl#hasGUID");
    ...
    public int sdDevice(OntModel model, String url) {
        ...
        if (!results.hasNext()) {
            ...
            if (!results.hasNext())
                gress = -1; // destination: connectedTo nobody
        }
        return gress;
    }
    ...
    public int deviceConnectedDetail(String url1, String url2) throws OntologyException {
        ...
        DatatypeProperty visited = ontModel
                .createDatatypeProperty("http://geni-orca.renci.org/owl/topology.owl#visited");
