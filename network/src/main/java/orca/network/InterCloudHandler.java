...
public class InterCloudHandler extends InterDomainHandler {
	...
	public Hashtable <String,LinkedList <Device> > handleCloudRequest(String requestFile) throws IOException, InetNetworkException{
	...
	public Hashtable <String,LinkedList <Device> > handleCloudRequest(InputStream requestStream) throws IOException, InetNetworkException{
	...
	public Hashtable <String,LinkedList <Device> > handleCloudRequest(OntModel requestModel) throws IOException, UnknownHostException, InetNetworkException{	
		...
		if(requestMap.size()!=0){
			...
			if(requestBounded){ //Domain is specified for each node of the requestConnection
				...
				logger.debug(connection.getEndPoint1_ip()+":"+connection.getResourceCount1()+":"+connection.getEndPoint2_ip()+":"+connection.getResourceCount2());
				if( ((connection.getEndPoint1_ip()!=null) && (connection.getResourceCount1()>=1)) || ((connection.getEndPoint2_ip()!=null) && (connection.getResourceCount2()>=1))) {
				...
				if(mapper.domain==null){ //
					if(numVMSite<=deltaVM){  //but the number of nodes in VT is too small, it is assigned to the maxDomain 
				...
				if(!cloudRequest){
					...
					OntModel cloudModel=ModelFactory.createOntologyModel(OntModelSpec.OWL_DL_MEM,buildCloudOntModel());
			...
			if(mapper.domain!=null){
				...
				for(Entry <String,NetworkConnection > entry:requestMap.entrySet()){
					...
					//generate a separate connection request for each individual VM, so each of them can be an individual reservation
					if(IPAddress!=null){ 
						for(int i=0;i<numVM;i++){
                                                        //String group = requestConnection.getEndPoint1().substring(requestConnection.getEndPoint1().indexOf("#")+1);
                                                        String group = cluster.getGroup();
                                                        logger.debug("############## group: " + group +  ", IPAddress: " + IPAddress);
							newRequestConnection = generateRequestConnection(requestConnection,cluster.getVMImageURL(),cluster.getVMImageGUID(),IPAddress,cluster.getIPNetmask(),i,0,false, group, null,cluster.getName());
	    		...
	    		if(!typeTable.containsKey(requestEndType)){
	    			logger.error("No Available Resource found for the requested edge resource:"+requestEndType+":"+rType.toString());
	    		}
	    		
		    	if(numVMSite<=deltaVM){  //but the number of nodes in VT is too small, 	
					mapper.domain=maxVMDomain;	//assign the request to the domain with most available VMs
					//mapper.domain=secondMaxVMDomain;
					logger.info("Unbounded request to be assigned to the cloud:"+mapper.domain+":"+numVM+":"+deltaVM+"\n");
		    		...
		    		if(numVMSite>typeTable.get(requestEndType)){
		    			logger.error("Number of requested VM is greater than the total amount of available VM resource in the system!\n");
		    			return null;
		    		}
		    		else{
		...
		if(cloudRequest){
			if(mapper.domain!=null){	//embedding within one domain
				...
				logger.info("Embedding a VC in the domain:"+this.domain+":"+connection.getName()+":"+connection.getConnection().size()+":"+connection.getDevice1()+"-"+connection.getDevice2()+" BW="+connection.getBw());
	...
	public void copyDomainList(Hashtable <String,LinkedList<Device>> domainConnectionList,Hashtable <String,LinkedList<Device>> connectionList){
	...
	public void generateIDVCConnection(NetworkConnection connection,Hashtable <String,LinkedList <Device> > connectionList,boolean isVT) throws InetNetworkException, IOException{
					...
					for(Device dMaster:dList){
						if(dMaster.getName().equals(d.getNodeDependency().getURI())){
							if(d.getIPAddress()!=null){
								...
								logger.info("Added InteDdomainHandler node dependency: d="+d.getName()+" ;dMaster="+dMaster.getName());
							}
						}
					}
				}
		...
		for(Entry <String,LinkedList <Device> > entry:connectionList.entrySet()){
			...
			for(Device d:dList){
				...
				if(dType.getResourceType().endsWith("VM")){ //two end domains
					if(d.getIPAddress()!=null){  //generate new request with IP address for multi unit request at the end
						logger.info("generateIDVCConnection:"+d.getName()+":"+d.getResourceType().toString()+":"+d.getIPAddress()+":"+d.getIPNetmask()+":"+d.getUpNeighbour());
						...
						//node group dependency, implies all the VMs reservation will share the same vlan, which implies it's a VC request within a domain 
						if(d.getNodeDependency()!=null){
							logger.info("Implies a VC request with node group dependency!:"+d.getNodeDependency().getURI());
							vcRequest=true;
						}
					if(d.getResourceType().getCount()>1){
						...
						if(vmPosition==0){
							...
							if(vcRequest==true){
								...
								logger.info("Parent Common Link Device:"+commonLinkDevice.getName()+":"+currentLinkDevice.getName()+":"+currentParentInterface);
								
								if((!currentLinkDevice.getName().equals(commonLinkDevice.getName())) && (currentLinkDevice.getResourceType().getResourceType().endsWith("VM"))){	
						...
						if(d.getResourceType().getCount()>1){
							if(d.getIPAddress()!=null){
								for(int i=0;i<d.getResourceType().getCount();i++){
                                    ...
                                    newRequestConnection = generateRequestConnection(connection,d.getVMImageURL(),d.getVMImageGUID(),ip,netmask,i,path_position,isVT, group, d.getPostBootScript(),d.getName());
									newRequestMap.put(newRequestConnection.getName(), newRequestConnection);
								}
							}	
	...
	//generate the reservation dependency due to the master-slave relationship, after the inter-cloud connection computation
	...
	public void generateVMDependency(Hashtable <String,LinkedList <Device> > connectionList) throws IOException{
		...
		for(Device d: cloudPartition){
			Hashtable <String,NetworkConnection> vcRequestMap = getVCRequestMap(d,requestMap,partition,cloudPartition.size(),position);
			logger.debug("Cloud partition provisioning:"+d.getUri()+":"+d.getName()+":"+d.getResourceType().toString()+";subRequest size:"+partition+":"+vcRequestMap.size()+":"+position+":"+requestMap.entrySet().size());
	...
	public void adjustDependency(LinkedList <Device> newDList,Device d,int partition){
		...
		for (Device child:newDList){
			//System.out.println("New dList:"+child.getName()+":"+child.getUri()+":"+child.getIPAddress()+":dIPAddress:"+d.IPAddress);
			...
			if(d.getPrecededBy()!=null){
				for(Entry<Device,Resource> parent : d.getPrecededBySet()){
					logger.info("Inherite the parent:"+parent.getKey().getName()+";rType:"+parent.getKey().getResourceType().toString());
	...
	public Hashtable <String,NetworkConnection> getVCRequestMap(Device d, Hashtable <String,NetworkConnection> requestMap,int partition,int numPartition,int position){
		Hashtable <String,NetworkConnection> vcRequestMap = new Hashtable <String,NetworkConnection> ();
		Device cluster;
		int starting=0;
		logger.debug("getVCRequestMap:"+partition+":"+numPartition+":"+position+":"+starting+":"+requestMap.entrySet().size()+"---d:"+d.getResourceType().toString());
		for(Entry <String,NetworkConnection > entry:requestMap.entrySet()){
			starting++;
			...
			if(requestMap.size()==1){
				...
				entry.getValue().setDevice1(d);
                        ...
                        if(cluster.getUpNeighbour()==null){ //the master node
                                ...
                                logger.info("This is the master node:"+cluster.getUri()+":"+cluster.getName()+":"+cluster.getIPAddress());
                        }
	...
	public NetworkConnection generateVCConnection(DomainResourceType rType){
		...
		logger.info("InterCloud VC connection:"+x1+":"+x2+":"+numVMSite+";"+connectionName+":"+rs0_str+":"+rs1_str+":"+connection.getEndPoint1_domain()+":"+connection.getEndPoint2_domain());
		
		return connection;
	}
	//generate a separate request connection for each VM because of the IP address assignment
	public NetworkConnection generateRequestConnection(NetworkConnection requestConnection, String imageURL, String imageGUID, String IPAddress, String netmask,int i,int path_position,boolean isVT, String group, String postBootScript,String dName) throws InetNetworkException, UnknownHostException{
		NetworkConnection connection = new NetworkConnection();
		...
		//System.out.println("New request:"+connection.name+":"+connection.getEndPoint1_ip()+":"+cluster.resource+":"+type);
	...
	public void getDomainResourcePools(ResourcePoolsDescriptor pools){
		...
		for (ResourcePoolDescriptor rpd : pools) {
            ...
            //logger.debug("******From Resource Pool:"+ rpd.getResourceType().getType()+":"+rpd.getAttribute(ResourceProperties.ResourceAvailableUnits).getIntValue());
	...
	public InfModel buildCloudOntModel(){
		...
		for(i=0;i<size;i++){
			...
			for(j=i+1;j<size;j++){
				...
				logger.debug("Cloud:"+c_s_rs+":"+c_d_rs+":"+source_intf_rs.getProperty(this.mapper.numHop).getInt()+":"+destination_intf_rs.getProperty(this.mapper.bandwidth).getLong()+"\n");
			}
		}
	...
	public Hashtable <Integer,String> generateGraph(Hashtable <String,NetworkConnection> connectionList) throws IOException{
		...
		for(Entry <String,NetworkConnection > entry:connectionList.entrySet()){	
			...
			if((requestConnection.getEndPoint1_domain()!=null)&&(requestConnection.getEndPoint2_domain()!=null)){
	...
	public void outputGraph(String outputFile, int numNode, Hashtable <String,NetworkConnection> connectionList) throws IOException{
	    Writer output = null;
	    ...
	    for(Entry <String,NetworkConnection > entry:connectionList.entrySet()){	
			...
			if(bw==0) bw=randomGenerator.nextInt(150)+1;
