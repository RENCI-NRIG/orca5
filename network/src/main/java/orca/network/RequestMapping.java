...
public class RequestMapping extends SliceRequest {
	...
	public NetworkConnection deviceMapping() throws RequestMappingException, UnknownHostException, InetNetworkException{
		...
		if(connectionNodeList.isEmpty()) return null;
	...
	public int deviceConnected(Resource rs1, Resource rs2, int gress,NetworkConnection requestConnection) throws OntologyException{
		boolean validSwitching=true;

		ArrayList<ArrayList<OntResource>> solution = findShortestPath(ontModel,rs1,rs2,requestConnection.getBw(),null);
	...
	public void findCommonLabel(LinkedList <Device> deviceList){
		...
		while(it.hasNext()){
			...
			if(count==1) continue;
			if(count==numDevice) continue; //the first and last devices are the servers or fake, ignore.
	...
	public BitSet findAvailableLabel(Device device, String currentLayer){
		...
		for(int j=0;j<actionCount;j++){	
			...
			logger.debug("--Find Available Label:"+device.getURI()+":"+actionCount+":"+currentLayer+":"+action.getAtLayer());
			if(action.getAtLayer().equals(currentLayer)){
				...
				for(int i=0;i<size;i++){
					...
					logger.debug(intf.getURI()+";"+layer+":"+layer_rank+";"+currentLayer+":"+currentLayer_rank+";"+i);
	...
	public BitSet getAvailableLabelRange(Interface intf, String currentLayer){
		...
		while (results.hasNext()){
			...
			logger.debug("\n getAvailableLabelRange:-------Label Range:"+availableSet_rs+"----:"+lower+":"+upper+":"+lowerLabel+":"+upperLabel+"\n");
		}
	...
	public void processAdaptation(Device device, String currentLayer){
		...
		for(int j=0;j<actionCount;j++){
			action = (SwitchingAction) actionList.get(j);
			logger.debug("---------Adaptation Client --------"+device.getURI()+":"+actionCount+":"+currentLayer+":"+action.getAtLayer());
			if(action.getAtLayer().equals(currentLayer)){
				...
				if(tmp!=null){
					...
					if (tmp_set!=null) {
			            //usedLabelSet = ontModel.createProperty(NdlCommons.ORCA_NS + "layer.owl#" + Layer.valueOf(layer).getUSet());
		...
		if(clientList==null) {
			...
			if(currentLayer.equals(Layer.LambdaNetworkElement.toString())){
				if(parent.getAtLayer().equals(Layer.OCGNetworkElement.toString())){
					...
					if(label_rs!=null) labelID=label_rs.getProperty(label_ID).getInt();
			...
			while(it.hasNext()){
				intf=(Interface) it.next();
				intf.getResource().addProperty(interfaceOf,parent.getResource().getProperty(interfaceOf).getResource());
				if(intf.getAtLayer().equals(currentLayer)) {
	...
	public Interface processClientInterface(Interface intf, Label currentLabel,Label labelFromStack){
		...
		if(intf.getAtLayer().equals(Layer.EthernetNetworkElement.toString())){	
			...
			if(label_rs!=null) label=label_rs.getProperty(label_ID).getInt();
...
//			else prefix="dtn";
	...
	public Resource processLabel(Interface intf,Label currentLabel,Label labelFromStack){
        ...
        Resource label_rs=processLabel(rsURI,availableLableSet,usedLabelSet,usedLabelSet_str,usedSet,currentLabel,labelFromStack,currentLayer);
        
        return label_rs;
	}
	
	public Resource processLabel(String rsURI, String availableLableSet_str,String usedLabelSet, String usedLabelSet_str, String usedSet_str,Label currentLabel, Label labelFromStack,String currentLayer){
	...
	if(label>0){
		...
		while (results.hasNext()){
			...
			logger.debug("\n Label Range:"+availableSet_rs+"-"+lowestLabelRange_rs+"----:"+lower+":"+lowestUpper+":"+lowerLabel+":"+lowestUpperLabel+":StackLabel="+stackLabel+":Label="+label);
			i++;
		}
	}	
		...
		Resource picked_label_rs=ontLabelUpdate(availableSet_rs, lowestLabelRange_rs, lowest,lowestUpper,lowestLabel,lowestUpperLabel,stackLabel);	
		 ...
		 if(deviceConnection!=null){
			...
			while(it.hasNext()){
				...
				for(int i=j+1;i<size;i++){
					...
					logger.debug("Device pair:"+device.getURI()+":"+type+";"+device_peer.getURI()+":"+type_peer+"\n");
									...
									if(type_peer_next!=null){
										if(!type_peer_next.equals("Server")){
											if(peerLayer(device,device_peer_next)){
												...
												connection_ont=createConnection(device_peer,device_peer_next,deviceList);
			...
			reservationOntClass=(Resource) requestModel.getOntClass(NdlCommons.ORCA_NS+"request.owl#Reservation");
			...
			for (OntResource top:top_connection_ont) {
				...
				logger.info("RequestMapping: carryReservation: " + top.getURI() + ";random.reservation: " + random_reservation);
				//top.addProperty(carryReservation, this.reservation);
	...
	public OntResource createConnection(Device device, Device device_peer,LinkedList <Device> deviceList){
		...
		logger.debug("This VC end point 2:"+intf_peer.getURI()+":"+neighbor_device.getURI()+":"+neighbor_device.getDownNeighbour().getURI());
	...
	public OntResource toNetworkConnection(Resource intf0_rs,Resource intf1_rs,Device device0, Device device1){
       ...
       for (Entry <String, Resource []> link : linkList.entrySet()){
    	   if(link.getValue()[1].getProperty(interfaceOf).getResource()!=intf0_rs.getProperty(interfaceOf).getResource() & link.getValue()[2].getProperty(interfaceOf).getResource()!=intf0_rs.getProperty(interfaceOf).getResource()){
    		   if(link.getValue()[1].getProperty(interfaceOf).getResource()!=intf1_rs.getProperty(interfaceOf).getResource() & link.getValue()[2].getProperty(interfaceOf).getResource()!=intf1_rs.getProperty(interfaceOf).getResource())
    		   {
	...
	public NetworkConnection releaseConnection(NetworkConnection connection, String requestURI){
		...
		if(connection!=null){
			...
			while(it.hasNext()){
				...
				/*if(releaseCRS(device,false,requestURI)){
					Device tmpDevice = getDevice(device.getResource(),releaseDeviceList);
					if(tmpDevice==null) {
						logger.debug("To Release:"+device.getURI());
						releaseDeviceList.add(device);
					...
					if(it.hasNext()){
						device=(Device) it.next();  //it will stop after the peer
						tmpDevice = getDevice(device.getResource(),releaseDeviceList);
						if(tmpDevice==null) {
							if(releaseCRS(device,false,requestURI)){
								logger.debug("To Release next:"+device.getURI());
								releaseDeviceList.add(device);
							}
						}
					}
	...
	public boolean switchingActionInModel(SwitchingAction action, String current_layer, boolean deleteFlag, boolean releaseFlag,String requestURI){
	...
	public boolean removeConnection(OntResource rs_ont,OntResource rs_next_ont,boolean releaseFlag,String requestURI){
	...
	public Device getReleaseDevice(Resource crs_rs){
		...
		if(device == null){
			...
			if(tmpDevice!=null) device=null;
		}
		if(device!=null) logger.info("Release CRS device:"+device.getURI());
	...
	public Resource returnLabel(SwitchingAction action, Interface intf1, Interface intf2){
		...
		if(parent!=null) layer=parent.getAtLayer();
		...
		if(rs1_parent_availableSet==null) return null;
		...
		if(rs1_parent.getProperty(uSet_p)!=null){
			...
			logger.info("Returned used label:"+label_rs+":"+rs1_parent_availableSet.hasProperty(element,label_rs)+":"+rs1_parent_usedSet+":"+rs1_parent_usedSet.hasProperty(element,label_rs)+"\n");
		}
		return label_rs;
	}
