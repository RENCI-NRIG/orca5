...
public class CloudHandler extends MappingHandler {
    ...
    public LinkedList<Device> handleMapping(boolean isConnection) {
        ...
        for (Entry<String, NetworkConnection> entry : requestMap.entrySet()) {
            ...
            logger.info("Request Connection:" + connectionName + ":" + connectionType + ":" + rs1_str + ":" + rs1_ip_str
                    + "-" + rs2_str + ":" + rs2_ip_str + " BW=" + requestConnection.getBw());
            if ((connectionName != "None") && (isConnection == true) && (!connectionType.equals("None"))) {
                if (!(connectionName.split("\\/")[0].equalsIgnoreCase("None"))
                        || ((connectionName.split("\\/")[0].equalsIgnoreCase("None")) && (numLink == 0))) { // Nodegroup
                                                                                                            // w/IP
                                                                                                            // request
                    ...
                    logger.debug("CloudHandler: link_device: url:name " + link_device.getUri() + ":"
                            + link_device.getName() + " BW=" + requestConnection.getBw());
                    type.setCount(1);
            ...
            if (rs1_str != null) {
                ...
                logger.debug("Device " + rs1 + " imageURL=" + device.getVMImageURL() + " imageGUID="
                        + device.getVMImageGUID() + " postBootScript=" + device.getPostBootScript());
                ...
                logger.info("CloudHandler: vm_device 1: url:name " + device.getUri() + ":" + device.getName() + ":"
                        + device.getResourceType().toString());
                if (link_device != null)
            ...
            if (rs2_str != null) {
                ...
                type.setResourceType(
                        mapper.getResourceType("http://geni-orca.renci.org/owl/compute.owl#VM").getResourceType());
                ...
                logger.debug("Device=" + rs2 + " imageURL=" + device.getVMImageURL() + " imageGUID="
                        + device.getVMImageGUID() + " postBootScript=" + device.getPostBootScript());
                ...
                logger.info("CloudHandler: vm_device 2: url:name " + device.getUri() + ":" + device.getName() + ":"
                        + device.getResourceType().toString());
                if (link_device != null)
                    logger.info("link_device:" + link_device.getURI() + ":" + link_device.getName());
            }
            ...
            if (rs_parent != null) {
                for (Device parent : deviceList) {
                    ...
                    if ((rs_parent != null) && (parent.getResource() != null)) {
                        if (parent.getResource().getURI() == null)
                            continue;
                        if ((parent.getResource().getURI().contains(rs_parent.getURI()))
                                || (parent.getName().contains(rs_parent.getURI()))) {
                            if (child.getName() == null)
                                continue;
                            if (!child.getName().equals(parent.getName())) {
                                logger.info("Master-Slave to be:" + child.getResource() + ":" + rs_parent + ":"
                                        + parent.getResource());
