...
public class Ndl implements LayerConstant {
    ...
    public static void main(String[] args) {
        ...
        if (v != null)
            System.out.println("Type:" + type + ":" + rs_ont.getNameSpace() + ":" + v.getLiteral());
        ...
        String queryPhrase1 = "SELECT ?resource " + "FROM <http://geni-orca.renci.org/owl/ben.rdf>" + "WHERE {"
        ...
        String queryPhrase2 = "SELECT ?p ?r ?s " + "FROM <http://geni-orca.renci.org/owl/ben-dtn.rdf> " + "WHERE {"
                + "<" + NdlCommons.ORCA_NS + "ben-dtn.rdf#Renci/Infinera/DTN/t1B/1/lambda>" + " ?p ?r."
                + " ?p rdf:type layer:AdaptationProperty" + "      }";

        String queryPhrase3 = "SELECT ?a ?b ?c " + "" + "WHERE {" + "(" + url1
                + " '[ndl:hasInterface]+/[ndl:linkTo]+/[ndl:interfaceOf]+'" + url2 + ") gleen:Subgraph (?a ?b ?c)"
                + "      }";

        String queryPhrase4 = "SELECT ?r " + "FROM <http://geni-orca.renci.org/owl/ben-dell.rdf> " + "WHERE {" + url3
                + " rdfs:label ?r" + "      }";
