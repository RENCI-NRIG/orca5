...
public class InterCloudHandlerTest extends TestCase {
    ...
    String[] inputFileName = { "orca/network/gpovmsite.rdf", "orca/network/dukevmsite.rdf",
            "orca/network/rencivmsite.rdf", "orca/network/ricevmsite.rdf", "orca/network/uhoustonvmsite.rdf",
            "orca/network/uncvmsite.rdf", "orca/network/nersc.rdf", "orca/network/ben-6509.rdf", "orca/network/nlr.rdf",
            "orca/network/starlight.rdf", "orca/network/ion.rdf", "orca/network/nerscNet.rdf",
            "orca/network/renciNet.rdf", "orca/network/uncNet.rdf", "orca/network/dukeNet.rdf",
            "orca/network/riceNet.rdf", "orca/network/uhoustonNet.rdf", "orca/network/learnNet.rdf",
            "orca/network/learn.rdf" };
    ...
    protected void setUp() throws Exception {
        ...
        requestFileDukeVMSVlan = "orca/network/duke-vms-vlan-request.rdf"; // a vlan connected 6-node cluster in duke
                                                                           // vmsite
        requestFileRenciVMSVlan = "orca/network/renci-vms-vlan-request.rdf"; // a vlan connected 3-node cluster in renci
                                                                             // vmsite
    ...
    public void testRequestHandle() throws IOException, InetNetworkException, RequestMappingException {
        ...
        Individual link_ont = model.createIndividual("http://geni-orca.renci.org/owl/compute.owl#testServer",
                handler.mapper.serverCloudOntClass);

        handler.handleCloudRequest(requestFileDukeVMSVlan);

        System.out.println("Term:" + handler.mapper.startTime + ":" + handler.mapper.endTime + ":"
                + handler.mapper.reservation + "\n");
    ...
    public void print(LinkedList<Device> list) throws IOException, RequestMappingException {
        ...
        for (Device d : list) {
            ...
            // System.out.println(d.getSwappingCapability()+":"+d.isLabelProducer()+":"+d.getDegree()+":"+d.isDepend()+"IP
            // Adress:"+d.getIPAddress()+"\n");
            ...
            if (d.getPrecededBy() != null) {
                for (Entry<Device, Resource> parent : d.getPrecededBySet()) {
                    ...
                    System.out.println("Precedded By:" + device.getName() + ":" + device.getURI() + ":" + type + ":"
                            + parent.getValue().getProperty(handler.getMapper().RDFS_Label));
                    System.out.println(intf_name + "\n");
                    // System.out.println("Static Label:
                    // parent:"+parent.getKey().getStaticLabel()+";d:"+d.getStaticLabel());
                    // System.out.println("Type,
                    // postBootScript:"+type+":"+d.getPostBootScript()+":"+parent.getKey().getIPAddress());
                    if (type.endsWith("VM")) {
                    ...
                    // interface:"+parent.getValue()+";"+parent.getKey().getDownNeighbour()+":"+parent.getKey().getUpNeighbour());
                    ...
                    if (site_host_interface == null) {
                        // System.out.println("Host Interface Definition not here: IP address is used as the parent
                        // value or its neighbors are network domains!!");
                        if (parent.getKey().getDownNeighbour() != null) {
                            if (parent.getKey().getDownNeighbour().getProperty(hostInterfaceName) != null) {
                                site_host_interface = parent.getKey().getDownNeighbour().getProperty(hostInterfaceName)
                                        .getString();
                            } else {
                    ...
                    if (site_host_interface == null) {
                        // System.out.println("Host Interface Definition not here: neither up neighbor or down
                        // neighbor!!");
            ...
            if (d.getFollowedBy() != null) {
                for (Entry<Device, Resource> follower : d.getFollowedBySet()) {
                    ...
                    System.out.println("Followed By:" + device.getName() + ":" + device.getURI() + ":" + type + ":"
                            + follower.getValue().getProperty(handler.getMapper().RDFS_Label));
                    System.out.println(intf_name + "\n");
                }
            ...
            for (SwitchingAction a : actions) {
                // System.out.print("Action=" + a.getDefaultAction() + " Label=" + a.getLabel_ID()+ " BandWidth = "
                // +a.getBw()+"\n");
