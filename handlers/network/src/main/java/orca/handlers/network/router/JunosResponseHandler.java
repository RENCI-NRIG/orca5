...
/***
 * Handle responses from Junos devices for the stages: 0. Session open 1. Login 2. Configuration update 3. Commit 4.
 * Session close
 * 
 * @author ibaldin
 *
...
public class JunosResponseHandler extends DefaultHandler {
    ...
    protected enum SessionStages {
        Initial, SessionOpen, LoggedIn, ConfigurationUpdated, ConfigurationCommitted, SessionEnded, Completed, Error, Warning, EndOfDoc
    };
    ...
    public void startElement(String uri, String name, String qname, Attributes attributes) throws SAXException {
        ...
        if ((localName.equals("junoscript")) && (stage == SessionStages.Initial)) {
            // check junos version?
            ...
            if (nextStage == SessionStages.Warning) {
                ...
                if (message.matches("^\\s*statement not found[.|\\s]*$")) {
                    ...
                    throw new RuntimeException(
                            "XML Reply returned warning: " + message + " in stage [" + getStage() + "]");
            }
            advanceStage();
            inReply = false;
            return;
