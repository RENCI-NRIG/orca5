...
public abstract class JunosSSLDevice implements INetworkDevice, IJunosInteractor {    
	...
	static protected TrustManager[] trustAllCerts = new TrustManager[]{
	 ...
	 * @param uid
	 * @param password
	 * @param validate - validate server cert?
	 */
    ...
    public void sendHandshakeAndLogin() {
		...
		sendToDevice("<junoscript version=\"1.0\" hostname=\"" + hostname +"\" release=\"" + 
				junosRelease +"\">");
		sendToDevice("<rpc> <request-login> <username>" + uid + 
				"</username> <challenge-response>" + password + 
