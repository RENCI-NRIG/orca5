...
/**
 * Similar to EC2 .INI file generator, this generates a bash script that can be executed by a baremetal node upon boot
...
public class XCatGenerateBashFileTask extends XCatGenerateBashFileBaseTask {
    ...
    protected void generateInterfaces(PrintWriter out) throws Exception {
        ...
        for (int i = 0; i < eths.length; i++) {
            ...
            if (mode.equals("vlan")) {
                ...
                if (ip != null) {
                    System.out.println(
                            "Configuring eth" + eth.toString() + " on " + hosteth + " vlan=" + tag + ", ip=" + ip);
                } else {
                    System.out.println(
                            "Configuring eth" + eth.toString() + " on " + hosteth + " vlan=" + tag + ", ip=[no ip]");
                }
    ...
    protected void generateStorage(PrintWriter out) throws Exception {
        ...
        for (int i = 0; i < stores.length; i++) {
            ...
            bashStorageCommands(out, i, store_type, target_ip, target_port, target_lun, target_chap_user,
