...
public class XCatGenerateBashFileAddIfaceTask extends XCatGenerateBashFileModTask {
    ...
    protected void generateInterface(PrintWriter out) throws Exception {
        ...
        if (mode.equals("vlan")) {
            ...
            if (ip != null) {
                System.out.println("Configuring modify interface " + modifyIndex + " on " + hosteth + " vlan=" + tag
                        + ", ip=" + ip);
            } else {
                System.out.println("Configuring modify interface " + modifyIndex + " on " + hosteth + " vlan=" + tag
                        + ", ip=[no ip]");
    ...
    protected void generateStorage(PrintWriter out) throws Exception {
        ...
        String target_should_attach = getProject()
                .getProperty(modifyPrefix() + UnitProperties.UnitTargetShouldAttachSuffix);
        ...
        String target_chap_secret = getProject()
                .getProperty(modifyPrefix() + UnitProperties.UnitTargetChapSecretSuffix);
        ...
        if (fs_type == null) {
            System.out
                    .println("Store " + modifyPrefix() + " does not specify file system type. Assuming empty string.");
            fs_type = "";
        }
        ...
        if (fs_options == null) {
            System.out.println(
                    "Store " + modifyPrefix() + " does not specify file system options. Assuming empty string.");
            fs_options = "";
        }
        ...
        if (fs_should_format == null) {
            System.out.println("Store " + modifyPrefix()
        ...
        bashStorageCommands(out, 100 + modifyIndex, store_type, target_ip, target_port, target_lun, target_chap_user,
                target_chap_secret, target_should_attach, fs_type, fs_options, fs_should_format, fs_mount_point);
    }
