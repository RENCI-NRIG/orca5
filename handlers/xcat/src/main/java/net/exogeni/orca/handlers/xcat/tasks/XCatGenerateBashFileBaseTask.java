package net.exogeni.orca.handlers.xcat.tasks;
...
public class XCatGenerateBashFileBaseTask extends OrcaAntTask {
    ...
    private static final String[] netmaskConverter = { "128.0.0.0", "192.0.0.0", "224.0.0.0", "240.0.0.0", "248.0.0.0",
    ...
    protected void bashStorageCommands(PrintWriter out, int i, String store_type, String target_ip, String target_port,
            String target_lun, String target_chap_user, String target_chap_secret, String target_should_attach,
        ...
        if (target_should_attach.equalsIgnoreCase("yes")) {
            ...
            out.println("TARGET=`/sbin/iscsiadm --mode discovery --type  sendtargets --portal " + target_ip
                    + " | grep '^" + target_ip + "' | awk '{ print $2}'`");

            out.println("/sbin/iscsiadm --mode node --targetname $TARGET --portal " + target_ip + ":" + target_port
                    + " --op=update --name node.session.auth.authmethod --value=CHAP");
            out.println("/sbin/iscsiadm --mode node --targetname $TARGET --portal " + target_ip + ":" + target_port
                    + " --op=update --name node.session.auth.username --value=" + target_chap_user);
            out.println("/sbin/iscsiadm --mode node --targetname $TARGET --portal " + target_ip + ":" + target_port
                    + " --op=update --name node.session.auth.password --value=" + target_chap_secret);
            out.println("/sbin/iscsiadm --mode node --targetname $TARGET --portal " + target_ip + ":" + target_port
                    + " --login");
            ...
            if (fs_should_format.equalsIgnoreCase("yes")) {
                // format
                out.println("DEVICE='/dev/disk/by-path/ip-" + target_ip + ":" + target_port + "-iscsi-'${TARGET}'-lun-"
                        + target_lun + "'");
