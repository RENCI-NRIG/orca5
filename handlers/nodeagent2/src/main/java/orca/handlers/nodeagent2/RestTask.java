...
public abstract class RestTask extends OrcaAntTask {
    ...
    protected void doPost() throws BuildException {
        ...
        try {
            client.getCredentialsProvider().setCredentials(new AuthScope(getHost(), getPort()),
            ...
            HttpPost post = new HttpPost(na2Url + "/" + rop.name().toLowerCase() + "/" + plugin
    ...
    protected void _execute() throws BuildException {
        // check validity
        if ((plugin == null) || (password == null) || (na2Url == null) || (prefix == null) || (returnPrefix == null))
            throw new BuildException(
                    "The following input attributes must be specified for the task: plugin, password, baseUrl, prefix and returnPrefix");

        if ((statusProperty == null) || (reservationIdProperty == null) || (errorMsgProperty == null))
            throw new BuildException(
                    "The following attributes denoting expected property names must be specified in order to produce output: statusProperty, reservationIdProperty, errorMsgProperty");
