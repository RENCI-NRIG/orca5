...
try:
    ...
    resp = comet.get_family(comethost, cid, rId, readToken, family)
    ...
    if resp.json()["value"] and not resp.json()["value"]["error"]:
        ...
        resp = comet.update_family(comethost, cid, rId, readToken, writeToken, family, updatedVal)
        if resp.status_code != 200:
            raise ApiError('Cannot Update Family: {}'.format(resp.status_code))

    resp = comet.enumerate_families(comethost, cid, readToken)
    if resp.status_code != 200:
    ...
    resp = comet.delete_family(comethost, cid, rId, readToken, writeToken, family)
    if resp.status_code != 200:
        raise ApiError('Cannot Delete Family: {}'.format(resp.status_code))

    r = comet.delete_families(comethost, cid, rId, readToken, writeToken)
    if r != True:
