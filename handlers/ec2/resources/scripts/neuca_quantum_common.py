...
class NEuca_Quantum_Network:
    ...
    def create_network(self, net_type, network, vlan_tag, dataplane_network, openflow_network, project_name, max_rate=None, burst_rate=None ):
        ...
        cmd = ["openstack", "network", "create", 
               ...
               "--provider-physical-network", physNetwork, 
               "--provider-segment", str(vlan_tag), 
               "--disable-port-security", str(name) , "-fjson" ]
    ...
    def _clean_port(self, network_uuid, port_uuid, instance_id):
        ...
        try:
            if instance_id.strip():
                self._neutron_remove_port_from_server(network_uuid, port_uuid, instance_id)
    ...
    def _get_all_ports_for_network(self, network_uuid):
        # openstack port list --network $network_uuid -fjson
        cmd = ["openstack", "port", "list", "--network", str(network_uuid), "-fjson"]
        rtncode, data_stdout, data_stderr = Commands.run(cmd, timeout=60)  # TODO: needs real timeout
    ...
    @classmethod
    def _clean_all_ports_for_network(self, network_uuid):
        for port in self._get_all_ports_for_network(network_uuid):
            try:
                n_uuid, p_uuid, i_id = NEuca_Quantum_Network.__get_network_and_port_by_port_name_or_id(port)
                self._clean_port(network_uuid, port, i_id)
            except Exception as e:
    ...
    def delete_network(self, network_uuid):
        try:
            self._clean_all_ports_for_network(network_uuid)
        except Exception as e:
    ...
    def add_iface_to_network(self, network_uuid, instance_id, instance_name, mac_addr, project_name):
        ...
        if rtncode == 0:
            LOG.debug(port_name + " with id " + data_stdout.strip() + " already exists")
            return data_stdout.strip() 

        cmd = ["openstack", "port", "create", 
               "--project", project_name,
    ...
    @classmethod
    def remove_iface(self, instance_id, iface_name):
        # get network_uuid and port_uuid

        n_uuid, p_uuid, i_id  = NEuca_Quantum_Network.__get_network_and_port_by_port_name_or_id(iface_name)

        self._clean_port(n_uuid, p_uuid, i_id)

        return 'OK'


    @classmethod
    def remove_all_vm_ifaces(self, iface_uuids):
        for iface_uuid in iface_uuids:
            try:
                n_uuid, p_uuid, i_id  = NEuca_Quantum_Network.__get_network_and_port_by_port_name_or_id(iface_uuid)

                self._clean_port(n_uuid, p_uuid, i_id)
            except Exception as e:
                LOG.error("Exception occured while deleting iface e=" + str(e))
        return 'OK'
