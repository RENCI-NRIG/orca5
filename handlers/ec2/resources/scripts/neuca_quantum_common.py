...
class NEuca_Quantum_Network:
    ...
    def _quantum_delete_port(self, tenant_id, network_uuid, port_uuid):
        ...
        if line == 'Command failed with error code: 409':
            raise NEuca_Quantum_Port_Plugged_In_Exception, "Delete Port Failed, A resource is currently attached to the logical port: cmd = " + str(
        ...
        if reported_network_uuid != network_uuid or reported_port_uuid != port_uuid or reported_tenant_id != tenant_id:
            raise NEuca_Quantum_Exception, "Delete Port Failed, bad stdout reported network uuid does not match uuid: cmd = " + str(
                cmd) + "\nstdout = " + str(data_stdout)
    ...
    def _quantum_unplug_iface(self, tenant_id, network_uuid, port_uuid):
        ...
        if reported_network_uuid != network_uuid or reported_port_uuid != port_uuid or reported_tenant_id != tenant_id:
            raise NEuca_Quantum_Exception, "Unplug iface Failed, bad stdout reported network uuid does not match uuid: cmd = " + str(
                cmd) + "\nstdout = " + str(data_stdout)
    ...
    def _clean_iface(self, tenant_id, network_uuid, port_uuid):
        ...
        if delete_port_e != None or unplug_iface_e != e:
            raise NEuca_Quantum_Exception, "unplug_iface exception: " + str(
                unplug_iface_e) + ", delete_port_exception: " + str(delete_port_e)
    ...
    def delete_network(self, tenant_id, network_uuid):
        ...
        if reported_network_uuid != network_uuid:
            raise NEuca_Quantum_Exception, "Delete Network Failed, bad stdout reported network uuid does not match uuid.  reported_network_uuid = " + str(
                reported_network_uuid) + ",  network_uuid=" + str(network_uuid) + ", cmd = " + str(
                cmd) + "\nstdout = " + str(data_stdout)
    ...
    def add_iface_to_network(self, tenant_id, network_uuid, instance_name, mac_addr):
        ...
        if rtncode != 0:
            ...
            raise NEuca_Quantum_Exception, "Add iface Failed (plug_iface), bad error code (" + str(
                rtncode) + ") : " + str(cmd)
        ...
        if len(data_split) >= 14:
            ...
            self._clean_iface(tenant_id, network_uuid, port_uuid)
            raise NEuca_Quantum_Exception, "Add iface Failed (plug_iface), bad stdout: cmd = " + str(
                cmd) + "\nstdout = " + str(data_stdout)
        ...
        if reported_iface_name != iface_name or reported_port_uuid != port_uuid or reported_network_uuid != network_uuid or reported_tenant_id != tenant_id:
            self._clean_iface(tenant_id, network_uuid, port_uuid)
            raise NEuca_Quantum_Exception, "Delete Network Failed, bad stdout reported info does not match supplied info: cmd = " + str(
                cmd) + "\nstdout = " + str(data_stdout)
    ...
    def get_network_uuid_for_port(self, tenant_id, iface_uuid):
        ...
        if rtncode != 0:
            raise NEuca_Quantum_Exception, "list_nets_detail failed, bad error code (" + str(rtncode) + ") : " + str(
                cmd)
        ...
        for line in lines:
            ...
            if len(line) >= 5:
                ...
                ports_rtncode, ports_data_stdout, ports_data_stderr = Commands.run(ports_cmd,
                                                                                   timeout=60)  # TODO: needs real timeout
                ...
                if ports_rtncode != 0:
                    raise NEuca_Quantum_Exception, "list_ports failed, bad error code (" + str(
                        ports_rtncode) + ") : " + str(ports_cmd)
