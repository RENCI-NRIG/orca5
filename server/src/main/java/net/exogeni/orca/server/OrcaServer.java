...
public class OrcaServer {
    ...
    private static int getServerPort(String env, int def) {
        ...
        if (System.getProperty(env) != null) {
            try {
                ...
                System.err.println("Unable to parse server port number " + System.getenv(env) + ", using default " + def
                        + " instead");
            }
    ...
    public void start() throws Exception {
        if (System.getProperty(PROPERTY_BASEDIR) != null) {
            System.err.println(
                    "WARNING: basedir property is set system-wide. ORCA requires this property is NOT set. Clearing and proceeding.");
        ...
        servletHandler.getServletContext().setAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE,
                wctx);
        ...
        // shutdown net.exogeni.orca when the server is about to stop
    ...
    static class OrcaServerContextListener implements ServletContextListener {
        public void contextInitialized(ServletContextEvent event) {
            try {
                Globals.Log.info("Orca application context initializing port=" + ServerPort + " ORCA_HOME="
                        + Globals.HomeDirectory);
                ...
                ConfigurationContext context = ConfigurationContextFactory
                        .createConfigurationContextFromFileSystem(repo, config);
                context.setProperty(org.apache.axis2.Constants.CONTAINER_MANAGED,
