...
public abstract class ExportClaimTest extends ManagementTest {
	...
	public void testExport() {
		...
		ReservationID exported = site.exportResources(sliceId, start, end, 10, null, null, rid, new AuthToken(BROKER_NAME, BROKER_GUID));
		ReservationMng exp = site.getReservation(exported);
		Assert.assertNotNull(exp);

		System.out.println("Exported reservation: rid=" + exp.getReservationID() + " rtype=" +
				exp.getResourceType() + " units=" + exp.getUnits() + " start=" + (new Date(exp.getStart())) + 
				" end=" + (new Date(exp.getEnd())));
		...
		System.out.println("Claimed reservation: rid=" + claimed.getReservationID() + " rtype=" +
				claimed.getResourceType() + " units=" + claimed.getUnits() + " start=" + (new Date(claimed.getStart())) + 
				" end=" + (new Date(claimed.getEnd())));
	}
}