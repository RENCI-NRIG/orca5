 ...
 * the classes are properly annotated. The intent is to run this tool as part of the build process to catch
 * persistence problems during build instead of at runtime.
 * Note: a successful execution of the tool does not guarantee that the code is free of persistence-related bugs.
 * This tool should be considered as a "meets-min" validator.
 * @author aydan
 */
	...
	private void validate(Class<? extends Persistable> c) throws PersistenceException {
		...
		if (!c.isInterface()){
			if (!ReflectionUtils.hasDefaultConstructor(c)){
				throw new PersistenceException("Class " + c.getName() + " does not provide a default (parameter-less) constructor");
			}
		}
	}
